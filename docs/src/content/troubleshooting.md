...
Every request this app answers has an id, and an error that was not your fault quotes it:

> Internal server error
> *Reference `a1b2c3d4`*

Give that to whoever runs the instance. Every line the server wrote while serving that request —
including the stack trace, and any warning from the mail sender, the vault or a webhook along the
way — carries the same string, so `a1b2c3d4` is one grep in the log:

```sh
grep a1b2c3d4 server.log
```

Which is a different proposition from "it broke at about three", where the log holds every request
from everybody in that minute and nothing says which one was yours.

The id is on *every* response as an `X-Request-Id` header, not only the failures, so it is in the
browser's network tab if you need one for a request that did not error. A reverse proxy that stamps
its own `X-Request-Id` has that honoured instead, so a trace already spans the proxy and the app.

> [!note]
> A refusal is not a malfunction, and does not quote an id. "That name is already taken" or "a
> booking cannot end before it starts" is the app answering you correctly — the reference appears
> only where something genuinely went wrong, so its presence is itself a signal.

## "That is a lot of reports in a short time"

Generating is the most expensive thing this app does: the template is filled, every figure
renumbered, every screenshot inlined, and a PDF starts an external converter on top of that. It is
seconds of work each time, and while one is running everybody else's requests wait behind it.

So there is a ceiling — twenty renders in five minutes, per account. It is generous enough that
the honest loop of *generate, look at page four, fix the template, generate again* never touches it,
and low enough that a stuck button or a held-down keyboard cannot take the instance down for the
floor. The allowance is per account, so one person tuning a template does not spend anybody else's.

Wait a few minutes and it clears on its own. If you hit it repeatedly during real work, the ceiling
