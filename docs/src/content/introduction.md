...
![The findings tab of an engagement in Engy Report. Six findings ordered by score, each with a severity chip, the category it belongs to, the person who wrote it and who owns the fix, above a bar showing the severity spread and a row of tabs for enumeration, scope, evidence, delivery and the rest of the engagement.](/shots/findings.jpg "An engagement, open at its findings. Everything else in this documentation is a tab along that row.")
...
- **Automating the dull parts.** A [versioned API](/api) with scoped tokens, so a scanner can file
  what it found, a build can read an engagement, and a tool run can be recorded without anybody
  pasting output into a box.
...
- **It does not lay a PDF out by itself.** It writes `.docx`, and converts that to PDF only if you
  point it at LibreOffice or Word — see [Generating](/generating). Without one, a `.docx` is opened
  in Word and saved as a PDF, and an HTML template can be printed from the browser.
...
- [The API](/api) — if the first thing you want to know is whether you can drive it from a script.
