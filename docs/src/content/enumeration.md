...
![The enumeration workbench. On the left a tree of ten tool runs grouped under three sections, each row showing its tool and how many lines of output it holds; on the right the selected step with its tool, target, when it ran, its outcome, the exact curl command and the response that came back.](/shots/enumeration.jpg "Ten steps under three sections. The green mark on a row means that step became a finding.")
...
### Recording one from the terminal

Pasting output into a box is the friction this tab has that nothing else does — the run has already
happened, in a terminal, and the tree wants it typed in somewhere else. [The API](/api) takes it
directly: `POST /api/v1/engagements/:id/enumeration` with a token scoped to `enumeration:write`
files a step with its title, tool, command, target and output, so a wrapper script or a shell alias
can record the run as it happens.

The step arrives in the tree like any other, and the activity log says which token filed it. What
the API cannot do is write the step's *notes* — that is prose a colleague reads, and a script
putting words in somebody's mouth is not a feature.
...
Press Ctrl/Cmd+K, type a hostname or a banner, and the last row offers **Search tool output**. That
is a second, deliberate search: the ordinary one covers engagements, findings, sections, notes and
the library, and has never touched a byte of tool output — output lives in its own collection at up
to 200 KB a step, and dragging that through a search that runs on every keystroke would make the
box useless.

Asked for explicitly it answers the question this app is uniquely placed to answer: *where have I
seen this host before*. You get the step, the engagement, and the matching lines with their line
numbers — the answer itself, not a place to go and look for it. Write-ups are searched too, and
labelled apart from the output.

It matches substrings rather than words, deliberately. `api-staging.acme.example`, `nginx/1.24.0`
and `10.0.5.0/24` are exactly the things a word index would break into pieces, and they are exactly
what people search for. Three characters minimum — anything shorter matches every sweep anybody has
ever pasted.

> [!note]
> Scoped to engagements you are on, and restricted engagements are excluded unless you could
> actually open one. Raw output is where a hash dump or a session cookie ends up, so a search that
> returned a line from an engagement you cannot open would be a way around that rule rather than an
> exception to it.
