...
Disabling also stops every [API token](/api) that account holds, in the same instant. That is the
lever worth knowing about: there is no route for an administrator to read or revoke somebody else's
tokens one at a time — "which credentials does this person hold" is not an authority that managing
