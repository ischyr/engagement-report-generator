...
export default function GlobalSearch() {
  ...
  /**
   * The output search, which is a second search rather than part of the first.
   *
   * Tool output is a separate collection holding up to 200 KB a step. Folding it into the search
   * that runs while somebody types would drag megabytes through the browser to answer a half-typed
   * word — so it is asked for explicitly, from a row at the bottom of the ordinary results, and
   * what comes back replaces them until the query changes.
   */
  ...
  /* A new query means the old output answer is about something else. */
  useEffect(() => {
    setOutput(null);
  }, [debounced]);
  ...
  /**
   * Ask the output collection, once, because somebody asked.
   *
   * Not debounced and not automatic: this is the expensive door, and it opens when a person
   * chooses it. What comes back replaces the ordinary results, which is honest — they were
   * answering a different question.
   */
  const searchOutput = async () => {
    setOutput({ results: [], loading: true, total: 0 });
    try {
      const data = await api.get(`/search/output?q=${encodeURIComponent(needle)}`);
      setOutput({ ...data, loading: false });
      setActive(0);
    } catch {
      setOutput({ results: [], loading: false, total: 0, failed: true });
    }
  };

  /*
   * Offered once the needle is long enough to mean something. Three characters, matching what the
   * endpoint will accept: a row that produces an error when chosen is worse than no row.
   */
  const canSearchOutput = !isSales && needle.length >= 3;
  const results = output ? output.results : state.results;
  ...
  const navigable = idle
    ...
    : /* The output row is the last thing the arrows reach, after everything already found. */
      [...results, ...(canSearchOutput && !output ? [{ __searchOutput: true, href: null }] : [])];

  const go = (result) => {
    if (!result) return;
    if (result.__searchOutput) return searchOutput();
  ...
  const hint = useMemo(() => {
    if (output?.loading) return 'Reading the output…';
    if (output?.failed) return 'The output search failed';
    if (output) {
      if (!output.results.length) return 'Nothing in any tool output';
      return `${output.total} step${output.total === 1 ? '' : 's'} matched${
        output.capped ? ' — and there may be more' : ''
      }`;
    }
  ...
  }, [state, output, query, results.length, opened.length, queries.length]);
          ...
          >
            ...
            <div className="max-h-[55vh] overflow-y-auto">
              ...
              */}
              {idle && (opened.length || queries.length) ? (
              ...
              ) : results.length === 0 && !(canSearchOutput && !output) ? (
                ...
                <ul>
                      ...
                      <li key={`${result.type}-${result.id}`}>
                        ...
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            {/*
                              The matched lines themselves, monospaced and with their line numbers.
                              A search of tool output that showed only which step matched would send
                              somebody to open a step and search it again by eye — the whole answer
                              is the line.
                            */}
                            {(result.lines ?? []).map((line) => (
                              <span
                                key={`o-${line.line}`}
                                className="mt-0.5 flex gap-2 font-mono text-[0.625rem] text-fg-subtle"
                              >
                                <span className="shrink-0 tabular-nums opacity-60">{line.line}</span>
                            ...
                            {(result.notes ?? []).map((line) => (
                              <span
                                key={`n-${line.line}`}
                                className="mt-0.5 block truncate text-[0.625rem] italic text-fg-subtle"
                              >
                                in the write-up: {line.text}
                              </span>
                            ))}
                            {result.internal ? (
                              <span className="mt-0.5 block text-[0.625rem] text-fg-subtle">
                                This step is held back from the report.
                              </span>
                            ) : null}
                          </span>
                  ...
                  {/*
                    ...
                    Last in the list on purpose: everything above was found without asking, and this
                    is the one that costs something. Reachable by arrow keys like any other row.
                  */}
                  {canSearchOutput && !output ? (
                    <li>
                      <button
                        type="button"
                        ...
                        className={cn(
                          'flex w-full items-center gap-3 border-t border-line-soft px-4 py-2.5 text-left transition',
                          active === results.length ? 'bg-brand-500/10' : 'hover:bg-white/[0.035]'
                        )}
                      >
                        <Terminal size={15} className="shrink-0 text-fg-subtle" />
                        <span className="min-w-0 flex-1 truncate text-sm text-fg">
                          Search tool output for “{needle}”
                        </span>
                        <span className="shrink-0 text-[0.625rem] uppercase tracking-wider text-fg-subtle">
                          Enumeration
                        </span>
