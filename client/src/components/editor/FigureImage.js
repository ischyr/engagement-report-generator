...
export const FigureImage = Image.extend({
  ...
  addAttributes() {
    return {
      ...this.parent?.(),
      /**
       * The recording this still frame came out of, as a `/api/media/…` URL.
       *
       * A document cannot hold a video, so a recording is stored beside a frame taken out of it
       * and the *frame* is what sits in the prose: it is an ordinary image, so figure numbering,
       * captions, cross-references and the .docx converter all go on working without knowing that
       * videos exist. This attribute is the only thing that does know, and all it does is let the
       * app play the recording when somebody opens the still.
       *
       * A URL rather than a bare id, deliberately: the orphan sweep finds evidence by looking for
       * `/api/media/<id>` in the HTML, so a recording referenced any other way would be collected
       * as unused while the still that stands for it stayed.
       */
      video: {
        default: '',
        parseHTML: (element) =>
          element.getAttribute?.('data-video') ??
          element.querySelector?.('img[src]')?.getAttribute('data-video') ??
          '',
