...
import { ArrowRightLeft, CheckCheck, Hash, Trash2, UserPlus, X } from 'lucide-react';

import { api } from '../../lib/api.js';
import { useToast } from '../../context/ToastContext.jsx';
import { useResource } from '../../hooks/useResource.js';
import { COMPLEXITY_LABELS, PRIORITY_LABELS, displayName } from '../../lib/utils.js';
...
export default function BulkFindingBar({ audit, ids, onClear, onDone }) {
  ...
  /*
   * Who the selection can be handed to. From the engagement, because the server refuses anybody
   * who is not on it — the same list the checklist and the findings rows offer.
   */
  const team = useMemo(() => {
    const seen = new Map();
      ...
      */}
      <div className="pointer-events-none fixed inset-x-0 bottom-4 z-30 flex justify-center px-4">
        <div className="pointer-events-auto flex flex-wrap items-center gap-2 rounded-xl border border-line bg-surface/95 px-3 py-2 shadow-lg shadow-black/40 backdrop-blur">
          ...
          {/* Only where there is somebody to hand them to. */}
          {team.length > 1 ? (
            <Button
              size="sm"
              variant="ghost"
      ...
      <Modal
        open={dialog === 'assign'}
        onClose={() => setDialog(null)}
        title={`Hand ${many} to somebody`}
        description="Splitting the writing up: “you take these nine, I will do the rest.” They get one notification for the batch, not one per finding."
        size="sm"
        footer={
          <>
            <Button variant="ghost" onClick={() => setDialog(null)}>
            ...
            <Button
              variant="primary"
              loading={busy === 'assign'}
              onClick={() => run({ action: 'assign', value: value || null }, 'assign')}
            >
              {value ? 'Assign' : 'Clear'}
            </Button>
          </>
      ...
      >
        <Select
          ...
          value={value}
          onChange={(event) => setValue(event.target.value)}
          hint="Only people on this engagement. Somebody who cannot open it cannot write it."
          options={[
            { value: '', label: 'Nobody — clear it' },
            ...team.map((member) => ({ value: String(member._id), label: displayName(member) })),
          ]}
        />
      </Modal>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          />
          <TagRow
            busy={busy === 'tag' || busy === 'untag'}
            onApply={(tag, action) => run({ action, value: tag }, action)}
          />
        </div>
      </Modal>
...
/**
 * A tag, added to or taken off everything ticked.
 *
 * Two buttons rather than one, because unlike every other row here this is not a value being *set*.
 * Setting the tags of nine findings would throw away whatever else each of them was labelled with,
 * and the reason to tag nine at once is that they have something in addition in common — "these all
 * need retesting", "these are the ones the client pushed back on". So: add, or remove, and leave
 * the rest alone.
 */
function TagRow({ busy, onApply }) {
  ...
  return (
    <div className="flex items-end gap-2">
      <Input
        label="Tag"
        hint="Yours to filter by. Never printed in the report."
