...
export default function EnumerationEditor({
  ...
  */
  ...
  /*
   * The four arguments every shared box on this step needs, in one place.
   *
   * Spread rather than repeated: six boxes each restating the engagement, the scope and the id is
   * six places for one of them to be wrong, and a room name that is wrong by one character is a
   * field that silently stops being shared.
   */
  const shared = (fieldName) => ({
    auditId: idOf(audit?._id),
    scope: 'step',
    id: idOf(selected?._id),
    field: fieldName,
    enabled: editable,
  });
              ...
              */}
              ...
              >
                ...
                <CollaborativeInput
                  ...
                  onChange={(value) => patch('ranAt', value)}
                />
                <Select
                  label="Phase"
                  value={draft.phase}
              ...
              */}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {draftFor === idOf(selected._id) ? (
                  /*
                    Shared with whoever else has this step open. Gated on the draft belonging to
                    this step for the same reason the finding editor gates on its write-up having
                    arrived: sharing a document seeded from text that is not loaded yet would share
                    an empty one.
                  */
                  <CollaborativeField
                    key={selected._id}
