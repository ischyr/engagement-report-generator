...
/** What a retest is booked for, when nobody says otherwise. Matches the server default. */
const RETEST_DAYS = 30;

const BLANK = {
  version: '',
  /* Not part of the delivery record: a request to put a day in the schedule alongside it. */
  retest: false,
  retestInDays: RETEST_DAYS,
  sentAt: '',
  channel: 'email',
  recipients: [],
  filename: '',
...
export default function DeliveryTab({ audit, editable, lastGenerated }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      /*
       * Only on a new record, and only when asked.
       *
       * Editing a delivery from three weeks ago must not book anything: the checkbox is about the
       * report going out now, and a correction to an old row is not that.
       */
      if (!form._id && form.retest) {
        payload.retest = true;
        payload.retestInDays = form.retestInDays || RETEST_DAYS;
      }
      let answer = null;
      if (form._id) await api.put(`/audits/${audit._id}/deliveries/${form._id}`, payload);
      else answer = await api.post(`/audits/${audit._id}/deliveries`, payload);
      setForm(null);
      await reload({ quiet: true });
      toast.success(
        form._id ? 'Delivery record updated' : 'Delivery recorded',
        /* Said only when it happened: a booking the server declined must not be announced. */
        answer?.retest?.on ? `Retest booked for ${answer.retest.on}.` : undefined
      );
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          actions={
            editable ? (
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" icon={Plus} onClick={() => open(null)}>
                  Record a delivery
                </Button>
                {/*
                  The primary action, because it is the one that makes the register trustworthy:
                  sending from here means the version, the hash and the recipients are observed
                  rather than retyped from a mail client afterwards.
                */}
                {/*
                  A copy each, when it matters who leaks it.

                  Beside the two existing actions rather than inside them: it is a different
                  decision from "record what I sent" and from "send it from here", and it is the
                  one you make before either.
                */}
                <Button
                  variant="ghost"
                  size="sm"
                ...
                <Button variant="primary" size="sm" icon={Mail} onClick={() => setSendOpen(true)}>
                  Send it by email
                </Button>
              </div>
            ) : null
      ...
      >
        {form ? (
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {/*
              And the retest, on a new record only.

              Here rather than in the schedule because this is the moment the date is obvious and
              somebody is already thinking about it. Off by default: plenty of recorded deliveries
              are drafts, and a schedule filling itself with retests nobody agreed to would be worse
              than an empty one. Hidden when editing, because correcting a row from three weeks ago
              is not a report going out.
            */}
            {form._id ? null : (
              <div className="sm:col-span-2 flex flex-col gap-2 rounded-lg border border-line-soft bg-canvas/40 px-3.5 py-3">
                <Toggle
                  checked={Boolean(form.retest)}
                  onChange={(next) => setForm({ ...form, retest: next })}
                  label="Book the retest"
                  hint="Puts a day in the schedule and lets the existing reminder chase it, so a retest cannot quietly slip after the report goes out."
                />
                {form.retest ? (
                  <Input
                    ...
                    value={form.retestInDays}
                    onChange={(event) =>
                      setForm({ ...form, retestInDays: Number(event.target.value) || RETEST_DAYS })
                    }
                  />
                ) : null}
              </div>
      ...
      */}
      {/*
        After the register and before the render history: the three cards answer, in order, what
        the client was sent, what they can see of it now, and what we generated along the way.
      */}
