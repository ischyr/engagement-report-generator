  ...
  const results = useMemo(() => {
    ...
    for (const finding of audit.findings ?? []) {
      ...
      add(
        ...
        [
          ...
          /*
            The write-up, flattened by the server rather than shipped as HTML.
            The engagement payload no longer carries the four bodies — a hundred findings' worth
            of markup so that a text box could search it — and sends `searchText` instead: the
            same prose, plain, capped at a screenful per finding. A match deep in a long proof of
            concept can fall outside that; opening the finding and searching it is the answer to
            that, and a page that loads is the answer to everything else.
          */
          ['write-up', finding.searchText ?? ''],
    ...
    for (const note of audit.notes ?? []) {
      add('note', note._id, note.title || 'Untitled note', [
        ['title', note.title],
        ['text', note.searchText ?? plain(note.content)],
      ]);
    }
