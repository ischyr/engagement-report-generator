...
const BLANK = {
  ...
  scope: '',
  references: [],
  /* An array from the start, so the picker never has to wonder. */
  tags: [],
  customFields: [],
};
...
function FindingEditor({
  ...
  locale = 'en',
  /** The engagement's scope, so the affected assets can be picked rather than retyped. */
  scopeHosts = null,
}) {
  const toast = useToast();
  const { user } = useAuth();
  ...
  /** What other findings are already tagged, so the same idea is not spelled three ways. */
  const findingTags = useResource('/audits/finding-tags', { initial: [] });
  ...
  const insertFromBin = (item) => {
    ...
    /*
     * A recording goes in as the still frame that stands for it, carrying the recording's own URL.
     * The prose, the numbering and the document all deal in the picture; the attribute is what
     * lets the app play the thing it came from.
     */
    const recording = item.kind === 'video' ? item.url : '';
    const src = recording ? (item.poster ?? item.url) : item.url;
    const video = recording ? ` data-video="${recording}"` : '';
    const figure = caption
      ? `<figure class="engy-figure"><img src="${src}"${video} alt="${caption}"><figcaption>${caption}</figcaption></figure>`
      : `<figure class="engy-figure"><img src="${src}"${video} alt=""></figure>`;
      ...
      <Card>
        <CardBody className="grid gap-4 sm:grid-cols-2">
          <CollaborativeInput
            as={Input}
            /*
              Shared, like the prose below it. A title has no document of its own, so this binds a
              `Y.Text` to the box instead — see `CollaborativeInput` for why a remote change is
              applied through `onChange` rather than assigned, and how the caret is put back.
            */
            auditId={auditId}
            scope="finding"
            id={form._id}
            field="title"
            enabled={mayWrite && Boolean(form._id)}
            label="Title"
            required
          ...
          {/*
            ...
            Unlike everything above it, this never reaches the document — which is the point of it.
            The fields on this card are things the client is told; a tag is a note to ourselves, and
            "client disputes" printing in the client's own report would be a bad afternoon.
          */}
          <div className="sm:col-span-2">
            <TagPicker
              label="Tags"
              hint="Yours to filter and search by, and never printed in the report — “needs retest”, “client disputes”, “chained with 4”."
              value={form.tags ?? []}
              editable={mayWrite}
              onChange={(tags) => set({ tags })}
              suggestions={findingTags.data ?? []}
              empty="None. They are for the questions the other fields cannot answer at the end of a test."
            />
          </div>
        </CardBody>
      </Card>
            ...
            <CardBody>
              ...
              */}
              {bodyReady ? (
                <CollaborativeField
                  /*
                    Shared with whoever else has this field open, when the instance can — and the
                    ordinary editor when it cannot. `bodyReady` still gates it: a shared document
                    seeded from a write-up that has not arrived yet would share an empty one.
                  */
                  auditId={auditId}
                  scope="finding"
                  id={form._id}
                  ...
                  /*
                    ...
                    thing anybody wants, so the command is not offered where it would be noise.
                  */
                  scopeHosts={
                    field.key === 'scope' || field.key === 'description' ? scopeHosts : null
                  }
                />
              ) : (
                <div
 ...
 * columns their importers accept, and somebody who picks the wrong one finds out at the far end.
 */
const TRACKER_CHOICES = [
  {
    value: 'jira',
    label: 'Jira',
    hint: 'Import as issues, then map the columns Jira offers. Anything it does not ask about is ignored.',
  },
  {
    value: 'servicenow',
    label: 'ServiceNow',
    hint: 'Load through an import set. The column names are descriptive, for mapping onto your own fields.',
  },
  {
    value: 'devops',
    label: 'Azure DevOps',
    hint: 'Import as work items. The columns match the Bug type in the Agile, Scrum and CMMI processes.',
  },
];
...
export default function FindingsTab({ audit, editable, onReload, onPatch }) {
   ...
   *
   * From the engagement rather than `/users`, exactly as the checklist does it: the server
   * refuses anybody who is not a member, because a finding assigned to somebody who cannot open
   * the engagement is worse than an unassigned one.
   */
  const team = useMemo(() => {
    const seen = new Map();
   ...
   *
   * The ordinary finding update, deliberately — not an endpoint of its own. It is what logs the
  ...
  const findings = useMemo(() => {
    ...
    /*
     * Whatever is open stays in the list even when the filter would drop it. A filter that
   ...
   *
   * From this engagement rather than from the instance: the filter is over the list on screen, and
   * offering a tag that would empty it is a control that lies. The picker in the editor is the
   * opposite case and asks the server, because there the useful suggestion is the one somebody used
   * on a different test last month.
   */
  const tagsInUse = useMemo(() => {
    const counts = new Map();
    for (const finding of audit.findings ?? []) {
      for (const tag of finding.tags ?? []) counts.set(tag, (counts.get(tag) ?? 0) + 1);
    }
  ...
  /**
   * The same findings, in the shape the client's own tracker imports.
   *
   * A remediation team that lives in Jira does not want a tracker of ours; they want tickets in
   * theirs, and the spreadsheet was being retyped into one by hand. Which tracker is asked for
   * here rather than guessed, because the three importers want three different shapes.
   */
  const [trackerOpen, setTrackerOpen] = useState(false);
  const downloadTracker = async (tracker, label) => {
    setExporting(true);
    try {
      const response = await api.raw(`/audits/${audit._id}/findings.csv?tracker=${tracker}`);
      const blob = await response.blob();
      downloadBlob(blob, filenameFromResponse(response, `${audit.name} — ${label}.csv`));
      setTrackerOpen(false);
      toast.success(`${label} CSV downloaded`, 'One row per finding, each carrying its report id.');
    } catch (error) {
      toast.fromError(error, `Could not build the ${label} export`);
    } finally {
      setExporting(false);
    }
  };
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/*
            Only once somebody has actually split the work. On a one-person engagement this
            would be a control that does nothing, and a permanent invitation to wonder why.
          */}
          {anyAssigned ? (
            <button
              type="button"
              ...
              className={cn(
                'rounded-full px-2 py-0.5 text-[0.6875rem] ring-1 transition',
                mineOnly
                  ? 'bg-brand-500/12 text-brand-300 ring-brand-500/30'
                  : 'text-fg-subtle ring-line-soft hover:text-fg'
              )}
            >
              {mineOnly ? 'Showing only mine' : 'Just mine'}
            </button>
          ) : null}

          {/*
            One pill per tag, like the one beside it, rather than a dropdown: there are rarely more
            than a handful and a chip you can see is a chip you remember using. Absent entirely
            until something is tagged, so an engagement nobody has labelled looks as it always did.
          */}
          {tagsInUse.map((entry) => (
            <button
              key={entry.tag}
              type="button"
              onClick={() => setTagFilter((current) => (current === entry.tag ? '' : entry.tag))}
              className={cn(
                'rounded-full px-2 py-0.5 text-[0.6875rem] ring-1 transition',
                tagFilter === entry.tag
                  ? 'bg-brand-500/12 text-brand-300 ring-brand-500/30'
              ...
              title={
                tagFilter === entry.tag
                  ? 'Showing only this tag — click to stop'
                  : `Show only findings tagged ${entry.tag}`
              }
            >
              {entry.tag}
          ...
          {findings.length ? (
            <>
              ...
              <Button
                ...
                title="The same findings as tickets in the client's own tracker"
              >
                Tickets
              </Button>
            </>
      ...
      */}
      ...
      <Modal
        ...
        title="Findings as tickets"
        description="A CSV in the shape one of these three importers wants. Every ticket carries its report id, so it can be traced back to the paragraph it came from."
        size="sm"
        footer={
          <Button variant="ghost" onClick={() => setTrackerOpen(false)}>
            Cancel
          </Button>
        }
      >
        <div className="flex flex-col gap-2">
          {TRACKER_CHOICES.map((choice) => (
            <button
              key={choice.value}
              type="button"
              disabled={exporting}
              onClick={() => downloadTracker(choice.value, choice.label)}
              className="rounded-lg border border-line-soft bg-canvas/40 px-3 py-2.5 text-left transition hover:border-brand-500/40 disabled:opacity-50"
            >
              <span className="block text-xs font-medium text-fg">{choice.label}</span>
