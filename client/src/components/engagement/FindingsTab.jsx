...
function FindingEditor({
  ...
  useUnsavedWork(dirty, finding._id ? 'This finding' : 'The new finding');
    ...
    <div className="flex flex-col gap-5">
      ...
      {previewing ? (
        ...
        RICH_FIELDS.map((field) => (
          <Card key={field.key}>
            <CardHeader title={field.label} description={field.hint} />
            <CardBody>
              <RichTextEditor
...
export default function FindingsTab({ audit, editable, onReload, onPatch }) {
  ...
  const isDraft = (finding) =>
    isHtmlEmpty(finding.description) && (finding._cvss?.baseScore ?? 0) === 0;
                ...
                */}
                ...
                >
                  ...
                  <p className="mt-0.5 truncate text-xs text-fg-muted">
                    ...
                    {finding.description
                      ? `${finding.category || finding.vulnType ? ' — ' : ''}${htmlToSnippet(finding.description, 110)}`
                      : ''}
                  </p>
                </button>
