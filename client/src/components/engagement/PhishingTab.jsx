...
export default function PhishingTab({ audit, editable }) {
  ...
  const remove = async () => {
    try {
      const result = await api.del(`/audits/${audit._id}/phishing/${pendingDelete._id}`);
  ...
  /**
   * The campaign, written up as a finding.
   *
   * Every number here is already on this screen, and the way they used to reach a finding was
   * somebody reading them off it and typing them in — which is where a report that says 38% beside
   * a table saying 31% comes from. The server drafts it from the same summary this tab is drawn
   * from, so the two cannot disagree.
   */
  const draftFinding = async () => {
    setDrafting(true);
    try {
      const { finding } = await api.post(`/audits/${audit._id}/phishing/finding`);
      toast.success('Drafted as a finding', 'The numbers are in it. The remediation is yours.');
      navigate(`/engagements/${audit._id}/findings/${finding._id}`);
    } catch (error) {
      /* Already drafted: the useful answer is that finding, not a second copy of it. */
      if (error.status === 409 && error.details?.findingId) {
        toast.info('Already drafted', 'Opening the finding that is already here.');
        navigate(`/engagements/${audit._id}/findings/${error.details.findingId}`);
        return;
      }
      toast.fromError(error);
    } finally {
  ...
  return (
    <div className="flex flex-col gap-4">
      {summary?.total ? (
        <>
          {/*
            The figures below are the finding, and until now they reached one by being read off
            this screen and retyped. The button is here rather than on the Findings tab because
            this is where somebody is standing when they decide the campaign is worth writing up.
          */}
          {editable ? (
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-card border border-line-soft bg-white/[0.02] px-4 py-3">
              <p className="text-xs text-fg-muted">
                Write this campaign up as a finding — the numbers below, already in it.
              </p>
              <Button
                variant="secondary"
                size="sm"
