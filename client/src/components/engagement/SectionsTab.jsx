...
import { Check, ChevronDown, ChevronUp, ListChecks, Plus, Save, Sparkles, Trash2 } from 'lucide-react';
...
/**
 * Which sections the assistant can draft.
 *
 * Matched on the name rather than a fixed field, because section types are defined per instance
 * and an instance that calls it "Management Summary" or "Synthèse" should get the button too. The
 * prompt writes an executive summary specifically — three to five paragraphs for a reader who will
 * read nothing else — so offering it on Methodology would produce something confidently wrong.
 */
const SUMMARY_SECTION = /exec|summary|synth|sommaire|resumen|zusammenfassung/i;
...
function SectionCard({ section, auditId, editable, onSaved, onDelete, position, onMove }) {
  ...
  useUnsavedWork(dirty, `The "${section.name}" section`, () => save());
  ...
  return (
    <Card>
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            */}
            ...
            {editable && SUMMARY_SECTION.test(`${section.field} ${section.name}`) ? (
              /*
               * Drawn only when the instance has an assistant — see `useAssistant`. The draft
               * lands in the editor unsaved, so it is subject to the same save, the same conflict
               * check and the same "you have unsaved work" guard as anything typed by hand.
               */
              <AssistantAction
                job="summary"
                label="Draft it"
                icon={Sparkles}
                title="A first draft from the findings, their severities and the scope"
                dialogTitle="A first draft of the executive summary"
                dialogDescription="Written from this engagement's findings, their severities and its scope. It knows nothing else, and it has invented nothing."
                request={() => api.post('/assistant/summary', { auditId })}
                preview={(result) => (
                  <div
                ...
                applyLabel={isHtmlEmpty(text) ? 'Put it in the editor' : 'Replace what is there'}
                onApply={(result) => setText(result.html)}
              />
            ) : null}
            {editable ? (
              <>
      ...
      />
      <CardBody>
        {/* Shared with whoever else has this section open, where the instance can. */}
        <CollaborativeField
          auditId={auditId}
          scope="section"
          id={section._id}
...
export default function SectionsTab({ audit, editable, onReload }) {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      const result = await api.del(`/audits/${audit._id}/sections/${pendingDelete._id}`);
