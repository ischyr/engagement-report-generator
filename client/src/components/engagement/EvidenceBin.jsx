...
import { Highlighter, ImagePlus, Play, Trash2, Upload } from 'lucide-react';
...
export default function EvidenceBin({ auditId, onInsert, compact = false }) {
  ...
  const upload = useCallback(
    async (files) => {
      ...
      const recordings = all.filter((file) => isRecording(file));
      if (!images.length && !recordings.length) {
        toast.error('Those are not pictures or recordings');
        return;
      }
      setUploading(true);
      try {
        ...
        /*
         * A recording is two objects: the frame that stands for it, then the recording itself
         * pointing at that frame. Both are tagged with the engagement, and the bin hides the
         * frame — see `auditEvidenceBin` — so it reads as one piece of evidence rather than two.
         */
        const unreadable = [];
        for (const file of recordings) {
          const still = await posterFromRecording(file);
          if (!still) {
            unreadable.push(file.name || 'a recording');
            continue;
          }
          const posterBody = new FormData();
          posterBody.append('file', still.file, still.file.name);
          posterBody.append('audit', auditId);
          ...
          videoBody.append('audit', auditId);
          videoBody.append('poster', storedPoster.id);
          await api.post('/media', videoBody);
        }

        const stored = images.length + (recordings.length - unreadable.length);
        toast.success(
          stored === 1 ? 'Captured' : `${stored} captured`,
          changed
        ...
        if (unreadable.length) {
          toast.error(
            ...
            `${unreadable.join(', ')} — export as MP4 or WebM, which every browser can play.`
          );
        }
        reload({ quiet: true });
      } catch (err) {
  ...
  return (
    <Card>
      <CardHeader
        title="Evidence bin"
        icon={ImagePlus}
        description="Screenshots and recordings captured for this engagement that no finding uses yet. Paste or drop them here while you test; insert them when you write up."
        actions={
          canWrite ? (
            <>
              <input
              ...
              {items.map((item) => (
                ...
                >
                  <span className="relative block">
                    ...
                    />
                    {/* So a recording is recognisable as one before anybody inserts it. */}
                    {item.kind === 'video' ? (
                      <span className="absolute bottom-1 right-1 flex items-center gap-1 rounded bg-black/70 px-1.5 py-0.5 text-[0.625rem] text-white">
                        <Play size={9} />
                        recording
                      </span>
                    ) : null}
                  ...
                  <div className="mt-auto flex items-center gap-1">
                    ...
                    {canWrite ? (
                      <>
                        {/*
                          Marking up a recording is not offered. The annotator flattens boxes into
                          the image it was given, so on a recording it would produce a redacted
                          still sitting in front of an unredacted video — which is worse than no
                          redaction, because it looks like one.
                        */}
