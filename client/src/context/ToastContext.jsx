  ...
  const toast = useMemo(
    () => ({
      ...
      fromError: (error, fallback = 'Something went wrong') => {
        ...
        /*
         * The request id, but only when the server broke rather than when it disagreed.
         *
         * A 409 or a 422 is an answer: "that name is taken" is the whole message and an id after it
         * is clutter that makes a normal refusal look like a malfunction. A 500 is not an answer,
         * and the id is the only thing that connects what somebody just saw to the stack trace in
         * the log — so it goes on those, where an administrator can be given something to grep for.
         */
        const broke = error instanceof ApiError && (error.status >= 500 || error.status === 0);
        const reference = broke && error.requestId ? `Reference ${error.requestId}` : '';

        return push({
          variant: 'error',
          title: error?.message || fallback,
          description: [description, reference].filter(Boolean).join(' · ') || undefined,
        });
      },
      dismiss,
    }),
