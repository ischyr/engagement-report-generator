...
import { closeAllRooms } from '../lib/collab-rooms.js';
...
export function AuthProvider({ children }) {
  ...
  const logout = useCallback(async () => {
    try {
      ...
      /*
       * Shared documents are closed here rather than left to the components that were using them.
       * Those are kept open for a few seconds after a field unmounts, on purpose — a remount must
       * not pay for a new connection — and signing out is the one case where that is exactly wrong:
       * the credential the socket was opened with has just been revoked, and a caret with somebody's
       * name on it would sit in a colleague's editor until the grace period ran out.
       */
      closeAllRooms();
      clearSession();
      await loadStatus();
  ...
  const branding = useMemo(
    () => ({
      appName: status.branding?.appName || 'Engy • Report Generation',
...
export function useBranding() {
  return useContext(AuthContext)?.branding ?? {
    appName: 'Engy • Report Generation',
