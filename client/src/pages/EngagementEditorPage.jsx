...
import ProtectedReportDialog from '../components/engagement/ProtectedReportDialog.jsx';
 ...
 *
 * Anything the client has marked, anything being retested, anything already fixed. A finding that
 * is simply `open` does not count: every engagement is full of those while it is being written,
 * and a Retest tab on an engagement nobody has delivered yet is a tab in the way.
 */
const inRemediation = (audit) =>
  (audit?.findings ?? []).some(
    (finding) => finding.clientClaim?.status || finding.remediationStatus !== 'open'
  );

/**
 * The tabs every engagement has.
 *
 * `only` marks a tab that belongs to particular kinds of engagement — one kind or a list of them.
 * `when` marks one that belongs to a *state* rather than a kind: it appears when there is something
 * for it to show. Thirteen is already a lot, and a network test showing a mailing list is how a tab
...
const TABS = [
  ...
  /*
   * Beside Findings, because it is the same findings later: the report has gone, the client has
   * marked things through their link, and this is where somebody checks whether they are right.
   */
  { value: 'retest', label: 'Retest', icon: RefreshCw, when: inRemediation },
   ...
   *
   * Beside Credentials rather than beside Notes, because the two are read together: what was
   * done, and which borrowed account it was done with. Present on every engagement, because any
   * test can turn out to have changed something and the tab appearing only on certain kinds would
   * be a place people did not think to look.
   */
  { value: 'intrusive', label: 'Intrusive', icon: ShieldOff },
  /*
  ...
  /*
   * Red teams phish too — it is the ordinary way in, and an operation that starts with a pretext
   * email has a mailing list, timings and a click rate exactly like a campaign does. The tab was
   * gated to the phishing kind alone, so a red team had nowhere to record any of it.
   */
  {
    value: 'phishing',
    label: 'Sending list',
    icon: MailCheck,
    only: ['phishing', 'redteam'],
  },
...
export default function EngagementEditorPage() {
  ...
  /**
   * Whether the report is being looked at.
   *
   * Only offered where a PDF can actually be made: `settings.pdf.enabled` is the one thing a
   * non-admin is told about the converter, and a Render button that always answered "no converter
   * is set up" would be a button that teaches people not to press it.
   */
  const [previewing, setPreviewing] = useState(false);
  /*
   * Whether a PDF can be made here at all. From the one capabilities request the app already makes
   * rather than a second one of its own — which it used to be, before shared editing needed the
   * same answer in a dozen places.
   */
  const { pdf: pdfAvailable } = useCapabilities();
  ...
  const tabs = useMemo(
    () =>
      TABS.filter((entry) => {
        /* A tab that belongs to a state shows when the state is true, whatever its kind. */
        if (entry.when) return entry.when(audit);
        if (!entry.only) return true;
        /* One kind or several — the sending list belongs to two. */
        if ([entry.only].flat().includes(audit?.kind)) return true;
        /*
         * And it stays once it holds something, whatever the kind says now. A mailing list that
         * vanished because somebody changed a dropdown would look exactly like data loss.
         */
        return entry.value === 'phishing' && audit?.phishingCount > 0;
      }),
    [audit?.kind, audit?.phishingCount, audit?.findings]
  );
  ...
  const COUNTS = {
    ...
    /* A count for the same reason the others have one: how big the list is, without opening it. */
    phishing: audit.phishingCount ?? 0,
    /*
     * What is waiting for somebody, not how many findings are in the cycle.
     *
     * The other badges are totals because a total is the useful number there. Here it is not: forty
     * verified-fixed findings are finished work, and a badge counting them would say "40" on an
     * engagement with nothing left to do. What matters is the claims nobody has checked.
     */
    retest: (audit.findings ?? []).filter(
      (finding) => finding.clientClaim?.status || finding.remediationStatus === 'retesting'
    ).length,
  };
    ...
    <div className="flex flex-col gap-6">
        ...
        actions={
          <>
            ...
            {audit.template?.kind === 'html' ? (
              ...
              <>
                {/* Beside Generate, because it is the same document — one to look at, one to send. */}
                {pdfAvailable ? (
                  <ReportPreviewButton
                ...
                {/*
                  ...
                  machine; the ones that go to a client by mail are the ones worth wrapping, and the
                  person pressing the button is the only one who knows which this is.
                */}
                <Button
                  variant="secondary"
                  icon={ShieldCheck}
                  ...
                  title={
                    audit.template
                      ? 'Generate it inside a password-protected archive'
                      : 'Assign a template on the Overview tab first'
                  }
                >
                  Protected
      ...
      */}
      {/*
        ...
        */}
      <ProtectedReportDialog
        auditId={id}
        auditName={audit?.name}
        open={protecting}
        onClose={() => setProtecting(false)}
        onDownloaded={setLastGenerated}
      />
      ...
      */}
      ...
      {tab === 'intrusive' ? <IntrusiveTab audit={audit} editable={editable} /> : null}
      {tab === 'notes' ? (
        <NotesTab
          audit={audit}
          editable={editable}
      ...
      {tab === 'retest' ? (
        <RetestTab audit={audit} editable={editable} onReload={reload} />
      ) : null}
      ...
      */}
      ...
      {/*
        Last, and outside the tabs: it is a view of the whole engagement rather than of any one
        part of it, and it renders nothing at all until somebody asks for it.
      */}
      <ReportPreview
        audit={audit}
        open={previewing}
