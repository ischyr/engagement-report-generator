...
export default function SharedFindingsPage() {
  ...
  const visible = useMemo(
    /*
     * `Array.isArray` because the same route serves two shapes: a progress link's payload has a
     * `findings` object of counts rather than a list, and this runs before the branch below can
     * hand that payload off.
     */
    () =>
      (Array.isArray(data?.findings) ? data.findings : []).filter(
        (finding) => showFixed || finding.status !== 'fixed'
      ),
    [data, showFixed]
  );
  ...
  /*
   * The other kind of link. Two payloads, two pages, chosen on what the server sent rather than on
          ...
          {visible.map((finding) => (
            <Card key={finding._id} className={cn(finding.status === 'fixed' && 'opacity-60')}>
              ...
              <CardBody className="flex flex-col gap-4">
                ...
                {/*
                  And the box for what they did about it, directly under the instructions.

                  Only on a link that can be written to: a read-only link showing a field that
                  cannot save would be worse than showing nothing. `canAttach` is the link's
                  permission, passed per finding because the component asks per finding.
                */}
                {data.allowUpdates ? (
                  <RemediationClaim
                    token={token}
