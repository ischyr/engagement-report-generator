...
export default function InboxPage() {
  ...
  const tabs = useMemo(
    () => [
      ...
      /*
       * Before the checks, and before anything you are only waiting on: a finding that is yours
       * to write is the largest single piece of work this page ever lists.
       */
      { value: 'findings', label: 'Yours to write', count: counts.findings || undefined },
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {nothingListed ? (
        <Card>
          <EmptyState
            icon={CheckCheck}
            title="You are clear"
            description="Findings that are yours to write, reviews assigned to you, mentions, unresolved comments on your findings, checks somebody gave you, and checks you asked for all show up here."
          />
        </Card>
      ) : null}
      ...
      {/*
        Above the checks and the comments: writing a finding is hours, and a comment is minutes.
        A queue that puts the small things first is a queue that gets read and not worked.
      */}
      {show('findings') && data.findings?.length ? (
        <Card>
          <CardHeader
            icon={PenLine}
            title="Findings that are yours to write"
            description="Somebody split the engagement and these are your half. Worst first."
          />
          <CardBody className="flex flex-col gap-1.5">
            {data.findings.map((finding) => (
              <Link
                key={finding.findingId}
                to={`/engagements/${finding.auditId}/findings/${finding.findingId}`}
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-0.5 block truncate text-[0.625rem] text-fg-subtle">
                    {[finding.reference, finding.auditName].filter(Boolean).join(' · ')}
                  </span>
                </span>
              </Link>
            ))}
