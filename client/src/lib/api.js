...
import { clearResourceCache } from './resource-cache.js';
...
export class ApiError extends Error {
   ...
   * @param {string} [requestId] what the server called this request, for the log
   */
  constructor(status, message, details, kind = 'http', requestId = '') {
    super(message);
    this.name = 'ApiError';
    this.status = status;
    ...
    this.kind = kind;
    /**
     * The server's own id for the request that failed.
     *
     * Carried this far so a person can quote it. Every log line the server wrote while serving that
     * request has the same string in it, so "it broke and it said a1b2c3d4" is a complete bug
     * report — which is a different thing from "it broke at about three".
     */
...
async function parseError(response) {
  ...
  /*
   * From the body when the API answered, and from the header otherwise. The header is set on every
   * response, so it survives the cases where there is no JSON to read — which are exactly the cases
   * somebody needs help with.
   */
  const requestId = payload?.requestId || response.headers.get('X-Request-Id') || '';

  if (payload?.error) {
    return new ApiError(response.status, payload.error, payload.details, 'http', requestId);
  }
  ...
  if (response.status >= 500) {
    return new ApiError(response.status, UNREACHABLE_MESSAGE, undefined, 'unavailable', requestId);
  }
  if (response.status === 413) {
    return new ApiError(413, 'That upload is too large', undefined, 'http', requestId);
  }
  return new ApiError(response.status, `Request failed (${response.status})`, undefined, 'http', requestId);
}
...
export async function request(path, options = {}) {
  ...
  /*
   * Anything that changed something invalidates every remembered read.
   *
   * All of it rather than a guess at which URLs this write touched: saving a finding changes the
   * engagement, its pulse, its history, the review readiness, the tags and the search index, and a
   * list of those maintained by hand would be wrong within a month. What it costs is one request on
   * the next mount. See the note in `resource-cache.js`.
   */
  if (method !== 'GET') clearResourceCache();
