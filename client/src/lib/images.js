...
/**
 * How many pictures one gesture may bring in.
 *
 * Not a limit anybody writing a report will meet — fifty screenshots off one engagement is an
 * ordinary day and this is twice that. It is here for the accident: dropping a folder rather than
 * a selection, which without a ceiling would upload two thousand files one after another with no
 * way to stop it.
 */
export const MAX_IMAGES_AT_ONCE = 100;

/**
 * Which of the dropped files are pictures, in the order they were given.
 *
 * Pure, and separated from the uploading for the usual reason: this is the part with consequences.
 * The editor took `files[0]` and `files.find(...)`, so dragging twenty screenshots in inserted one
 * and discarded nineteen without a word — the worst kind of bug, because it looks like it worked.
 *
 * Order is the other thing that matters and is easy to lose. Figures are numbered by the order they
 * appear in the finished document, so the order these come back in is the order they will be
 * numbered in, which is the order somebody selected them. Nothing here sorts.
 *
 * @param {Iterable<File>} list a `FileList`, a `DataTransfer.files`, or an array
 * @returns {{images: File[], ignored: number, dropped: number}} `ignored` is how many were not
 *   pictures at all; `dropped` is how many pictures fell off the end of the ceiling
 */
export function pickImages(list) {
  const all = [...(list ?? [])];
  const images = all.filter((file) => String(file?.type ?? '').startsWith('image/'));
  return {
    images: images.slice(0, MAX_IMAGES_AT_ONCE),
    ignored: all.length - images.length,
    dropped: Math.max(0, images.length - MAX_IMAGES_AT_ONCE),
  };
}

/**
 * The same gesture, when a recording is allowed to come in with the screenshots.
 *
 * Its own function rather than a flag on `pickImages`, which is what the paste handler and the
 * annotator want and should keep wanting: those two really do only take pictures.
 *
 * The interleaved list is the point. Figures are numbered by the order they appear in the
 * document, which is the order they are inserted, which is the order somebody selected them — so a
 * drop of two screenshots, a recording and a third screenshot has to stay in exactly that order.
 * Splitting them into an images list and a videos list and uploading one after the other would
 * silently renumber somebody's evidence.
 *
 * @param {Iterable<File>} list
 * @returns {{items: File[], images: number, videos: number, ignored: number, dropped: number}}
 */
export function pickEvidence(list) {
  const all = [...(list ?? [])];
  const taken = all.filter((file) => {
    const type = String(file?.type ?? '').toLowerCase();
    return type.startsWith('image/') || RECORDING_TYPES.has(type);
  });
  const items = taken.slice(0, MAX_IMAGES_AT_ONCE);
  return {
    items,
    images: items.filter((file) => String(file?.type ?? '').startsWith('image/')).length,
    videos: items.filter((file) => RECORDING_TYPES.has(String(file?.type ?? '').toLowerCase())).length,
    ignored: all.length - taken.length,
    dropped: Math.max(0, taken.length - MAX_IMAGES_AT_ONCE),
  };
}

/**
 * Kept here rather than imported from `recordings.js`, which is browser-only: this file's
 * arithmetic is tested without a DOM, and one duplicated pair of strings is a smaller price than
 * dragging a `<video>` element into that test.
 */
const RECORDING_TYPES = new Set(['video/mp4', 'video/webm']);

/**
 * What to say about the files that did not make it, or nothing at all.
 *
 * One sentence rather than one toast per file: twenty toasts to say the same thing is not a
 * warning, it is a wall, and the useful information is the count.
 */
export function ignoredNote({ ignored = 0, dropped = 0, what = 'a picture' } = {}) {
  const parts = [];
  if (ignored) parts.push(`${ignored} ${ignored === 1 ? 'file was' : 'files were'} not ${what}`);
  if (dropped) {
    parts.push(
      `${dropped} more than the ${MAX_IMAGES_AT_ONCE} this can take at once — drop the rest after`
    );
  }
