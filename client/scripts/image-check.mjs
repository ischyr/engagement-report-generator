...
const { planResize, chooseEncoding, pickImages, ignoredNote, MAX_IMAGES_AT_ONCE, IMAGE_LIMITS } =
  await vite.ssrLoadModule('/src/lib/images.js');
...
/* -------------------------------------------------------------------------- */
console.log('\nWhat comes in when a folder is dropped:');

/*
 * The bug this covers shipped and looked like it worked, which is the worst kind. The editor took
 * `files[0]` on the picker and `files.find(...)` on the drop, so dragging in twenty screenshots
 * inserted one and discarded nineteen without a word — found out much later, by a report with one
 * picture in it.
 */
const file = (name, type) => ({ name, type });
const dropped = [
  file('01-login.png', 'image/png'),
  file('notes.txt', 'text/plain'),
  file('02-token.png', 'image/png'),
  file('03-shell.jpg', 'image/jpeg'),
];

const taken = pickImages(dropped);
check('every picture is taken, not the first one', taken.images.length === 3, String(taken.images.length));
check(
  'in the order they were given, because that is the order they will be numbered in',
  taken.images.map((f) => f.name).join() === '01-login.png,02-token.png,03-shell.jpg',
  taken.images.map((f) => f.name).join()
);
check('and what was not a picture is counted rather than silently dropped', taken.ignored === 1);
check('nothing is reported as over the ceiling when it is not', taken.dropped === 0);

check('an empty drop takes nothing', pickImages([]).images.length === 0);
check('so does a drop with no pictures in it', pickImages([file('a.txt', 'text/plain')]).images.length === 0);
check('and a missing list is not an error', pickImages(undefined).images.length === 0);
check(
  'a file with no type at all is not assumed to be a picture',
  pickImages([file('mystery', '')]).images.length === 0
);

/* The ceiling is for the accident — a folder rather than a selection — not for a real day's work. */
const many = Array.from({ length: MAX_IMAGES_AT_ONCE + 5 }, (_, i) => file(`shot-${i}.png`, 'image/png'));
const capped = pickImages(many);
check('fifty screenshots is an ordinary day and goes through', pickImages(many.slice(0, 50)).images.length === 50);
check(`the ceiling holds at ${MAX_IMAGES_AT_ONCE}`, capped.images.length === MAX_IMAGES_AT_ONCE);
check('and says how many did not fit', capped.dropped === 5, String(capped.dropped));

check('nothing to say when nothing was left out', ignoredNote({ ignored: 0, dropped: 0 }) === '');
check(
  'one sentence rather than one message per file',
  ignoredNote({ ignored: 3, dropped: 0 }) === '3 files were not a picture',
  ignoredNote({ ignored: 3, dropped: 0 })
);
check(
  'and it counts one file correctly too',
  ignoredNote({ ignored: 1, dropped: 0 }) === '1 file was not a picture'
);
check(
  'both reasons read as one sentence',
  ignoredNote({ ignored: 2, dropped: 4 }).includes(' and ') &&
    ignoredNote({ ignored: 2, dropped: 4 }).includes('drop the rest after'),
  ignoredNote({ ignored: 2, dropped: 4 })
);
