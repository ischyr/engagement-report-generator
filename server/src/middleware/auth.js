...
/* Only the prefix, so this module does not depend on the API token machinery to refuse one. */
import { TOKEN_PREFIX as API_TOKEN_PREFIX } from '../models/api-token.model.js';
...
/**
 * The credential a WebSocket handshake can carry.
 *
 * Its own cookie rather than the media one, because a cookie is only sent to the path it was
 * scoped to — `/api/media` — which is exactly why every collaboration upgrade was refused while
 * evidence loaded perfectly. The fix is not to widen that scope: the media cookie is a seven-day
 * credential signed with the refresh secret, and widening it to `/api` would attach it to every
 * request the app makes, most of which have no business carrying it.
 *
 * So there are two narrow cookies instead of one broad one, and they are not interchangeable: the
 * scope claim differs, so a stolen media cookie cannot open a document and a stolen collaboration
 * cookie cannot read evidence.
 */
export const COLLAB_COOKIE = 'engy_collab';

export function signCollabToken(user, sid) {
  return jwt.sign(
    { sub: user._id.toString(), version: user.tokenVersion, scope: 'collab', ...(sid ? { sid } : {}) },
    env.jwt.refreshSecret,
    { expiresIn: env.jwt.refreshTtl }
  );
}

export function collabCookieOptions() {
  return {
    httpOnly: true,
    sameSite: 'lax',
    secure: env.isProd,
    /* Only where the sockets are. Nothing else on the API ever sees it. */
    path: '/api/collab',
    maxAge: 7 * 24 * 60 * 60 * 1000,
  };
}
...
export const requireAuth = asyncHandler(async (req, _res, next) => {
  ...
  /*
   * An API token, sent here by mistake, gets told where it belongs.
   *
   * The other half of the pair in `api-auth.js`, which tells a browser session that it cannot use
   * `/api/v1`. Both directions are somebody's first afternoon with a script, and "Invalid access
   * token" sends them looking for a fault in a string that is perfectly valid — just not here.
   * Matched on the prefix alone: no lookup, so an unknown string learns nothing from this.
   */
  if (token.startsWith(API_TOKEN_PREFIX)) {
    throw unauthorized(
      'That is an API token. It only works on /api/v1 — see /docs/api. These routes need a signed-in session.'
    );
  }
