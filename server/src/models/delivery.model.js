...
const recipientSchema = new mongoose.Schema(
  {
    ...
    /**
     * Which numbered copy this person was given, when copies were made per recipient.
     *
     * The whole point of making them. A report that turns up somewhere it should not is matched
     * against `fileHash` here and the answer is a name, rather than "one of the four people we
     * sent it to". Zero and empty on an ordinary delivery, where everybody got the same bytes and
     * the delivery's own hash is the only one there is.
     */
    copyNumber: { type: Number, default: 0, min: 0 },
    /** The SHA-256 of that recipient's own bytes. Lower-case hex, like the delivery's. */
    fileHash: {
      type: String,
      default: '',
      trim: true,
...
const deliverySchema = new mongoose.Schema(
  {
    ...
    /**
     * How many distinct copies this delivery is, when each recipient got their own.
     *
     * One on an ordinary delivery. Above one means every recipient above has their own
     * `fileHash`, the delivery's own `fileHash` is the first of them, and the register can trace
     * a loose file to a person rather than to a mailing list.
     */
...
/*
 * And the per-recipient hashes, for the same reason.
 *
 * "Whose copy is this?" is asked with a file in hand and no idea which engagement it belongs to,
 * so the lookup has to be able to start from the hash alone across the whole register.
 */
deliverySchema.index({ 'recipients.fileHash': 1 });
