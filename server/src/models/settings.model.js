...
const settingsSchema = new mongoose.Schema(
  {
     ...
     */
    branding: {
      appName: { type: String, default: 'Engy • Report Generation', trim: true, maxlength: 60 },
     ...
     * party, and "Engy • Report Generation" is not one — a contract names a registered entity at
     * an address.
     ...
     *
     * Off by default, and the whole feature is behind it: with this off the app behaves exactly as
     * it did before there was a socket — one writer, the soft lock, the conflict merge, saving as
     * it always worked. Nothing about an engagement changes, because the shared copy was never
     * where the text lived.
     *
     * Worth a switch rather than being always on for two reasons. A reverse proxy that does not
     * pass WebSocket upgrades produces a connection attempt per field that can only fail, and an
     * administrator who does not want concurrent editing at all should not have to configure a
     * proxy to prevent it.
     */
    /**
     * Telling a chat channel what happened, as it happens.
     *
     * Off until an administrator fills it in, like the mail and the assistant — and like those, the
     * credential lives in the vault rather than in this document in the clear. For Teams and for
     * Discord the URL *is* the credential: anybody holding it can post to that channel as this app.
     */
    webhooks: {
      enabled: { type: Boolean, default: false },
      /** One of the presets in `webhooks/providers.js`. */
      provider: { type: String, default: 'teams', trim: true, maxlength: 20 },
      /** AES-256-GCM under VAULT_KEY. Never leaves the server, not even to an admin. */
      url: {
        iv: { type: String, default: '' },
        tag: { type: String, default: '' },
        data: { type: String, default: '' },
      ...
      /**
       * The host, in the clear, so the settings page can say which service it points at.
       *
       * Kept beside the ciphertext rather than derived from it, because deriving it would mean
       * decrypting on every read of the settings — which every page does — to render one line of
       * text. A host is not a secret; the path and its token are, and those stay in the vault.
       */
      urlHost: { type: String, default: '', trim: true, maxlength: 200 },
      /** For the plain-JSON provider only: what the HMAC header is computed with. */
      signingSecret: {
        iv: { type: String, default: '' },
        tag: { type: String, default: '' },
        data: { type: String, default: '' },
      ...
      /**
       * Which groups are worth a message. Named in `webhooks/index.js`.
       *
       * The four useful ones default to on, so switching the feature on does something rather than
       * nothing. Links are off: a record of every share link is useful and is not conversation.
       */
      events: {
    ...
    /**
     * When a backup was last taken, so the Settings page can say how long ago.
     *
     * Written by the route that builds one, not by the command — `npm run backup` in cron is the
     * one that should be running regularly, and a card that only counted the ones taken by hand
     * would be reassuring about the wrong thing. It says who and how big, which between them are
     * enough to tell "somebody clicked it before upgrading" from "the nightly job is working".
     */
    /**
     * Signing the deliverable.
     *
     * Off until somebody generates a key, like the mail and the assistant, and for the same
     * reason: a capability nobody has configured should do nothing rather than something
     * half-configured. The private half is AES-256-GCM under VAULT_KEY and never leaves the
     * server, not even to an administrator; the public half is deliberately in the clear,
     * because handing it out is the entire point of having it.
     *
     * See `signing.service.js` for why Ed25519 and a detached file rather than a certificate.
     */
    signing: {
      enabled: { type: Boolean, default: false },
      /** 'ed25519'. Recorded so a future second algorithm cannot be mistaken for this one. */
      algorithm: { type: String, default: '', trim: true, maxlength: 20 },
      /** SPKI PEM. Not a secret: it is published so a client can check what we sent them. */
      publicKey: { type: String, default: '', maxlength: 2000 },
      /** PKCS#8 PEM, in the vault. */
      privateKey: {
        iv: { type: String, default: '' },
        tag: { type: String, default: '' },
        data: { type: String, default: '' },
      },
      /** `SHA256:...`, so the settings page and the note beside a report agree on one string. */
    ...
    backup: {
      ...
      byName: { type: String, default: '', maxlength: 200 },
      counts: {
     ...
     *
     * Off by default and not a dependency: nothing here is installed or bundled, and an instance
     * that leaves it alone behaves exactly as it always has — Generate downloads a .docx. See
     * `pdf.service.js` for why this converts the generated bytes rather than laying a PDF out
     * itself.
     */
    pdf: {
      /** 'none', 'libreoffice' or 'word'. The presets live in `pdf.service.js`. */
      converter: { type: String, enum: ['none', 'libreoffice', 'word'], default: 'none' },
      /**
       * The program to run. Empty means the preset's own default — `soffice`, or `powershell` for
       * Word — which is right on any machine where it is on the PATH.
       */
      command: { type: String, default: '', trim: true, maxlength: 400 },
      /** A hundred-page report with forty screenshots is not a fast conversion. */
      timeoutSeconds: { type: Number, default: 120, min: 10, max: 600 },
    },
