...
/**
 * What was done: the verb, not the target.
 *
 * A closed list because it drives the cleanup view and the report's own grouping, and free text
 * would produce "deleted", "delete", "removed" and "rm" in one engagement. `other` is deliberate
 * rather than a gap: an operator with something that does not fit should not be choosing the
 * nearest wrong word.
 */
export const INTRUSION_ACTIONS = [
  'created',
  'modified',
  'deleted',
  'uploaded',
...
export const INTRUSION_ACTION_LABELS = {
  created: 'Created',
  modified: 'Modified',
  deleted: 'Deleted',
  uploaded: 'Uploaded',
  executed: 'Executed',
  disabled: 'Disabled or bypassed',
  other: 'Something else',
};

/**
 * One thing the test changed on somebody else's system.
 *
 * The three fields that matter are `before`, `after` and `revertedAt`. Everything else is context
 * for a reader; those three are what make the change reversible, and what make an un-reversed change
 * findable at the end of a job rather than at the start of an awkward phone call.
 */
const intrusionSchema = new mongoose.Schema(
  {
    /** One line, as it would be read out on a call: "Disabled MFA on the test user". */
    title: { type: String, default: '', trim: true, maxlength: 200 },
    action: { type: String, enum: INTRUSION_ACTIONS, default: 'modified' },

    /**
     * What was touched, exactly enough to find it again.
     *
     * An endpoint with its method, an object id, a table and a row, a path on disk. Free text
     * because the systems this is done to are all different, and a schema that insisted on a URL
     * would be wrong the first time somebody edited a database directly.
     */
    target: { type: String, default: '', trim: true, maxlength: 500 },
    /** Which host or service it was on. Free text, matched against the scope for a warning. */
    host: { type: String, default: '', trim: true, maxlength: 200 },

    /**
     * The value as it was, and the value as it was left.
     *
     * Plain text on purpose, and the reason is the only reason this feature exists: `before` is
     * what somebody types back in. A JSON body, a field value, a permission bit, "the file did not
     * exist". Rich text would look better and would be worse, because what is wanted here is
     * something copy-and-pasteable at three in the morning.
     */
    before: { type: String, default: '', maxlength: 8000 },
    after: { type: String, default: '', maxlength: 8000 },

    /**
     * How to put it back, when `before` is not enough on its own.
     *
     * "Re-enable MFA in the admin panel, the toggle is under Security" is a different thing from
     * the old value, and it is what the person doing the cleanup actually needs.
     */
    revertPlan: { type: String, default: '', maxlength: 2000 },

    /**
     * The write-up, with screenshots pasted into it.
     *
     * Editor HTML like a finding, because "how the endpoint behaved when I changed this" is the
     * interesting part and it is usually a picture. Scanned by `mediaIdsInAudit`, so an image
     * pasted here is a real reference and is neither lost from the report nor collected as an
     * orphan.
     */
    notes: { type: String, default: '' },

    /** When it was done. Free text like an enumeration step's `ranAt`: it is often approximate. */
    at: { type: String, default: '', trim: true, maxlength: 120 },

    /**
     * Who on the client's side agreed to it, if anybody was asked.
     *
     * Not every intrusive action needs sign-off and pretending otherwise would make this field a
     * lie people fill in. But the ones that do — deleting production data, disabling a control —
     * need the name of the person who said yes, recorded before rather than remembered after.
     */
    authorisedBy: { type: String, default: '', trim: true, maxlength: 160 },

    /** Whether it can be undone at all. Some things cannot, and saying so is the honest answer. */
    reversible: { type: Boolean, default: true },

    revertedAt: { type: Date, default: null },
    revertedBy: { type: mongoose.Schema.Types.ObjectId, ref: 'User', default: null },
    revertNote: { type: String, default: '', maxlength: 1000 },

    /**
     * Whether it prints.
     *
     * On by default, because the client is owed this list. Off for the ones that are ours: a file
     * written to our own jump box, a test account on our own infrastructure.
     */
    print: { type: Boolean, default: true },

    /** The findings this produced, the same way an enumeration step names its own. */
    ledTo: [{ type: mongoose.Schema.Types.ObjectId }],

    /**
     * Which borrowed account it was done with.
     *
     * The other half of the credential trail: "you deleted that with which login?" is the question
     * a client asks about exactly this list.
     */
    credentialsUsed: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Credential' }],
...
const findingSchema = new mongoose.Schema(
  {
     ...
     *
     * `createdBy` says who wrote it first and `updatedBy` who touched it last; neither answers
     * "whose is it now", which is the question a two-person engagement asks every morning. So
     * splitting forty findings between two testers happened in chat, and the same one got written
     * twice while another got written by nobody — exactly the problem `assignedTo` on a test check
     * already solves for a checklist, and the same field for the same reason.
     *
     * Not cleared when the finding is finished, because there is no such state: a finding is not
     * "done", it is delivered with the report. What takes it out of somebody's queue is the
     ...
     */
    /**
     * Which borrowed account this was found with.
     *
     * The third place the credential trail is written, beside the enumeration step and the
     * intrusion. All three are the work; `credentialUsage` reads them and answers the question a
     * client actually asks, which is per account rather than per finding.
     */
    credentialsUsed: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Credential' }],
    ...
    clientClaim: {
      ...
      /**
       * What they say they did about it.
       *
       * The point of the whole round trip. A claim on its own moves a row from one column to
       * another; a sentence saying "we moved the endpoint behind the session check and deployed on
       * Tuesday" is what makes the retest a five-minute job instead of an email thread.
       *
       * **Plain text, and it stays plain text.** Everything else in this document that a person
       * types is editor HTML and goes through a sanitiser on the way in; this arrives over a
       * credential that can be forwarded in an email, so it is not offered the chance. Stored as
       * typed, rendered as text, never interpolated into a document as markup.
       */
      note: { type: String, default: '', maxlength: 2000 },

      /**
       * Screenshots they attached, if the link allowed any.
       *
       * Ids in the media bucket, tagged with this engagement and marked as the client's own, so
       * they appear in the evidence bin as something somebody else supplied rather than as
       * something the team captured. Nothing puts them in the report automatically: a tester
       * chooses to insert one, exactly as with any other capture, because a document that quoted
       * the client's own screenshot as our evidence would be the wrong kind of surprise.
       */
    ...
    /**
     * Free labels on one finding — "needs retest", "client disputes", "chained with 4".
     *
     * The engagement has had these for a while, and the note there explains why: the filters this
     * app ships are by client, state and engagement type, so the cross-cutting question has no
     * answer. One level down the same thing is true and more often — the questions an operator asks
     * at the end of a test are "which of these am I still waiting on", "which did the client push
     * back on", "which ones are really the same bug" — and none of those is a field.
     *
     * Deliberately **not** in the report. `severityOverrideReason` prints and this does not, which
     * is the distinction: a reason is something the client is owed, a tag is a note to ourselves,
     * and "client disputes" reaching the client's own document would be a bad afternoon. A weakness
     * class that *should* print wants its own field rather than a convention on this one.
     *
     * Lower-cased on the way in, so "PCI" and "pci" cannot both exist.
     */
...
const enumerationStepSchema = new mongoose.Schema(
  {
    ...
    /**
     * Which borrowed account this step was run as.
     *
     * A client lends three logins and afterwards asks what was done with them. Recorded here, on
     * the work, rather than as a list on the credential: one place to write it, and the
     * per-credential answer is derived by `credentialUsage` rather than kept in a second array
...
const auditSchema = new mongoose.Schema(
  {
    ...
    /**
     * Everything the test changed, created or removed on somebody else's system.
     *
     * See `intrusionSchema`. Its own array rather than a kind of note, because the questions asked
     * of it are different: "what is still outstanding" at the end of a job, and "what did you touch"
     * from the client. A note answers neither.
     */
    intrusions: [intrusionSchema],
