...
/**
 * Recordings, which are evidence too.
 *
 * A race condition, a click-through, a UI that only misbehaves in motion: none of those are a
 * screenshot, and until now they went into a chat message and died there. The store already
 * chunks arbitrary bytes through GridFS, so holding them costs nothing new.
 *
 * Two formats, and deliberately only two. The browser has to *decode* the file to take the still
 * frame that goes in the report, and it has to play it back for anybody reviewing the evidence —
 * MP4 and WebM are what every browser does both for. A QuickTime .mov is refused with a message
 * saying to export it, because storing a recording the app can neither show nor illustrate is
 * worse than not taking it: it looks like evidence and is not.
 */
...
/**
 * Recordings get more room, and not much more.
 *
 * The upload is buffered in memory before it is chunked, so this is a ceiling on what one request
 * can cost the API process, not on what disk can hold. Sixty-four megabytes is a couple of minutes
 * of screen capture, which is also about the longest recording anybody watches — evidence of a
 * specific behaviour, not a session.
 */
export const MAX_VIDEO_BYTES = 64 * 1024 * 1024;

/** What a stored object is, without having to look at its content type. */
export const mediaKind = (contentType) =>
  ALLOWED_VIDEO_TYPES[contentType] ? 'video' : 'image';

/**
 * Why a file was refused, said usefully.
 *
 * Shared with the upload route because that is where most refusals actually happen: multer decides
 * from the declared type before any of this runs, so a message written only here would never be
 * the one anybody saw. The commonest wrong file by a distance is a QuickTime `.mov` off a Mac's
 * screen recorder, and the fix is one export away — so it is worth naming rather than answering
 * with a list of what we do take.
 */
export function unsupportedTypeMessage(contentType) {
  const type = String(contentType ?? '');
  if (/quicktime|x-msvideo|x-matroska|mpeg|ogg|3gpp|x-ms-wmv/i.test(type)) {
    return 'That recording is in a format browsers cannot all play. Export it as MP4 or WebM and it will go in.';
  }
  if (/^video\//i.test(type)) {
    return `${type} is not a recording we can store — MP4 and WebM are the two every browser plays.`;
  }
  return `${type || 'That file type'} is not evidence we can store`;
}
...
export function mediaIdsInAudit(audit, { enumerationHtml = [] } = {}) {
  ...
  /*
   * And the write-up on an intrusive change, which is usually the whole reason one was recorded:
   * "this is what the endpoint did when I sent it a different id". Missing this would drop the
   * screenshot from the report and count it as an orphan for the media collector.
   */
  for (const intrusion of audit?.intrusions ?? []) scan(intrusion.notes);
...
/**
 * The same question for a recording.
 *
 * MP4 and its relatives put a `ftyp` box at the front, four bytes in; Matroska — which is what a
 * WebM is — starts with the EBML magic number. Neither tells us the codec inside, which is why
 * the browser making a still frame at upload time is also the check that the file is playable.
 *
 * @returns {string|null} the sniffed extension, or null if it is not a video we take
 */
...
export async function saveMedia({
  ...
  const declared = ALLOWED_UPLOAD_TYPES[contentType];
  if (!declared) throw badRequest(unsupportedTypeMessage(contentType));
  if (!buffer?.length) throw badRequest('That file is empty');

  const wantsVideo = Boolean(ALLOWED_VIDEO_TYPES[contentType]);
  const ceiling = wantsVideo ? MAX_VIDEO_BYTES : MAX_MEDIA_BYTES;
  if (buffer.length > ceiling) {
    throw badRequest(
      `${wantsVideo ? 'Recordings' : 'Images'} are limited to ${Math.round(
        ceiling / 1024 / 1024
      )} MB each`
    );
  }
  ...
  const sniffed = wantsVideo ? sniffVideoType(buffer) : sniffImageType(buffer);
  if (!sniffed) {
    throw badRequest(
      wantsVideo
        ? 'That file is not a recording we recognise'
        : 'That file is not an image we recognise'
    );
  }
  ...
  const ext = sniffed;
  const resolvedType =
    Object.entries(ALLOWED_UPLOAD_TYPES).find(([, value]) => value === ext)?.[0] ?? contentType;
  ...
  const id = await new Promise((resolve, reject) => {
    const stream = mediaBucket().openUploadStream(safeName, {
      ...
      metadata: {
        ...
        height,
        ext,
        /* Stored rather than derived on every read: the bin, the lightbox and the report all ask. */
        kind: wantsVideo ? 'video' : 'image',
        /*
         * The still frame that stands in for a recording.
         *
         * Taken by the browser at upload time and stored as an ordinary image, because deriving a
         * frame on the server would mean a video decoder in the dependency list of a reporting
         * tool. It is what the document prints — a .docx cannot hold a video — and what the
         * evidence bin shows.
         */
        poster: wantsVideo && poster ? String(poster) : null,
         ...
         *
         * 'client' on a screenshot attached to a remediation claim through a share link. Empty on
         * everything the team captured itself, which is almost everything. The evidence bin reads
         * it to say so, because a screenshot the client sent is evidence of what they believe and
         * not evidence of what was tested, and a report that quoted one as ours would be a
         * different kind of document.
         */
        source: source ? String(source).slice(0, 20) : '',
      },
    });
...
/**
 * A read stream, for serving. Never buffers the whole file in the API process.
 *
 * The optional window is what makes a recording scrubbable: a `<video>` asks for a byte range and
 * GridFS can open the stream at an offset rather than reading from the beginning and throwing away
 * the front. `end` is exclusive here, as GridFS has it, and inclusive in the HTTP header — the
 * route does that conversion at the boundary.
 */
export function mediaStream(id, { start, end } = {}) {
  const options = {};
  if (Number.isFinite(start) && start > 0) options.start = start;
  if (Number.isFinite(end)) options.end = end;
  return mediaBucket().openDownloadStream(toObjectId(id), options);
}
...
export async function loadMediaMap(ids) {
  ...
  await Promise.all(
    unique.map(async (id) => {
      ...
      /*
       * A recording is never embedded, so its bytes are never read.
       *
       * Both ids reach this function: a still frame is referenced as an `<img src>` and the
       * recording behind it as a `data-video` attribute on the same picture, and the scan that
       * feeds this looks for `/api/media/<id>` wherever it appears — which is exactly what keeps
       * the recording from being swept up as unused. Without this line every render would pull
       * sixty megabytes of video into the render cache to embed nothing at all.
       *
       * Left out of the map rather than set to null: null means "an image that should be here and
       * is not", which prints "(figure removed)". Nothing ever looks a recording up, because a
       * `data-video` attribute is not an image source.
       */
      if ((file.metadata?.kind ?? mediaKind(file.metadata?.contentType)) === 'video') return;
      try {
        /*
...
export async function auditEvidenceBin(auditId) {
  ...
  /*
   * A recording's still frame is not a second piece of evidence.
   *
   * It is stored as an ordinary image, so without this the bin would offer both the recording and
   * the frame taken out of it as two separate things to insert — and somebody would insert the
   * still, which looks right and loses the recording.
   */
  const posters = new Set(files.map((file) => file.metadata?.poster).filter(Boolean).map(String));

  return files
    .filter((file) => !used.has(file._id.toString()) && !posters.has(file._id.toString()))
    .map((file) => ({
      ...
      /** 'client' when somebody outside the team attached it. Empty for the team's own. */
      source: file.metadata?.source ?? '',
      uploadedAt: file.uploadDate ?? null,
      uploader: file.metadata?.uploader ? String(file.metadata.uploader) : null,
      /* What it is, and — for a recording — the frame to show in its place. */
      kind: file.metadata?.kind ?? mediaKind(file.metadata?.contentType),
      poster: file.metadata?.poster ? mediaUrl(String(file.metadata.poster)) : null,
    }));
}
