...
async function readWindow({ from, to, viewer }) {
  const [people, allBookings, leave, visibleAudits, liveAuditIds] = await Promise.all([
    ...
    /*
     * Every live engagement's id, whoever it belongs to — a *different* question from the
     * scoped list above it. That one decides what a booking is labelled; this one decides
     * whether the booking is a commitment at all. Unscoped for the reason in the file note:
     * a day taken by an engagement the reader cannot see is still taken.
     */
    Audit.distinct('_id', { deletedAt: null }),
  ]);

  /*
   * A booking against an engagement that has been trashed or destroyed is not a promise
   * anybody will keep, so it does not make somebody unavailable. Without this, trashing a
   * job kept its people out of the staffing list until the window rolled past it.
   */
  const live = new Set(liveAuditIds.map((id) => String(id)));
  const bookings = allBookings.filter((booking) => live.has(String(booking.audit)));
