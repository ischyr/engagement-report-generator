...
const ALLOWED_ATTRS = {
  ...
  /*
   * `data-figure` says whether a picture is evidence. The app sets it to "no" on the charts and
   * signatures it composes itself, and the numbering reads it — so it has to survive sanitising or
   * the HTML report captions a signature as a figure.
   */
  /*
   * `data-video` is the other one, and it is here for the same reason: it says this picture is a
   * frame out of a recording, and the caption says so in the deliverable. Dropped here, the HTML
   * report would quietly claim a still was the whole of the evidence while the .docx said
   * otherwise.
   */
  img: new Set(['src', 'alt', 'width', 'height', 'data-figure', 'data-video']),
  /*
   * The figure chip, and only the figure chip.
   *
   * `data-figref` carries a media id and nothing else, and it is what turns "the screenshot below"
   * into "Figure 7" in the deliverable. Allowing one named data attribute rather than the family:
