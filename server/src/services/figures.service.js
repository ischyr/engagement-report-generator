...
/**
 * A picture the app composed rather than one somebody put in a write-up.
 *
 * Charts and signatures are marked at the point they are built. They are not evidence: a chart
 * carries its own legend and a signature captioned "Figure 8" reads badly, and numbering either
 * would push every real figure along.
 */
const NOT_EVIDENCE = /\bdata-figure=["']?no["']?/i;
...
export function numberFiguresHtml(html, { label = 'Figure' } = {}) {
   ...
   */
  /*
   * `recording` adds the note that keeps the two deliverables honest with each other: a document
   * can only hold the still frame, and a reader looking at a static picture has no way of knowing
   * there is half a minute of evidence behind it. The Word path says exactly the same sentence —
   * see `#figureCaption` in `html2ooxml.js`.
   */
  const mark = (media, existing = '', recording = false) =>
    `<span class="engy-figure-number" data-fignum="${media}"></span>${
      existing.trim() ? ` — ${existing}` : ''
    }${recording ? `${existing.trim() ? ' ' : ''}(still from a screen recording)` : ''}`;

  /** Set by the editor on a frame taken out of a recording. */
  const isStill = (markup) => /\sdata-video=/i.test(String(markup ?? ''));
