...
import { CLASSIFICATION_LABELS } from './classification.service.js';
...
import { copyLabel, provenanceFor, outputHash } from './provenance.service.js';
import { INTRUSION_ACTION_LABELS } from '../models/audit.model.js';
import { credentialUsageForReport } from './credential-usage.service.js';
...
import { operationTimeline } from './operation-timeline.service.js';
...
/**
 * The one rich field on an intrusive change: the write-up, with the screenshots in it.
 *
 * `before` and `after` are deliberately not here. They are plain text and stay plain text, because
 * what they are for is being copied back into somebody's system.
 */
const INTRUSION_RICH_FIELDS = ['notes'];
 ...
 */
/**
 * Fields a finding carries that the document must never see.
 *
 * `tags` is here for a different reason from the other three, and the more important one. Those are
 * bookkeeping the template has no use for; a tag is a note the team wrote to *itself* — "client
 * disputes", "needs retest", "chained with 4" — and the one place it must not turn up is the
 * client's own report. A weakness class that should print wants a field of its own rather than a
 * convention on this one.
 *
 * `clientClaim` is the same argument again, and sharper. It holds what somebody outside the firm
 * asserted through their own link, now including a sentence they typed and the screenshots they
 * attached. A claim is not a verified fix — the model keeps the two apart deliberately — so a
 * template able to print it could report a fix on the strength of the client saying so, in their
 * own words. A retest that has actually been done moves `remediationStatus`, which does print.
 */
const INTERNAL_FINDING_FIELDS = ['comments', 'createdBy', 'updatedBy', 'tags', 'clientClaim'];
...
export function buildReportData(audit, settings, ooxmlOptions, options = {}) {
  ...
  /**
   * The four chronologies as one, from the blocks already gathered above.
   *
   * Computed here rather than handed in like the others, because it needs no query of its own:
   * every source is already in this function, and a fifth argument on every call site would be
   * four things restated. It reads the *report* shapes deliberately — a detection row that has
   * already been formatted still carries its instant and its latency in minutes, and the campaign
  ...
  const timelineLog = operationTimeline(
    ...
    /* The client's own date pattern, with the time, exactly as the detection log is formatted. */
    { formatDateTime: (value) => formatDate(value, `${dateFormat} HH:mm`) }
  );
  ...
  /*
   * What the test changed, in the order it was recorded.
   *
   * Only the ones marked to print: a file written to our own jump box is not the client's
   * business, and the tab lets an operator say so. Dates are formatted here, like every other
   * date in this file, so this appendix follows the client's own pattern.
   */
  const intrusionList = (audit.intrusions ?? [])
    .filter((entry) => entry.print !== false)
    .slice()
    .sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
    .map((entry) => {
      const raw = typeof entry.toObject === 'function' ? entry.toObject() : entry;
      const { plain, rich } = expandRichFields(raw, INTRUSION_RICH_FIELDS, ooxmlOptions, target);
      const reverted = Boolean(raw.revertedAt);
      const reversible = raw.reversible !== false;
      return {
        ...plain,
        rich,
        title: raw.title ?? '',
        action: raw.action ?? 'modified',
        actionLabel: INTRUSION_ACTION_LABELS[raw.action] ?? 'Modified',
        target: raw.target ?? '',
        host: raw.host ?? '',
        before: raw.before ?? '',
        after: raw.after ?? '',
        revertPlan: raw.revertPlan ?? '',
        at: raw.at ?? '',
        authorisedBy: raw.authorisedBy ?? '',
        reversible,
        reverted,
        revertedAt: raw.revertedAt ? formatDate(raw.revertedAt, dateFormat) : '',
        revertNote: raw.revertNote ?? '',
        /* One column for a table, rather than three flags a template has to combine. */
        state: reverted
          ? 'Put back'
          : !reversible
            ? 'Cannot be undone'
  ...
  /* What was done with the accounts the client lent us. Never a secret: see the service. */
  const credentialUse = credentialUsageForReport(audit, options.credentials ?? []);
  ...
  const data = {
    ...
    /**
     * How the document is to be handled — the marking that belongs on the page.
     *
     * The classification has driven the retention window and the second factor on the engagement
     * since it was added, and it stopped at the door: the app knew a report was restricted and the
     * document said nothing at all. Which is the one place it matters, because handling markings are
     * for the printed copy somebody leaves on a desk, not for the database row.
     *
     * Exposed the same way `isDraft` is — a label to print and a flag to wrap a header, a footer or
     * a watermark in — rather than as a banner this code composes. Where a marking goes and what it
     * says is a house-style decision, and this file has no business having an opinion about the
     * top of somebody's page.
     */
    /**
     * And who this copy was made for, on the copies that were made for somebody.
     *
     * The same shape as the handling marking above and for the same reason: a flag to wrap a
     * footer or a watermark in, and a line of text to put inside it. Where a per-recipient
     * marking belongs on the page is a house-style decision, so this file states the fact and
     * expresses no opinion:
     *
     *     {{#isCopy}}{{ copyLabel }}{{/isCopy}}
     *
     * A template that ignores it still produces a traceable copy, because the recipient is in
     * the document's properties either way, but it will not say so where a reader can see it.
     * `POST /audits/:id/report/copies` reports which of the two happened.
     */
    isCopy: Boolean(options.copy?.recipient),
    copyLabel: copyLabel(options.copy ?? {}),
    copyFor: String(options.copy?.recipient ?? ''),
    copyNumber: Number(options.copy?.number) || 0,
    copyOf: Number(options.copy?.of) || 0,

    classification: audit.classification ?? 'standard',
    classificationLabel: CLASSIFICATION_LABELS[audit.classification] ?? CLASSIFICATION_LABELS.standard,
    isRestricted: audit.classification === 'restricted',
     ...
     *
     * The appendix every pentest report has and nobody has ever generated from data: the accounts
     * created, the files uploaded, the records modified, and whether each one has been put back.
     * It is what a client's operations team reads before they sign the job off.
     *
     * Preflight refuses to approve a report while anything reversible is still standing, so a
     * document that prints this list is a document whose list is finished.
     */
    intrusions: intrusionList,
    hasIntrusions: intrusionList.length > 0,
    intrusionSummary: {
      total: intrusionList.length,
      reverted: intrusionList.filter((entry) => entry.reverted).length,
      outstanding: intrusionList.filter((entry) => !entry.reverted && entry.reversible).length,
      irreversible: intrusionList.filter((entry) => !entry.reversible).length,
    },
    /** The ones the client has to deal with, for a "left for you" table. */
    intrusionsOutstanding: intrusionList.filter((entry) => !entry.reverted && entry.reversible),

    /**
     * What was done with the accounts the client lent us.
     *
     * Labels, usernames and where each one was used. No secret is in this data and none can be:
     * the vault is never opened during a render.
     */
    credentialsUsed: credentialUse,
    hasCredentialsUsed: credentialUse.length > 0,
    ...
    rich: {
       ...
       */
      /**
       * The scope changes as dated prose: `{{@rich.scopeTimeline}}`.
       *
       * The `scopeChanges` loop is the record — a table of what was agreed with whom. This is the
       * same facts written out, and it is the one that answers "you never tested the payment
       * sandbox", because a client reads a sentence and interprets a row.
       *
       * Built eagerly, unlike the signatures and the charts below: it is a handful of paragraphs
       * and allocates nothing, so there is no orphaned media to avoid.
       */
      scopeTimeline:
        target === 'html'
    ...
    /**
     * "Two additions and one removal were agreed with the client while testing was under way."
     *
     * The counts as a sentence, because that is the form the counts are read in. A template that
     * wants to build its own has the numbers above.
     */
    scopeChangeSentence: scopeLog?.sentence ?? '',
    ...
    /**
     * The whole operation in one order: what was run, what the campaign did, what anybody noticed,
     * when the scope moved, when the report went.
     *
     * The four tables it merges all stay — this is the narrative version, and a red team readout
     * needs both: the chronology to argue from, and the detection table to be precise from.
     * `timelineSummary.longestQuiet` is the sentence the room actually wants, and it is only
     * knowable once the four are in one order.
     */
    timeline: timelineLog.rows,
    timelineSummary: timelineLog.summary,
    hasTimeline: timelineLog.recorded,
    /** How many steps had no recorded run time, so a template can be honest about the gap. */
    timelineSkipped: timelineLog.skipped,
...
export async function generateReport({
  ...
  const data = buildReportData(audit, resolvedSettings, ooxmlOptions, {
    ...
    phishing,
    enumerationBodies,
    /* Who this copy is for, when it is a copy for somebody. See the note beside `isCopy`. */
    copy,
  });
