...
export function renderDocx({
  ...
  /*
   * Number the figures, now that there is a document to count them in.
   *
   * After the render because until docxtemplater has finished there is no document order — the
   * template decides which sections come first and whether the appendix is at the front, and the
   * number on a caption is a fact about the finished file rather than about the finding it came
   * from. Before the commit because this rewrites a part, and commit is what writes the package.
   *
   * A template with no figures in it is untouched: the pass finds nothing and replaces nothing.
   */
