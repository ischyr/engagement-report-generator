...
export async function recordActivity({ audit, actor, action, target, fields, meta, summary }) {
  try {
    ...
    /*
     * And, if an administrator has asked for it, a chat channel hears about it.
     *
     * Here rather than in a `post('save')` hook on the model, because such a hook is ignored when
     * it is added after the model has been compiled — which it always is. Not awaited: the entry is
     * the caller's answer, and a slow chat service is not allowed to become a slow save. See
     * `webhooks/index.js`.
     */
    announceActivity(entry);
    return entry;
  } catch (error) {
...
function buildSummary({ actor, action, target, fields, meta }) {
  ...
  const sentences = {
    ...
    [ACTIONS.AUDIT_TEXT_REPLACED]: `${who} ${
      meta?.replaced ? `renamed ${what} to "${meta.replaced}"` : `removed ${what}`
    } in ${meta?.count ?? 0} place(s)${
      meta?.skipped ? `, leaving ${meta.skipped} that somebody was holding` : ''
    }`,
    ...
    [ACTIONS.FINDING_ASSIGNED]: meta?.assigned
      ? `${who} gave ${what} to ${meta?.to || 'somebody'} to write`
      : `${who} left ${what} unassigned`,
    ...
    [ACTIONS.SHARE_LINK_CREATED]: `${who} made a client ${
      meta?.kind === 'status' ? 'progress' : 'findings'
    } link for ${what}`,
...
/**
 * Tells somebody a finding is theirs to write.
 *
 * One notification per *person*, not per finding. Splitting an engagement is one decision made
 * once — "you take the web ones, I will do the infrastructure" — and twenty bells announcing it
 * twenty times is how a bell stops being read, which is the same argument `notifyCheckAssigned`
 * makes about a checklist and the reason the bulk route hands the whole list to this in one call.
 *
 * @param {object} options
 * @param {object|string} options.user who it is now theirs
 * @param {object|string} options.actor who decided that
 * @param {object} options.audit the engagement, for the message and the link
 * @param {string[]} options.titles the findings, in the order they were assigned
 */
export async function notifyFindingAssigned({ user, actor, audit, titles = [] }) {
  const recipient = String(user?._id ?? user ?? '');
  if (!recipient || !titles.length) return false;
  /* Taking one yourself is not news. Telling you what you just did is noise. */
  if (recipient === String(actor?._id ?? actor ?? '')) return false;
  ...
  await Notification.create({
    ...
    audit: audit._id,
    auditName: audit.name ?? '',
    target: one ? titles[0] : `${titles.length} findings`,
    message: one
      ? `${nameOf(actor)} gave you "${titles[0]}" on ${audit.name}`
      : `${nameOf(actor)} gave you ${titles.length} findings on ${audit.name}`,
    /*
     * The list, not the finding, even when there is only one of them: what somebody does next is
     * look at everything that is now theirs, and the list is where the rest of it is.
     */
