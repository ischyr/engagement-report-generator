...
/**
 * Whether a rich field has anything in it worth printing.
 *
 * Not the same question as "does it have text", and the difference cost a real report. A write-up
 * that is a single screenshot — which is most of them, in an enumeration — strips to the empty
 * string, so a `{{#hasContent}}` guard skipped it and the picture never reached the document. No
 * error: the template asked whether there was content, and was told there was not.
 *
 * So an image counts, and so does a table. Both are things a reader sees and neither leaves any
 * text behind.
 */
export function htmlHasContent(input) {
  const html = String(input ?? '');
  if (!html) return false;
  if (htmlToPlainText(html).trim().length > 0) return true;
