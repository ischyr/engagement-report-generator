...
export class DocxAssembler {
  constructor(zip) {
    ...
    this.docPrId = 1000;
    /**
     * Figure bookmarks, allocated once per screenshot for the whole document.
     *
     * Here rather than in the rich-text converter because a converter is built per *value* — one
     * for a finding's description, another for its proof of concept, another for the next finding —
     * and a bookmark name has to be unique across the document, while a reference written in one
     * field points at a figure that lives in another. The assembler is the only thing all of them
     * share.
     *
     * Keyed by the media id, so the reference does not have to know where the picture ended up.
     * The same screenshot printed twice is deliberately one bookmark and one number: to a reader it
     * is one figure, and a second number for it would send the reference somewhere they have
     * already been.
     */
  ...
  /**
   * The bookmark name for one figure, allocated the first time anybody asks.
   *
   * Asked by both ends and deliberately says nothing about which: a reference written in the
   * description is converted before the picture in the proof of concept, so whichever arrives
   * first decides the name and the other agrees. Naming it after the media id would have done the
   * same job, but only for stored evidence — this works for a pasted data URI too.
   *
   * @param {string} key the media id, or anything unique for a picture that is not stored evidence
   * @returns {{name:string, id:number}}
   */
  figureBookmark(key) {
    const existing = this.figureBookmarks.get(key);
    if (existing) return existing;

    this.nextBookmarkId += 1;
    const entry = {
      name: `${FIGURE_BOOKMARK}${String(key).replace(/[^A-Za-z0-9_]/g, '')}`,
      id: this.nextBookmarkId,
    };
    this.figureBookmarks.set(key, entry);
    return entry;
  }

  /**
   * Whether this is the first caption written for a picture — and therefore the one that carries
   * the number.
   *
   * Separate from the bookmark on purpose, and the separation is not academic: allocating the name
   * and deciding which caption is first were one method to begin with, so a *reference* converted
   * before its figure took the "first" flag and the caption that followed was silently left
   * unnumbered. Two questions, two answers.
   *
   * The same screenshot printed in two findings still gets one number: to a reader it is one
   * figure, and a second number for it would send every reference somewhere they have been.
   */
