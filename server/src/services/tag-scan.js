...
export function stepScope(stack, { marker, name }) {
  ...
  /*
   * An inverted section opens a block too, and has to be pushed for the same reason `#` is.
   *
   * It did not, and the consequence was not a missing scope on the tags inside it — those are
   * usually none — but on everything *after* it. `{{^x}}…{{/x}}` inside `{{#findings}}` left the
   * close with nothing named `x` on the stack, and the tolerant fallback below popped whatever was
   * there instead: the findings loop. Every tag from that point on was reported at the top level,
   * so the six `rich.*` fields of a finding — which exist only inside the loop — were reported as
   * tags the app does not have, in a template that renders perfectly.
   *
   * `{{^x}}{{/x}}` is exactly as common as `{{#x}}{{/x}}` in a template that has to say "none
   * attached" when something is empty, which is why this only ever surfaced on the longer one.
   */
  if (marker === '^') {
    const opened = { kind: 'inverted', tag: name, scope: [...stack] };
    stack.push(name);
    return opened;
  }
