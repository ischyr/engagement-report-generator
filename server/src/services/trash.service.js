...
export async function purgeAuditData(ids) {
  ...
  await Promise.all([
    ...
    EnumerationBody.deleteMany({ audit: { $in: ids } }),
    KitItem.deleteMany({ audit: { $in: ids } }),
    /*
     * Booked days, which are a plan rather than a record: nobody is going to work on an
     * engagement that has been destroyed, and a row pointing at one that is gone could
     * never be reached to delete by hand either. Hours logged are deliberately not here —
     * those are a record of work that did happen.
     */
    Booking.deleteMany({ audit: { $in: ids } }),
    /* The undo window's contents, which would otherwise outlive the engagement they belong to. */
    Recycled.deleteMany({ audit: { $in: ids } }),
    /* A live link to an engagement that no longer exists would answer with somebody else's. */
    ShareLink.deleteMany({ audit: { $in: ids } }),
