...
export function chartBlockHtml(slices, { kind = 'donut', size, width, alt } = {}) {
  ...
  return (
    /*
     * `data-figure="no"`: this is not evidence and must not be numbered.
     *
     * Screenshots become "Figure 12" automatically, which is what a reader needs to point at one.
     * A chart drawn from the severity counts already carries its own legend directly underneath,
     * and a number over it would push every real figure in the report along by two.
     */
    `<p style="text-align:center"><img src="data:image/png;base64,${data}" ` +
    `width="${drawn.width}" height="${drawn.height}" alt="${description}" data-figure="no"></p>` +
    legendHtml(slices)
  );
}
