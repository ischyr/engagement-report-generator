 ...
 */
/**
 * Whose copy a hash belongs to, when it belongs to one person's rather than to everybody's.
 *
 * Reads the rows already fetched rather than asking the database again: the lookup has just found
 * them, and a second query to answer a question about the same documents would be able to disagree
 * with the first.
 */
function traceCopy(rows, hash) {
  for (const row of rows ?? []) {
    for (const recipient of row.recipients ?? []) {
      if (recipient.fileHash && recipient.fileHash === hash) {
        return {
          name: recipient.name ?? '',
          email: recipient.email ?? '',
          number: recipient.copyNumber ?? 0,
          of: row.copies ?? 0,
          delivery: String(row.id ?? row._id ?? ''),
        };
      }
    }
  }
...
export async function deliveryRegister(user, query = {}) {
  ...
  if (validHash) {
    /*
     * Either the bytes everybody got, or one recipient's own copy.
     *
     * A delivery made with a copy per recipient has a different hash for each of them, and the
     * delivery's own `fileHash` is only the first. Matching that alone would fail to find the one
     * kind of file this lookup exists for: the marked copy that turned up somewhere it should not.
     */
    filter.$or = [{ fileHash: hash }, { 'recipients.fileHash': hash }];
  }
    ...
    .map((row) => {
      ...
      return {
        ...
        recipients: (row.recipients ?? []).map((entry) => ({
          name: entry.name,
          email: entry.email,
          /* Present only on a delivery made as a copy per recipient. Zero and empty otherwise. */
          copyNumber: entry.copyNumber ?? 0,
          fileHash: entry.fileHash ?? '',
        })),
        /** One on an ordinary delivery; above one when each recipient got their own bytes. */
        copies: row.copies ?? 1,
  ...
  return {
     ...
     *
     * `copy` is the answer to the question somebody with a loose file is really asking. When the
     * hash matched one recipient's own copy rather than the delivery's shared bytes, it names that
     * person; when the bytes went to everybody, it stays null and the recipient list is the answer.
     */
    match: wantsHash
      ? {
          hash,
          ...
          reason: '',
          copy: traceCopy(deliveries, hash),
        }
      : null,
  };
}
