...
export const TAG_GROUPS = [
  ...
  },
  {
    ...
    tags: [
      ...
      {
        tag: 'classificationLabel',
        description:
          '"Standard" or "Restricted" — the handling marking. Put it in the header and the footer, '
          + 'so it is on the page somebody prints rather than only in the app.',
      },
      {
        tag: 'isRestricted',
        description:
          'True on a restricted engagement. Wrap a marking or a watermark in it: '
          + '{{#isRestricted}}RESTRICTED — HANDLE ACCORDINGLY{{/isRestricted}}.',
        kind: 'flag',
      },
      {
        tag: 'classification',
        description: 'The raw value, "standard" or "restricted", for a template that maps it itself.',
      },
      {
        tag: 'isCopy',
        description:
          'True on a copy made for one named recipient. Wrap a marking in it: '
          + '{{#isCopy}}{{ copyLabel }}{{/isCopy}}. False on an ordinary render, so the line '
          + 'disappears entirely.',
        kind: 'flag',
      },
      {
        tag: 'copyLabel',
        description:
          '"Prepared for Marijke de Vries · copy 2 of 4" — the whole marking as one line, which is '
          + 'the wording the register and the document properties also use. Empty unless this is a '
          + 'copy for somebody.',
      },
      {
        tag: 'copyFor',
        description: 'Just the name, for a template that wants to phrase the marking itself.',
      },
      {
        tag: 'copyNumber',
        description: 'Which copy this is, from 1. Zero on an ordinary render.',
      },
      /* ------------------------------------------------- what the test changed */
      {
        tag: 'intrusions',
        description:
          'What the test created, modified or deleted on the client\'s systems. A loop: '
          + '{{#intrusions}}{{ actionLabel }} {{ target }} — {{ state }}{{/intrusions}}. Each one has '
          + 'title, action, actionLabel, target, host, before, after, revertPlan, at, '
          + 'authorisedBy, reversible, reverted, revertedAt, revertNote, state, and rich.notes for '
          + 'the write-up with its screenshots. Only the ones marked to print are in it.',
        kind: 'loop',
      },
      {
        tag: 'hasIntrusions',
        description: 'True when anything was changed. Wrap the appendix in it.',
        kind: 'flag',
      },
      {
        tag: 'intrusionsOutstanding',
        description:
          'The changes still standing, for a "left for you to undo" table. Empty on an approved '
          + 'report: preflight refuses to approve one while a reversible change is outstanding.',
        kind: 'loop',
      },
      {
        tag: 'intrusionSummary.total',
        description: 'How many changes were recorded. Also .reverted, .outstanding, .irreversible.',
      },
      /* ------------------------------------------- what the accounts were used for */
      {
        tag: 'credentialsUsed',
        description:
          'The accounts the client lent you and what was done with each. A loop: label, username, '
          + 'url, unused, counts.findings, counts.steps, counts.intrusions, counts.total, and the '
          + 'lists findings, steps and intrusions. Never a secret — the vault is not opened by a '
          + 'render.',
        kind: 'loop',
      },
      {
        tag: 'hasCredentialsUsed',
        description: 'True when the engagement was given any credentials.',
        kind: 'flag',
      },
      { tag: 'copyOf', description: 'How many copies were made. Zero on an ordinary render.' },
  ...
  },
  {
    ...
    tags: [
      ...
      { tag: 'rich.references', description: 'References as a bullet list.', kind: 'rich' },
      {
        tag: 'discoveredBy',
        description:
          'Loop of { title, tool, target } — the enumeration steps this finding came out of. "How did you find this" is the first question in a red team readout. Guard with {{#hasDiscoveredBy}}.',
        kind: 'loop',
      },
  ...
  },
  {
    ...
    tags: [
      ...
      {
        tag: 'scopeChangeSentence',
        description:
          'The counts written out: "Two additions and one removal were agreed with the client while testing was under way."',
      },
      {
        tag: 'rich.scopeTimeline',
        description:
          'The same changes as dated prose — "Day 3 — 5 August 2026 — Added to scope: two staging hosts. Agreed with Dana by email." One tag, the whole narrative.',
        kind: 'rich',
      },
    ],
  },
  ...
  },
  {
    ...
    description:
      'Every dated thing that happened, merged into one chronology: steps that were run, what a phishing campaign did, what the client noticed, when the scope moved and when the report went. The four separate tables all stay — this is the narrative version, and it is the only place the shape of the time is visible. Guard it with {{#hasTimeline}}.',
    tags: [
      {
        tag: 'hasTimeline',
        description: 'True when anything dated was recorded at all.',
        kind: 'flag',
      },
      {
        tag: 'timeline',
        description:
          'Loop of { when, occurredAt, kind, label, detail, note, outcome, noticed, phase, source }, oldest first. `kind` is one of run, phishing, detection, scope, delivery; `note` is the sentence that matters for that row — "Noticed after 4 min", "Nobody noticed", "Agreed by email".',
        kind: 'loop',
      },
      {
        tag: 'timelineSummary.firstAction',
        description: 'When the first thing was done. A date, so give it a pattern with | date.',
      },
      {
        tag: 'timelineSummary.firstNoticed',
        description: 'When the client first noticed anything. Empty when they never did.',
      },
      {
        tag: 'timelineSummary.timeToFirstNoticedLabel',
        description:
          'How long the operation ran before anybody noticed, in words — "1 d 3 h". Empty when nothing was ever noticed, which is itself the finding, so guard on it.',
      },
      {
        tag: 'timelineSummary.longestQuiet.label',
        description:
          'The longest stretch in which nothing was noticed, in words. Inside {{#timelineSummary.longestQuiet}}, which also carries from, to and minutes.',
      },
      {
        tag: 'timelineSummary.actions',
        description: 'How many rows were things the team did, rather than things the client saw.',
      },
      { tag: 'timelineSummary.noticed', description: 'How many actions the client noticed.' },
      { tag: 'timelineSummary.events', description: 'Rows in the timeline altogether.' },
      {
        tag: 'timelineSkipped.runs',
        description:
          'Steps that were run with no recorded time, and are therefore not placed in the order. Worth printing: a timeline that silently omits half the work is worse than one that says so.',
      },
    ],
  },
  {
  ...
  },
  {
    title: 'Enumeration',
    description:
      'How the ground was mapped, from a red team engagement’s Enumeration tab — the tool, what it ran against, and what came back. Findings say what is wrong; this says how it was found, which on a red team is half of what the client is buying. Guard it with {{#hasEnumeration}}.',
    tags: [
      {
        tag: 'hasEnumeration',
        description:
          'True when at least one step has been recorded. Nothing recorded means the report says nothing rather than printing an empty heading.',
        kind: 'flag',
      },
      {
        tag: 'enumeration',
        description:
          'Loop of { index, title, tool, target, command, ranAt, output, author, depth, level, phase, phaseLabel, isGroup, hasChildren, hasOutput, hasCommand, hasContent, hasLedTo }, depth-first through the tree — the order the steps are arranged on the tab, which is the order they were done in.',
        kind: 'loop',
      },
      {
        tag: 'depth',
        description:
          'How deep this step sits: 0 at the top, 1 under a heading, and so on. The tree arrives flat because a template cannot loop recursively — indent by this, or print {{ .level }} for the same number counting from 1.',
      },
      {
        tag: 'number',
        description:
          'The hierarchical number — 1, 1.1, 1.2, 2 — which is what a reader wants against a tree. {{ .path }} is the same string. {{ .index }} stays flat, for counting. Steps held back as internal are removed before numbering, so the sequence never has gaps.',
      },
      {
        tag: 'commandRaw',
        description:
          'The command as it was authored, engagement variables unresolved — "subfinder -d $TARGET". {{ .command }} is the same line with the variables filled in, which is almost always the one to print.',
      },
      {
        tag: 'outputLines',
        description: 'How many lines the output has in total, whatever the print policy prints.',
      },
      {
        tag: 'printOmitted',
        description:
          'How many rows or lines the print policy left out of this step. Meaningful inside {{#printTruncated}}.',
      },
      {
        tag: 'printTotal',
        description: 'How many there were in total, so the extract can be put in proportion.',
      },
      {
        tag: 'printUnit',
        description:
          '"rows" or "lines", depending on whether the step printed a table or a pane — so one sentence reads correctly either way.',
      },
      {
        tag: 'printTruncated',
        description:
          'True when what this step prints — the pane or the table, whichever the template chose — was cut short by the print policy. {{ .printOmitted }} of {{ .printTotal }} {{ .printUnit }} were left out. Say so: a silently truncated sweep reads as a complete one.',
        kind: 'flag',
      },
      {
        tag: 'outputTruncated',
        description:
          'The same question about the raw pane specifically. {{ .outputOmitted }} is how many lines it left out. Prefer {{#printTruncated}} unless the template only ever prints the pane.',
        kind: 'flag',
      },
      {
        tag: 'outputFull',
        description:
          'The whole output as plain text regardless of the print policy — for a template that would rather decide for itself than be told.',
      },
      {
        tag: 'isGroup',
        description:
          'True when the row is a heading rather than something that was run: it has children and no tool, output or write-up of its own. Lets a template print "Subdomain Enumeration" as a heading and the six tools under it as steps.',
        kind: 'flag',
      },
      {
        tag: 'phaseLabel',
        description:
          'Reconnaissance / Initial access / Privilege escalation / Lateral movement / Actions on objective, or empty when the step is not labelled. {{ .phase }} is the raw value, for conditions.',
      },
      {
        tag: 'statusLabel',
        description:
          'Completed / Nothing found / Timed out / Blocked / Not pursued, or empty. A tool that ran and found nothing and one nobody got round to are different sentences at closeout. {{ .status }} is the raw value.',
      },
      {
        tag: 'summary',
        description:
          'One line saying what a section was for, meant to be printed above its children so the report reads as writing rather than as a log. Guard with {{#hasSummary}}.',
      },
      {
        tag: 'rich.outputTable',
        description:
          'The tool output as a real Word table, when its shape could be read — httpx, nmap and masscan by name, plus anything consistently delimited. Guard with {{#hasTable}}: most output does not parse, and the pane is the fallback. Inside the loop, write {{@rich.outputTable}}.',
        kind: 'rich',
      },
      {
        tag: 'tableColumns',
        description: 'The column headings of that table, as a loop, for a template that draws its own.',
        kind: 'loop',
      },
      {
        tag: 'table.rows',
        description:
          'The rows, as a loop of { cells }, each cell { column, value } — for drawing the table by hand instead of using {{@rich.outputTable}}.',
        kind: 'loop',
      },
      {
        tag: 'ledToFindings',
        description:
          'Loop of { identifier, title } — the findings this step was written up as. Guard with {{#hasLedTo}}.',
        kind: 'loop',
      },
      {
        tag: 'outputAt',
        description:
          'When this step\'s output was last pasted. Not `ranAt`, which is free text somebody typed, and not `updatedAt`, which moves when anybody fixes a typo in the title — this is the one that says when the tool actually ran.',
      },
      {
        tag: 'outputAge',
        description:
          'The same thing in whole days: 0 for today, null for a step that has never been run. `{{#reRun}}` is true when there is an earlier run behind it, with `{{ .previousOutputAt }}` for when it was replaced.',
      },
      {
        tag: 'notes',
        description:
          "Loop of { line, snippet, text, moved } — the lines of the output somebody marked, and what they said about each. `snippet` is the line's own text, which is why these are worth printing: the note reaches the reader even when the print policy capped the pane above it. Guard with {{#hasNotes}}; {{ .noteCount }} counts them.",
        kind: 'loop',
      },
      {
        tag: 'enumerationPhases',
        description:
          'Loop of { phase, label, count, steps } — the same steps grouped into the phases of the operation, for a report that tells the story in operation order rather than tab order. Phases with nothing in them are dropped, and {{#steps}} nests inside.',
        kind: 'loop',
      },
      {
        tag: 'rich.output',
        description:
          'The tool output as a monospaced code pane, line breaks and column alignment intact. Inside the loop, write {{@rich.output}}. Guard it with {{#hasOutput}} — a step that is a screenshot and a sentence has none.',
        kind: 'rich',
      },
      {
        tag: 'rich.content',
        description:
          'The write-up around the output: screenshots, an HTTP request and response, the prose. Inside the loop, write {{@rich.content}}.',
        kind: 'rich',
      },
      {
        tag: 'rich.notes',
      ...
      { tag: 'enumerationSummary.total', description: 'How many rows there are, headings included.' },
      {
        tag: 'enumerationSummary.steps',
        description: 'How many of them were actually something run, excluding the headings.',
      },
      { tag: 'enumerationSummary.groups', description: 'And how many are headings.' },
      {
        tag: 'enumerationSummary.ledToFindings',
        description: 'How many steps turned into a finding — the hit rate of the enumeration.',
      },
      {
        tag: 'enumerationSummary.notes',
        description: 'How many lines of output were marked across the whole chapter.',
      },
      {
        tag: 'enumerationSummary.withNotes',
        description: 'And how many steps have at least one.',
      },
      {
        tag: 'enumerationSummary.withTable',
        description: 'How many carry output that could be printed as a table rather than a pane.',
      },
      {
        tag: 'enumerationSummary.internal',
        description:
          'How many rows were held back as internal and are not in the loop above. Print it if you would rather tell the client the record is fuller than the report.',
      },
      {
        tag: 'enumerationSummary.byStatus',
        description:
          'Loop of { status, label, count } — outcomes with at least one step, so a closeout line can be written from it: "eight attempted, one timed out, none abandoned".',
        kind: 'loop',
      },
      {
        tag: 'enumerationSummary.withOutput',
        description: 'How many carry tool output, so an appendix can be sized honestly.',
      },
      {
        tag: 'enumerationSummary.tools',
        description: 'Loop of the tool names, deduplicated, in the order they first appear.',
        kind: 'loop',
      },
      {
        tag: 'enumerationSummary.toolList',
        description:
          'The same names as one comma-separated line — "httpx, amass, nuclei". The methodology paragraph most reports want.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    tags: [
      ...
      {
        tag: 'rich.severityChart',
        description:
          'The severity breakdown as a ring, drawn for you — {{@rich.severityChart}}. One arc per severity, sized by share, in the colours from Settings, with a legend underneath in the template’s own typeface. Must be alone in its paragraph, like every raw tag; prints nothing when there are no findings, so guard it with {{#stats.total}} if the heading above it should go too.',
        kind: 'rich',
      },
      {
        tag: 'rich.severityBar',
        description:
  ...
  },
  {
    ...
    description:
      'What had to be asked of the client during the test, and what was assumed when nobody answered. This is the caveats paragraph, recorded as it happened rather than remembered afterwards.',
    tags: [
      {
        tag: 'questions',
        description:
          'Loop of { text, context, askedOf, status, statusLabel, answer, answeredOn, isOpen, isAssumed } — the ones marked to print, in the order they were asked.',
        kind: 'loop',
      },
      {
        tag: 'assumptions',
        description:
          'The same, narrowed to what was assumed rather than answered. Most reports print this one and leave the rest internal.',
        kind: 'loop',
      },
      { tag: 'hasQuestions', description: 'Guard for the chapter.', kind: 'flag' },
      { tag: 'hasAssumptions', description: 'Guard for the caveats paragraph.', kind: 'flag' },
      {
        tag: 'allQuestions',
        description:
          'Every question including the ones not marked to print, for an internal copy of the report.',
        kind: 'loop',
      },
      {
        tag: 'openQuestions',
        description: 'How many were never settled — worth a sentence, or worth chasing before sending.',
      },
    ],
  },
  {
...
export function knownTagRoots() {
  ...
  const roots = new Set([
    ...
    /*
     * Fields of a `timeline` row. `label` above is the same name doing the same job, which is
     * why only three are new: what happened, the detail beside it, and the sentence about it.
     */
    'when', 'detail', 'note',
    ...
    'signedOn',
    /*
     * A row of the `deliveries` loop. Documented inside that loop's description and never added
     * here, so a template printing a distribution table — which the red team starter does — had
     * every one of its own tags reported as a tag that does not exist.
     */
    'version', 'recipientList', 'fileHashShort',
    ...
    'statement', 'image',
    // A row of the `questions` and `assumptions` loops.
    'askedOf', 'answer', 'isAssumed',
    // A row of the `scopeChanges` loop.
    'kind', 'kindLabel', 'summary', 'targets', 'targetList', 'agreedBy', 'channel', 'agreedOn', 'day',
    ...
    // A row of the `enumeration` loop, and of an `enumerationPhases` group.
    'tool', 'command', 'output', 'ranAt', 'content', 'hasOutput', 'hasCommand', 'hasContent',
    'depth', 'level', 'phase', 'phaseLabel', 'isGroup', 'hasChildren', 'hasLedTo',
    'ledToFindings', 'steps', 'discoveredBy', 'hasDiscoveredBy',
    'statusLabel', 'summary', 'hasSummary', 'table', 'hasTable', 'tableColumns',
    'tableRowCount', 'outputTable', 'cells', 'column', 'value', 'rows',
    'outputLines', 'outputTruncated', 'outputOmitted', 'outputFull', 'printOutput', 'internal',
    // A row of the `notes` loop on an enumeration step, plus its guard and count.
    'hasNotes', 'noteCount', 'line', 'snippet', 'text', 'moved',
    // When a step was actually run, as against when its row was last edited.
