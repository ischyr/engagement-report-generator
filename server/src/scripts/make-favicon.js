 ...
 * are both written by hand — neither format needs more than zlib, which Node has. The PNG
 * encoder itself lives in `utils/png.js`, because the report charts draw one too.
 */
...
/*
 * The encoder moved to `utils/png.js` when the report charts needed one too. Nothing about it
 * changed — the same colour type, the same filter, the same deflate level, so the bytes this
 * script writes are the bytes it wrote before.
 */
...
async function main() {
  ...
  const touch = encodePng(renderRgba(180), 180, 180);
