...
/* For reading a document as the driver sees it, which is the point of the storage checks below. */
...
async function main() {
  ...
  const call = async (who, method, path, body) => {
    ...
    const response = await fetch(`${base}${path}`, {
      ...
      headers: {
        /*
         * `null` means nobody: the client link is the one surface here that answers a request
         * with no account, and a test that could only speak as somebody could not check that.
         */
  ...
  try {
    ...
    log.info('Bulk actions on findings');
    {
      ...
      /* --------------------------------------------------------------- tags --- */
      /*
       * Labels on one finding, which the engagement has had for a while and a finding had not. The
       * questions they answer are the ones asked at the end of a test — which of these am I still
       * waiting on, which did the client push back on — and none of those was a field.
       */
      const tagged = await call(alice, 'POST', `/audits/${auditId}/findings`, {
        title: 'zz tags a labelled finding',
        cvssv3: 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:L/I:N/A:N',
        tags: ['Needs Retest', 'needs retest', 'PCI', '', '  spaced  '],
      });
      check(
        'a finding can be created with tags',
        tagged.status === 201,
        JSON.stringify(tagged.body?.error)
      );
      check(
        'normalised exactly as an engagement’s are: lower case, de-duplicated, blanks dropped',
        JSON.stringify(tagged.body?.tags) === JSON.stringify(['needs retest', 'pci', 'spaced']),
      ...
      const retagged = await call(
        alice,
        'PUT',
        `/audits/${auditId}/findings/${tagged.body._id}`,
        { tags: ['pci', 'chained with 4'] }
      );
      check(
        'and replaced by an edit',
        JSON.stringify(retagged.body?.tags) === JSON.stringify(['pci', 'chained with 4']),
        JSON.stringify(retagged.body?.tags)
      );
      ...
      const tooMany = await call(
        ...
        'PUT',
        `/audits/${auditId}/findings/${tagged.body._id}`,
        { tags: Array.from({ length: 21 }, (_unused, index) => `t${index}`) }
      );
      check('twenty-one tags is refused', tooMany.status === 422, `got ${tooMany.status}`);

      /* The suggestion list, which is what stops the same idea being spelled three ways. */
      const known = await call(alice, 'GET', '/audits/finding-tags');
      check(
        'every tag in use is offered, with how many findings carry it',
        known.status === 200 && (known.body ?? []).some((row) => row.tag === 'pci' && row.count >= 1),
        JSON.stringify(known.body)
      );
      check(
        'and the literal path is not read as an engagement id',
        Array.isArray(known.body),
        JSON.stringify(known.body).slice(0, 80)
      );

      /*
       * The one that matters most: a tag is a note to ourselves. `severityOverrideReason` prints and
       * this does not, and "client disputes" arriving in the client's own document would be a bad
       * afternoon. Checked against the whole payload rather than a field, because the failure worth
       * catching is the one that puts it somewhere nobody thought to look.
       */
      const forReport = await call(alice, 'GET', `/audits/${auditId}/report-data`);
      check(
        'tags never reach the report data',
        !JSON.stringify(forReport.body).includes('chained with 4'),
        'a finding’s private label is in what the document is built from'
      );

      /* --------------------------------------------- tagging a batch of them --- */
      /*
       * Adding rather than setting. Setting would throw away whatever else each of them was
       * labelled with, and the reason to tag five at once is that they have something *in addition*
       * in common.
       */
      const keepsOne = await call(alice, 'PUT', `/audits/${auditId}/findings/${made[0]}`, {
        tags: ['already here'],
      });
      check('one of the batch is labelled first', JSON.stringify(keepsOne.body?.tags) === '["already here"]');

      const batch = await call(alice, 'POST', `/audits/${auditId}/findings/bulk`, {
        ids: made,
        action: 'tag',
        value: 'Needs Retest',
      });
      check(
        'one call tags all five',
        batch.status === 200 && batch.body?.changed?.length === 5,
        JSON.stringify({ status: batch.status, changed: batch.body?.changed?.length })
      );
      check('and lower-cases it on the way in', batch.body?.tag === 'needs retest', batch.body?.tag);

      const after = await call(alice, 'GET', `/audits/${auditId}`);
      const firstOfBatch = (after.body?.findings ?? []).find((row) => row._id === made[0]);
      check(
        'without throwing away the tag it already had',
        JSON.stringify(firstOfBatch?.tags) === JSON.stringify(['already here', 'needs retest']),
        JSON.stringify(firstOfBatch?.tags)
      );

      const again2 = await call(alice, 'POST', `/audits/${auditId}/findings/bulk`, {
        ids: made,
        action: 'tag',
        value: 'needs retest',
      });
      check(
        'asking twice changes nothing and says so per finding',
        again2.body?.changed?.length === 0 && again2.body?.skipped?.length === 5,
        JSON.stringify({ changed: again2.body?.changed?.length, skipped: again2.body?.skipped?.length })
      );

      const removed = await call(alice, 'POST', `/audits/${auditId}/findings/bulk`, {
        ids: made,
        action: 'untag',
        value: 'needs retest',
      });
      check(
        'and taking it off again is the other half of the same control',
        removed.body?.changed?.length === 5,
        JSON.stringify(removed.body?.changed?.length)
      );
      const afterUntag = await call(alice, 'GET', `/audits/${auditId}`);
      const firstAgain = (afterUntag.body?.findings ?? []).find((row) => row._id === made[0]);
      check(
        'leaving the one it started with alone',
        JSON.stringify(firstAgain?.tags) === JSON.stringify(['already here']),
        JSON.stringify(firstAgain?.tags)
      );

      const nameless = await call(alice, 'POST', `/audits/${auditId}/findings/bulk`, {
        ids: made,
        action: 'tag',
        value: '   ',
      });
      check(
        'a tag of nothing is refused rather than stored',
        nameless.status === 400,
        `got ${nameless.status}`
      );

      await call(alice, 'DELETE', `/audits/${auditId}/findings/${tagged.body._id}`);
      await call(alice, 'PUT', `/audits/${auditId}/findings/${made[0]}`, { tags: [] });
    ...
    log.info('Schedule');
    {
      ...
      const window = await call(bob, 'GET', '/schedule?from=2025-08-31&to=2025-10-04');
      ...
      const narrow = await call(alice, 'GET', '/schedule?from=2025-09-01&to=2025-09-30');
      check(
    ...
    log.info('Findings as a spreadsheet');
    {
      ...
      /* ------------------------------------------- and as tickets, elsewhere --- */
      /*
       * The same findings in the shape somebody else's importer wants. The mappings themselves are
       * checked without a database in `npm run test:tracker`; what matters here is the route: that
       * it comes back as a CSV rather than a document, that it refuses a tracker nobody has mapped
       * rather than inventing one, and that the export is logged the way every other one is.
       */
      const jira = await fetch(`${base}/audits/${auditId}/findings.csv?tracker=jira`, {
        headers: { Authorization: `Bearer ${alice.token}` },
      });
      const csv = await jira.text();
      check(
        'the tracker export downloads as a CSV',
        jira.status === 200 && (jira.headers.get('content-type') ?? '').includes('text/csv'),
        `${jira.status} ${jira.headers.get('content-type')}`
      );
      check(
        'named for the tracker it is for',
        /jira\.csv/.test(jira.headers.get('content-disposition') ?? ''),
        jira.headers.get('content-disposition')
      );
      check(
        'with Jira\u2019s own column names on the first line',
        csv.split('\r\n')[0] === 'Summary,Description,Issue Type,Priority,Labels,Labels,Labels,Environment',
        JSON.stringify(csv.split('\r\n')[0])
      );
      check(
        'one line per finding once the quoted newlines are accounted for',
        (csv.match(/\r\n/g) ?? []).length >= (audit.body?.findings?.length ?? 0),
        `${(csv.match(/\r\n/g) ?? []).length}`
      );
      check(
        'and the write-up\u2019s own ampersand and angle brackets come through as text',
        csv.includes('Ampersand & angle <brackets>'),
        csv.slice(0, 400)
      );

      const unmapped = await fetch(`${base}/audits/${auditId}/findings.csv?tracker=trello`, {
        headers: { Authorization: `Bearer ${alice.token}` },
      });
      check(
        'a tracker nobody has mapped is refused, and says which are',
        unmapped.status === 400 && /jira/.test((await unmapped.json())?.error ?? ''),
        `${unmapped.status}`
      );
    ...
    log.info('Booking reminders');
    {
      ...
      await Books2.deleteMany({
        audit: { $in: [auditId, trashedId, expired.body._id] },
      });
    ...
    /* ---------------------------------------------- enumeration, on a red team -- */
    log.info('Enumeration steps');
    {
      /* The kind itself, which is what makes the tab appear. */
      const asRedTeam = await call(alice, 'PUT', `/audits/${auditId}`, { kind: 'redteam' });
      check(
        'an engagement can be a red team',
        asRedTeam.status === 200 && asRedTeam.body?.kind === 'redteam',
        `${asRedTeam.status} ${JSON.stringify(asRedTeam.body?.kind)}`
      );

      const OUTPUT = 'https://www.acme.example  [200]\nhttps://api.acme.example  [401]';
      const section = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz Subdomain Enumeration',
        phase: 'recon',
      });
      check(
        'a section can be recorded',
        section.status === 201 && Boolean(section.body?._id),
        `${section.status} ${JSON.stringify(section.body?.error)}`
      );

      const first = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        parent: section.body._id,
        title: 'zz subfinder',
        tool: 'subfinder',
        target: 'acme.example',
        command: 'subfinder -d acme.example',
        output: OUTPUT,
        phase: 'recon',
      });
      check(
        'and a step nested under it',
        first.status === 201 && String(first.body?.parent) === String(section.body._id),
        `${first.status} ${JSON.stringify(first.body?.parent)}`
      );
      /*
       * Verbatim, newlines and column padding included. The whole value of tool output is being
       * exactly what the tool printed, so anything that trims or collapses it is a bug even when
       * the text still reads the same.
       */
      check(
        'its output is stored exactly as pasted',
        first.body?.output === OUTPUT,
        JSON.stringify(first.body?.output)
      );

      const second = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        parent: section.body._id,
        title: 'zz crt.sh',
        tool: 'crt.sh',
      });
      const loose = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz WebServer Enumeration',
        tool: 'Burp',
        phase: 'access',
      });

      const listed = await call(alice, 'GET', `/audits/${auditId}/enumeration`);
      check(
        'the tree comes back depth-first with a depth on each row',
        (listed.body ?? []).map((r) => `${r.depth}:${r.title}`).join(' | ') ===
          '0:zz Subdomain Enumeration | 1:zz subfinder | 1:zz crt.sh | 0:zz WebServer Enumeration',
        JSON.stringify((listed.body ?? []).map((r) => `${r.depth}:${r.title}`))
      );
      check(
        'and a section says it has children',
        (listed.body ?? [])[0]?.hasChildren === true,
        JSON.stringify((listed.body ?? [])[0]?.hasChildren)
      );
      /* A child appends to the bottom of its own branch, not of the whole document. */
      check(
        'a new step is appended within its branch',
        (listed.body ?? [])[2]?._id === second.body?._id,
        JSON.stringify((listed.body ?? [])[2]?.title)
      );

      /* Moving a branch: the section, with both children, under the other top-level step. */
      const moved = await call(alice, 'PUT', `/audits/${auditId}/enumeration-order`, {
        order: [
          { id: loose.body._id, parent: null },
          { id: section.body._id, parent: loose.body._id },
          { id: first.body._id, parent: section.body._id },
          { id: second.body._id, parent: section.body._id },
        ],
      });
      check(
        'a whole branch can be moved and keeps its children',
        (moved.body ?? []).map((r) => `${r.depth}:${r.title}`).join(' | ') ===
          '0:zz WebServer Enumeration | 1:zz Subdomain Enumeration | 2:zz subfinder | 2:zz crt.sh',
        JSON.stringify((moved.body ?? []).map((r) => `${r.depth}:${r.title}`))
      );

      /*
       * A ring would make every reader of this data loop for ever — the tab, the report, the
       * subtree delete all walk parents. The walk is defended, but the request is a mistake worth
       * naming rather than tolerating.
       */
      const ring = await call(alice, 'PUT', `/audits/${auditId}/enumeration-order`, {
        order: [
          { id: section.body._id, parent: first.body._id },
          { id: first.body._id, parent: section.body._id },
        ],
      });
      check('a step cannot be moved inside itself', ring.status === 400, `got ${ring.status}`);

      /*
       * An order that names only some of them keeps the rest rather than dropping them. A client
       * that rearranged while somebody else was adding a step must not delete the new one.
       */
      const partial = await call(alice, 'PUT', `/audits/${auditId}/enumeration-order`, {
        order: [{ id: loose.body._id, parent: null }],
      });
      check(
        'an order that leaves steps out keeps them',
        (partial.body ?? []).length === 4,
        `${(partial.body ?? []).length} rows`
      );

      /* Re-running a sweep: the outgoing output is kept so the tab can say what changed. */
      const before = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === first.body._id
      );
      const rerun = await call(alice, 'PUT', `/audits/${auditId}/enumeration/${first.body._id}`, {
        output: `${OUTPUT}\nhttps://new.acme.example  [200]`,
        expectedUpdatedAt: before.updatedAt,
      });
      check(
        're-running a step keeps the previous output to compare against',
        rerun.body?.previousOutput === OUTPUT && Boolean(rerun.body?.previousOutputAt),
        JSON.stringify(rerun.body?.previousOutput)
      );
      /* Saving something else must not throw last week's sweep away. */
      const titleOnly = await call(
        alice,
        'PUT',
        `/audits/${auditId}/enumeration/${first.body._id}`,
        { title: 'zz subfinder', expectedUpdatedAt: rerun.body.updatedAt }
      );
      check(
        'and saving a title does not discard it',
        titleOnly.body?.previousOutput === OUTPUT,
        JSON.stringify(titleOnly.body?.previousOutput)
      );

      /* Two operators writing up the same sweep: the second save must be refused, not silent. */
      const stale = await call(alice, 'PUT', `/audits/${auditId}/enumeration/${first.body._id}`, {
        title: 'zz written over',
        expectedUpdatedAt: before.updatedAt,
      });
      check('a stale write to a step is refused', stale.status === 409, `got ${stale.status}`);

      /* Written up as a finding, and the step survives with a link to it. */
      const promoted = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${first.body._id}/promote`,
        {}
      );
      check(
        'a step can be written up as a finding',
        promoted.status === 201 && Boolean(promoted.body?.finding?._id),
        `${promoted.status} ${JSON.stringify(promoted.body?.error)}`
      );
      check(
        'the output came along as a code block',
        String(promoted.body?.finding?.poc ?? '').includes('api.acme.example') &&
          String(promoted.body?.finding?.poc ?? '').includes('<pre>'),
        String(promoted.body?.finding?.poc ?? '').slice(0, 90)
      );
      const linked = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === first.body._id
      );
      check(
        'and the step keeps a readable link to it',
        (linked?.findings ?? []).some((f) => f._id === promoted.body.finding._id),
        JSON.stringify(linked?.findings)
      );
      /* One sweep can produce several findings, so a second write-up appends. */
      const again = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${first.body._id}/promote`,
        { title: 'zz second finding from the same sweep' }
      );
      const twice = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === first.body._id
      );
      check(
        'a second write-up is added rather than replacing the first',
        (twice?.ledTo ?? []).length === 2,
        JSON.stringify(twice?.ledTo)
      );

      /* Hosts the sweep found, into the scope. */
      const scoped = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${first.body._id}/to-scope`,
        { group: 'zz Discovered', hosts: ['www.acme.example', 'api.acme.example'] }
      );
      check(
        'discovered hosts can be added to the scope',
        scoped.status === 200 && scoped.body?.added?.length === 2,
        JSON.stringify(scoped.body)
      );
      /* Running the same sweep twice should be free, not an error. */
      const twiceScoped = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${first.body._id}/to-scope`,
        { group: 'zz Discovered', hosts: ['www.acme.example', 'extra.acme.example'] }
      );
      check(
        'adding one that is already there skips it instead of failing',
        twiceScoped.body?.added?.length === 1 && twiceScoped.body?.skipped === 1,
        JSON.stringify(twiceScoped.body)
      );
      const withScope = await call(alice, 'GET', `/audits/${auditId}`);
      check(
        'and the group holds them, marked as not tested yet',
        (withScope.body?.scope ?? [])
          .find((g) => g.name === 'zz Discovered')
          ?.hosts?.every((h) => h.status === 'pending'),
        JSON.stringify((withScope.body?.scope ?? []).find((g) => g.name === 'zz Discovered')?.hosts?.length)
      );

      /* Reportable, unlike notes — the entire reason the tab is not just a note. */
      const data = await call(alice, 'GET', `/audits/${auditId}/report-data`);
      const payload = data.body?.data ?? data.body ?? {};
      check(
        'enumeration reaches the report data',
        payload.hasEnumeration === true && (payload.enumeration ?? []).length === 4,
        `${payload.hasEnumeration} / ${(payload.enumeration ?? []).length}`
      );
      check(
        'with a depth on each row so a template can indent',
        (payload.enumeration ?? []).some((step) => step.depth > 0),
        JSON.stringify((payload.enumeration ?? []).map((s) => s.depth))
      );
      check(
        'and the phases grouped for a phase-by-phase report',
        (payload.enumerationPhases ?? []).some((g) => g.phase === 'recon' && g.count > 0),
        JSON.stringify((payload.enumerationPhases ?? []).map((g) => `${g.phase}:${g.count}`))
      );
      const promotedFinding = (payload.findings ?? []).find(
        (f) => String(f._id) === String(promoted.body.finding._id)
      );
      check(
        'the finding can say which step found it',
        promotedFinding?.hasDiscoveredBy === true &&
          (promotedFinding?.discoveredBy ?? []).some((d) => d.tool === 'subfinder'),
        JSON.stringify(promotedFinding?.discoveredBy)
      );

      const enumLog = await call(alice, 'GET', `/audits/${auditId}/activity`);
      const actions = (enumLog.body?.entries ?? []).map((e) => e.action);
      check(
        'the log records the enumeration and what came of it',
        actions.includes('enumeration.created') && actions.includes('enumeration.promoted'),
        JSON.stringify(actions.slice(0, 5))
      );

      const stranger = await makeUser('enumerator', 'user');
      const notAllowed = await call(stranger, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz not mine',
      });
      check(
        'somebody off the engagement cannot add a step to it',
        notAllowed.status === 403 || notAllowed.status === 404,
        `got ${notAllowed.status}`
      );

      /* Deleting a section takes its branch: leaving the children behind would orphan them. */
      const gone = await call(
        alice,
        'DELETE',
        `/audits/${auditId}/enumeration/${section.body._id}`
      );
      check(
        'deleting a section removes its steps with it',
        gone.status === 200 && gone.body?.removed === 3,
        `${gone.status} removed ${gone.body?.removed}`
      );
      const left = await call(alice, 'GET', `/audits/${auditId}/enumeration`);
      check(
        'and nothing is orphaned',
        (left.body ?? []).length === 1 && (left.body ?? [])[0]?.depth === 0,
        JSON.stringify((left.body ?? []).map((r) => `${r.depth}:${r.title}`))
      );

      /* ------------------------------------------------------------- presets -- */
      const offered = await call(alice, 'GET', `/audits/${auditId}/enumeration/presets`);
      check(
        'the presets are offered',
        offered.status === 200 && (offered.body ?? []).some((p) => p.key === 'subdomains'),
        JSON.stringify((offered.body ?? []).map((p) => p.key))
      );

      const applied = await call(alice, 'POST', `/audits/${auditId}/enumeration/preset`, {
        preset: 'subdomains',
        target: 'zz-preset.example',
      });
      check(
        'applying one builds a section and its steps',
        applied.status === 201 && applied.body?.added >= 8,
        `${applied.status} added ${applied.body?.added}`
      );
      const withPreset = await call(alice, 'GET', `/audits/${auditId}/enumeration`);
      const presetSection = (withPreset.body ?? []).find(
        (r) => String(r._id) === String(applied.body.section)
      );
      const presetKids = (withPreset.body ?? []).filter(
        (r) => String(r.parent) === String(applied.body.section)
      );
      check(
        'nested under the section, not at the root',
        presetKids.length >= 8 && presetKids.every((r) => r.depth === presetSection.depth + 1),
        `${presetKids.length} children`
      );
      /*
       * The commands keep `$TARGET` and resolve against the engagement's variable — which is what
       * makes changing the target one edit rather than thirty. An earlier version of this pasted the
       * domain into each command at apply time; that read the same and drifted the moment the target
       * changed, so the assertion is now about the variable surviving rather than being consumed.
       */
      check(
        'the commands keep $TARGET rather than having it pasted in',
        presetKids.some((r) => String(r.command ?? '').includes('$TARGET')),
        JSON.stringify(presetKids.map((r) => r.command).filter(Boolean)[0])
      );
      check(
        'and resolve against the variable the apply defined',
        presetKids
          .filter((r) => String(r.command ?? '').includes('$TARGET'))
          .every((r) => String(r.commandResolved ?? '').includes('zz-preset.example')),
        JSON.stringify(presetKids.map((r) => r.commandResolved).filter(Boolean)[0])
      );
      check(
        'the section carries the line that says what it was for',
        String(presetSection?.summary ?? '').length > 20,
        JSON.stringify(presetSection?.summary)
      );
      const badPreset = await call(alice, 'POST', `/audits/${auditId}/enumeration/preset`, {
        preset: 'zz-does-not-exist',
      });
      check('an unknown preset is refused', badPreset.status === 400, `got ${badPreset.status}`);

      /* -------------------------------------------------- outcome, and the table -- */
      const httpxOut = [
        'https://www.zz.example      [200] [Home] [nginx:1.24.0,React]',
        'https://api.zz.example      [401] [] [nginx:1.24.0]',
      ].join('\n');
      const parsedStep = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz httpx validation',
        tool: 'httpx',
        status: 'completed',
        output: httpxOut,
      });
      check(
        'a step records what became of it',
        parsedStep.body?.status === 'completed',
        JSON.stringify(parsedStep.body?.status)
      );
      const nonsense = await call(alice, 'PUT', `/audits/${auditId}/enumeration/${parsedStep.body._id}`, {
        status: 'zz-not-a-status',
      });
      /* 422 from the schema gate, which is what this app answers for a body it cannot accept. */
      check('and refuses an outcome that is not one', nonsense.status === 422, `got ${nonsense.status}`);

      /*
       * The tree says whether the output reads as columns; the step says what the columns are.
       *
       * A deliberate split. Sending the parsed table with every row meant a sixty-step operation
       * answered its own tree with megabytes of JSON — so the list carries the shape and the counts,
       * which is everything the tree draws, and opening a step fetches the one body being read.
       */
      const listedTable = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === parsedStep.body._id
      );
      check(
        'the tree row says the output reads as columns',
        listedTable?.hasTable === true && listedTable?.tableRows === 2,
        JSON.stringify({ hasTable: listedTable?.hasTable, rows: listedTable?.tableRows })
      );
      check(
        'and carries none of the output itself',
        !('output' in (listedTable ?? {})) &&
          !('table' in (listedTable ?? {})) &&
          !('content' in (listedTable ?? {})),
        JSON.stringify(Object.keys(listedTable ?? {}).filter((k) => ['output', 'table', 'content'].includes(k)))
      );
      check(
        'but enough of it to search on',
        String(listedTable?.outputPreview ?? '').length > 0 && listedTable?.outputLines === 2,
        JSON.stringify({ lines: listedTable?.outputLines })
      );

      const stepDetail = await call(
        alice,
        'GET',
        `/audits/${auditId}/enumeration/${parsedStep.body._id}`
      );
      check(
        'httpx output is read into columns for the tab',
        stepDetail.body?.table?.columns?.join(',') === 'URL,Status,Title,Technology' &&
          stepDetail.body.table.rows.length === 2,
        JSON.stringify(stepDetail.body?.table?.columns)
      );
      check(
        'and the step arrives with its output and its place in the tree',
        String(stepDetail.body?.output ?? '').includes('http') &&
          stepDetail.body?.path === listedTable?.path &&
          stepDetail.body?.depth === listedTable?.depth,
        JSON.stringify({ path: stepDetail.body?.path, depth: stepDetail.body?.depth })
      );
      /*
       * The one thing this endpoint could quietly break. `:stepId` is declared below every literal
       * path under `/enumeration`, and if it ever moves above them then `vars`, `presets`, `history`,
       * `preview` and `export` all start answering "step not found".
       */
      for (const word of ['vars', 'presets', 'history', 'preview', 'export']) {
        const literal = await call(alice, 'GET', `/audits/${auditId}/enumeration/${word}`);
        check(`the ${word} endpoint still answers itself`, literal.status === 200, `got ${literal.status}`);
      }
      /*
       * Prose must not become a table. A misaligned table looks authoritative, which makes a false
       * positive here worse than no table at all.
       */
      const prose = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz prose',
        tool: 'notes',
        output: 'We looked at the perimeter and found little of interest.',
      });
      const proseRow = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === prose.body._id
      );
      check('and prose is left as a pane', proseRow?.hasTable === false, JSON.stringify(proseRow?.hasTable));

      const tableData = await call(alice, 'GET', `/audits/${auditId}/report-data`);
      const tablePayload = tableData.body?.data ?? tableData.body ?? {};
      const reportStep = (tablePayload.enumeration ?? []).find((x) => x.title === 'zz httpx validation');
      check(
        'the report gets the table too',
        reportStep?.hasTable === true && reportStep?.tableRowCount === 2,
        JSON.stringify({ hasTable: reportStep?.hasTable, rows: reportStep?.tableRowCount })
      );
      check(
        'and an outcome tally to write a closeout line from',
        (tablePayload.enumerationSummary?.byStatus ?? []).some((r) => r.status === 'completed'),
        JSON.stringify(tablePayload.enumerationSummary?.byStatus)
      );

      /* --------------------------------------------- what a save answers with -- */
      /*
       * The response is the row, enriched, so nothing has to be re-fetched.
       *
       * Saving used to be followed by a request for the whole tree — sixty rows to change a title.
       * The contract this rests on: a save answers with the same shape the detail endpoint sends,
       * plus `treeChanged` when the save affects rows other than its own.
       */
      const renamed = await call(
        alice,
        'PUT',
        `/audits/${auditId}/enumeration/${parsedStep.body._id}`,
        { title: 'zz httpx validation renamed' }
      );
      check(
        'a save answers with the row, ready to put back',
        renamed.body?.path === listedTable?.path &&
          renamed.body?.hasTable === true &&
          renamed.body?.title === 'zz httpx validation renamed',
        JSON.stringify({ path: renamed.body?.path, hasTable: renamed.body?.hasTable })
      );
      check(
        'and says the rest of the tree is unaffected',
        renamed.body?.treeChanged === false,
        JSON.stringify(renamed.body?.treeChanged)
      );

      /* ----------------------------------------------------------- duplicating -- */
      /*
       * What was authored is copied; what happened is not. A duplicate carrying last run's output
       * would be a lie sitting in the tree waiting to be believed.
       */
      const copied = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${parsedStep.body._id}/duplicate`,
        {}
      );
      check(
        'a step can be duplicated',
        copied.status === 201 && String(copied.body?.title ?? '').endsWith('(copy)'),
        `${copied.status} ${JSON.stringify(copied.body?.title)}`
      );
      check(
        'the command comes with it and the output does not',
        String(copied.body?.command ?? '') === String(parsedStep.body?.command ?? '') &&
          !String(copied.body?.output ?? '').trim() &&
          !copied.body?.status,
        JSON.stringify({ command: copied.body?.command, output: (copied.body?.output ?? '').length })
      );
      check(
        'and it lands beside the original, not inside it',
        String(copied.body?.parent ?? '') === String(parsedStep.body?.parent ?? ''),
        JSON.stringify({ copy: copied.body?.parent, original: parsedStep.body?.parent })
      );
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${copied.body._id}`);

      /* --------------------------------------------------------- marked lines -- */
      /*
       * A note is anchored to a line number *and* the text of that line, and reading it back
       * reconciles the two. Everything below is about the case that makes this necessary: the sweep
       * is run again, and line 2 is no longer the host somebody wrote a sentence about.
       */
      const marking = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz marking',
        tool: 'httpx',
        output: ['zz-a.example [200]', 'zz-b.example [401]', 'zz-c.example [200]'].join('\n'),
      });
      const noted = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${marking.body._id}/notes`,
        { line: 2, text: 'basic auth, default creds' }
      );
      check(
        'a line of output can be marked',
        noted.status === 201 && noted.body?.notes?.length === 1,
        `${noted.status}`
      );
      check(
        'and the server captures the line itself rather than trusting the caller',
        noted.body.notes[0].snippet === 'zz-b.example [401]',
        JSON.stringify(noted.body.notes[0].snippet)
      );
      const dupLine = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${marking.body._id}/notes`,
        { line: 2 }
      );
      check('the same line cannot be marked twice', dupLine.status === 400, `got ${dupLine.status}`);
      const pastEnd = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${marking.body._id}/notes`,
        { line: 99 }
      );
      check('nor a line the output does not have', pastEnd.status === 400, `got ${pastEnd.status}`);

      /* The sweep again, with a host inserted above the marked one. */
      await call(alice, 'PUT', `/audits/${auditId}/enumeration/${marking.body._id}`, {
        output: ['zz-new.example [200]', 'zz-a.example [200]', 'zz-b.example [401]'].join('\n'),
      });
      const followed = await call(
        alice,
        'GET',
        `/audits/${auditId}/enumeration/${marking.body._id}`
      );
      check(
        'a note follows its line when the output shifts',
        followed.body.notes[0].line === 3 && followed.body.notes[0].moved === true,
        JSON.stringify({ line: followed.body.notes[0].line, moved: followed.body.notes[0].moved })
      );

      /* And now a sweep where that host is simply gone. */
      await call(alice, 'PUT', `/audits/${auditId}/enumeration/${marking.body._id}`, {
        output: 'zz-a.example [200]',
      });
      ...
      check(
        'and is flagged rather than dropped when the line is gone',
        orphaned.body.notes.length === 1 && orphaned.body.notes[0].stale === true,
        JSON.stringify(orphaned.body.notes[0])
      );
      const markRow = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === marking.body._id
      );
      check(
        'the tree row counts the marked lines, and the stale ones',
        markRow?.noteCount === 1 && markRow?.notesStale === 1,
        JSON.stringify({ count: markRow?.noteCount, stale: markRow?.notesStale })
      );

      /* Put the host back, and the note with it, then check the report prints it. */
      await call(alice, 'PUT', `/audits/${auditId}/enumeration/${marking.body._id}`, {
        output: ['zz-a.example [200]', 'zz-b.example [401]'].join('\n'),
      });
      const markData = await call(alice, 'GET', `/audits/${auditId}/report-data`);
      const markPayload = markData.body?.data ?? markData.body ?? {};
      const markStep = (markPayload.enumeration ?? []).find((x) => x.title === 'zz marking');
      check(
        'the report prints the marked line with its text',
        markStep?.hasNotes === true &&
          markStep.notes[0].snippet === 'zz-b.example [401]' &&
          markStep.notes[0].text === 'basic auth, default creds',
        JSON.stringify(markStep?.notes)
      );
      /*
       * The reason the snippet is carried at all: a pane capped at one line still has to deliver a
       * note about the second one, or the marking is worthless on exactly the sweeps worth marking.
       */
      await call(alice, 'PUT', `/audits/${auditId}/enumeration/${marking.body._id}`, {
        printOutput: 'head',
        printLines: 1,
      });
      const noteCapped = await call(alice, 'GET', `/audits/${auditId}/report-data`);
      const noteCappedStep = ((noteCapped.body?.data ?? noteCapped.body ?? {}).enumeration ?? []).find(
        (x) => x.title === 'zz marking'
      );
      check(
        'and prints it even when the pane is capped above that line',
      ...
      const unmarked = await call(
        alice,
        'DELETE',
        `/audits/${auditId}/enumeration/${marking.body._id}/notes/${noted.body.notes[0]._id}`
      );
      check('a mark can be taken off', unmarked.body?.notes?.length === 0, JSON.stringify(unmarked.body?.notes));
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${marking.body._id}`);

      /* ------------------------------------------------ where the text is kept -- */
      /*
       * The contract this rests on: an engagement document holds the *shape* of its enumeration and
       * none of the text.
       *
       * Not a preference. A step can carry 200KB of output, as much again of the run it replaced,
       * and an uncapped write-up — about 400KB against MongoDB's 16MB limit on one document, which
       * is a wall at roughly forty steps. Reaching it does not make a page slow; it makes the next
       * paste refuse to save, mid-operation, with nothing having warned anybody.
       */
      const storageStep = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz storage',
        tool: 'httpx',
        output: ['zz-1.example [200]', 'zz-2.example [200]', 'zz-3.example [200]'].join('\n'),
        content: '<p>Three answered.</p>',
      });
      const storageId = storageStep.body._id;

      const rawAudit = await Audit.collection.findOne({ _id: new mongoose.Types.ObjectId(auditId) });
      const rawStep = (rawAudit.enumeration ?? []).find((e) => String(e._id) === String(storageId));
      check(
        'the engagement document holds no output, write-up or previous run',
        !('output' in rawStep) && !('content' in rawStep) && !('previousOutput' in rawStep),
        JSON.stringify(Object.keys(rawStep).filter((k) => /output|content/i.test(k)))
      );
      check(
        'but it does hold what the tree draws',
        rawStep.outputLines === 3 && rawStep.tableRows === 3 && rawStep.hasContent === true,
        JSON.stringify({ lines: rawStep.outputLines, rows: rawStep.tableRows, content: rawStep.hasContent })
      );
      const storedBody = await EnumerationBody.findOne({ audit: auditId, step: storageId }).lean();
      check(
        'and the text is in its own document',
        String(storedBody?.output ?? '').split('\n').length === 3 &&
          String(storedBody?.content ?? '').includes('Three answered'),
        JSON.stringify({ lines: String(storedBody?.output ?? '').split('\n').length })
      );

      /* A title-only save must not disturb the text, which is now a different document. */
      await call(alice, 'PUT', `/audits/${auditId}/enumeration/${storageId}`, {
        title: 'zz storage renamed',
      });
      const afterRename = await EnumerationBody.findOne({ audit: auditId, step: storageId }).lean();
      check(
        'renaming a step leaves its output alone',
        String(afterRename?.output ?? '') === String(storedBody?.output ?? ''),
        `${String(afterRename?.output ?? '').length} chars`
      );
      check(
        'and does not invent a previous run',
        !String(afterRename?.previousOutput ?? '').trim(),
        JSON.stringify(String(afterRename?.previousOutput ?? '').slice(0, 40))
      );

      /* ---------------------------------------------------------- when it was run -- */
      /*
       * `outputAt` is the only honest basis for calling a step stale: `updatedAt` moves when anybody
       * fixes a typo, and `ranAt` is free text somebody typed.
       */
      const freshRow = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === storageId
      );
      check('a step run today is nought days old', freshRow?.outputAge === 0, `${freshRow?.outputAge}`);
      check('and is not flagged as stale', freshRow?.outputStale === false);
      check('nor claimed to be a re-run', freshRow?.reRun === false);

      const neverRun = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz never run',
      });
      const neverRow = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === neverRun.body._id
      );
      /* Never run is not the same as fresh, and a filter that treated it as either would be wrong. */
      check('a step never run has no age at all', neverRow?.outputAge === null, JSON.stringify(neverRow?.outputAge));

      await call(alice, 'PUT', `/audits/${auditId}/enumeration/${storageId}`, {
        output: 'zz-1.example [200]',
      });
      const reRunRow = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === storageId
      );
      check(
        're-running one says so, and the line count follows',
        reRunRow?.reRun === true && reRunRow?.outputLines === 1,
        JSON.stringify({ reRun: reRunRow?.reRun, lines: reRunRow?.outputLines })
      );

      /* ------------------------------------------- the enumeration spreadsheet -------- */
      /*
       * The same deliverable for the other half of the report. Checked as a workbook rather than as
       * a 200: a file Excel offers to repair is not an appendix, and the parts are the only way to
       * know the difference.
       */
      /*
       * A row marked internal, made here rather than borrowed, so the assertion below cannot be
       * satisfied by accident: this one is definitely held back and definitely uniquely named.
       */
      const heldFromSheet = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz-sheet-held-back',
        tool: 'nmap',
        output: 'nothing anybody should read',
        internal: true,
      });

      const enumSheetResponse = await fetch(`${base}/audits/${auditId}/enumeration.xlsx`, {
        headers: { Authorization: `Bearer ${alice.token}` },
      });
      const enumSheetBytes = Buffer.from(await enumSheetResponse.arrayBuffer());
      check(
        'the enumeration downloads as a spreadsheet too',
        enumSheetResponse.status === 200 && enumSheetBytes.subarray(0, 2).toString() === 'PK',
        `${enumSheetResponse.status}, ${enumSheetBytes.length} bytes`
      );
      check(
        'as a real .xlsx, named after the engagement',
        (enumSheetResponse.headers.get('content-type') ?? '').includes('spreadsheetml') &&
          /Enumeration/.test(enumSheetResponse.headers.get('content-disposition') ?? ''),
        enumSheetResponse.headers.get('content-disposition') ?? ''
      );

      const EnumPizZip = (await import('pizzip')).default;
      const enumZip = new EnumPizZip(enumSheetBytes);
      const enumParts = Object.keys(enumZip.files);
      check(
        'the workbook has the parts a reader needs',
        ['[Content_Types].xml', '_rels/.rels', 'xl/workbook.xml', 'xl/styles.xml'].every((p) =>
          enumParts.includes(p)
        ),
        `${enumParts.length} parts`
      );
      const enumSheetNames = [
        ...enumZip.file('xl/workbook.xml').asText().matchAll(/<sheet[^>]*name="([^"]+)"/g),
      ].map((m) => m[1]);
      check(
        'with a summary sheet and a row-per-step sheet',
        enumSheetNames.includes('Summary') && enumSheetNames.includes('Steps'),
        enumSheetNames.join(', ')
      );

      const enumSheetText = enumParts
        .filter((p) => p.startsWith('xl/worksheets/'))
        .map((p) => enumZip.file(p).asText())
        .join('');
      check(
        'the steps are in it',
        enumSheetText.includes('httpx'),
        'no step reached the sheet'
      );
      /*
       * The one thing a spreadsheet must never do: carry a row the report held back. It gets
       * forwarded on its own, away from the document that would have explained the omission — so
       * the summary says how many were left out instead.
       */
      check(
        'no held-back row is in it',
        !enumSheetText.includes('zz-sheet-held-back'),
        'internal rows must be omitted exactly as the report omits them'
      );
      check(
        'but the summary says one was left out',
        /marked internal are omitted/.test(enumSheetText),
        'a forwarded copy must not read as everything'
      );
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${heldFromSheet.body._id}`);

      /* ------------------------------------------ a write-up that is only a picture -- */
      /*
       * The ordinary case in an enumeration: paste a screenshot, write nothing round it.
       *
       * It printed as nothing at all. Templates guard the write-up with `{{#hasContent}}`, and
       * `hasContent` asked whether the field had any *text* — which an `<img>` does not. So the
       * document showed the command and then stopped, with the picture sitting in the database and
       * no error anywhere to say so. Worth a test precisely because nothing about it looks broken.
       */
      const pictureStep = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz picture only',
        tool: 'burp',
        content: '<img src="/api/media/000000000000000000000000" alt="">',
      });
      const pictureRow = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === pictureStep.body._id
      );
      check(
        'a write-up that is only a screenshot counts as a write-up',
        pictureRow?.hasContent === true,
        JSON.stringify({ hasContent: pictureRow?.hasContent })
      );

      const pictureData = await call(alice, 'GET', `/audits/${auditId}/report-data`);
      const pictureInReport = (
        (pictureData.body?.data ?? pictureData.body ?? {}).enumeration ?? []
      ).find((x) => x.title === 'zz picture only');
      check(
        'so the report will print it rather than skipping it',
        pictureInReport?.hasContent === true,
        JSON.stringify({ hasContent: pictureInReport?.hasContent })
      );

      /* And genuinely empty still means empty, or the guard would stop being worth anything. */
      const blankStep = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz nothing written',
        tool: 'burp',
        content: '<p></p>',
      });
      const blankRow = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === blankStep.body._id
      );
      check(
        'an empty write-up still reads as empty',
        blankRow?.hasContent === false,
        JSON.stringify({ hasContent: blankRow?.hasContent })
      );
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${pictureStep.body._id}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${blankStep.body._id}`);

      /* --------------------------------------------- and the text goes when it does -- */
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${storageId}`);
      check(
        'deleting a step deletes its text too',
        (await EnumerationBody.countDocuments({ audit: auditId, step: storageId })) === 0,
        'no orphaned body left behind'
      );
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${neverRun.body._id}`);

      /* ------------------------------------- internal, numbering, print policy -- */
      /*
       * A section held back must take its children with it. A branch whose steps still printed
       * would be the worst kind of surprise: nothing on the page would show anything was withheld.
       */
      const heldSection = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz Held back',
        internal: true,
      });
      const heldChild = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        parent: heldSection.body._id,
        title: 'zz child of a held section',
        tool: 'secret-tool',
        output: 'zz-secret-host.example',
      });
      check(
        'a step can be marked internal',
        heldSection.body?.internal === true,
        JSON.stringify(heldSection.body?.internal)
      );

      const heldData = await call(alice, 'GET', `/audits/${auditId}/report-data`);
      const heldPayload = heldData.body?.data ?? heldData.body ?? {};
      const titles = (heldPayload.enumeration ?? []).map((x) => x.title);
      check(
        'an internal section is not in the report',
        !titles.includes('zz Held back'),
        JSON.stringify(titles)
      );
      check(
        'and its children go with it',
        !titles.includes('zz child of a held section'),
        JSON.stringify(titles)
      );
      check(
        'while the tab still shows them',
        (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.some(
          (r) => r.title === 'zz child of a held section'
        ),
        'missing from the tab too'
      );
      check(
        'and the report can say how many were held back',
        heldPayload.enumerationSummary?.internal >= 2,
        JSON.stringify(heldPayload.enumerationSummary?.internal)
      );

      /* Numbering is hierarchical, and computed after the internal rows are dropped. */
      const numbers = (heldPayload.enumeration ?? []).map((x) => x.number);
      check(
        'the report numbers the tree hierarchically',
        numbers.some((x) => /^\d+\.\d+$/.test(String(x))),
        JSON.stringify(numbers)
      );
      check(
        'with no gap where a held-back row used to be',
        new Set(numbers).size === numbers.length && !numbers.includes(undefined),
        JSON.stringify(numbers)
      );

      /* The print policy, and the thing it must not do: cap the pane and print the whole table. */
      const bigOutput = Array.from({ length: 12 }, (_, i) => `https://h${i}.zz.example  [200]`).join('\n');
      const capped = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz capped',
        tool: 'httpx',
        output: bigOutput,
        printOutput: 'head',
      ...
      const cappedData = await call(alice, 'GET', `/audits/${auditId}/report-data`);
      const cappedStep = ((cappedData.body?.data ?? cappedData.body ?? {}).enumeration ?? []).find(
        (x) => x.title === 'zz capped'
      );
      check(
        'the print policy says what it cut',
        cappedStep?.printTruncated === true && cappedStep?.printOmitted === 8,
        JSON.stringify({ t: cappedStep?.printTruncated, o: cappedStep?.printOmitted })
      );
      check(
        'and it caps the table, not only the pane',
        cappedStep?.tableRowCount === 4,
        JSON.stringify(cappedStep?.tableRowCount)
      );
      const silent = await call(alice, 'PUT', `/audits/${auditId}/enumeration/${capped.body._id}`, {
        printOutput: 'none',
      });
      const silentData = await call(alice, 'GET', `/audits/${auditId}/report-data`);
      const silentStep = ((silentData.body?.data ?? silentData.body ?? {}).enumeration ?? []).find(
        (x) => x.title === 'zz capped'
      );
      check(
        'printing none leaves nothing for a template to print',
        silentStep?.hasOutput === false && silentStep?.hasTable === false,
        JSON.stringify({ o: silentStep?.hasOutput, t: silentStep?.hasTable })
      );
      check(
        'though the app still holds every line',
        String(silentStep?.outputFull ?? '').split('\n').length === 12,
        JSON.stringify(String(silentStep?.outputFull ?? '').split('\n').length)
      );

      /* ------------------------------------------------------------ bulk edit -- */
      const bulk = await call(alice, 'PUT', `/audits/${auditId}/enumeration/bulk`, {
        ids: [heldSection.body._id, heldChild.body._id, capped.body._id],
        patch: { phase: 'lateral', status: 'completed' },
      });
      check(
        'one patch reaches many steps',
        bulk.status === 200 && bulk.body?.changed === 3,
        `${bulk.status} changed ${bulk.body?.changed}`
      );
      check(
        'and it applied',
        (bulk.body?.steps ?? []).filter((r) => r.phase === 'lateral').length >= 3,
        JSON.stringify((bulk.body?.steps ?? []).map((r) => r.phase))
      );
      /* Only the flags: a bulk edit that could overwrite eleven write-ups is one mis-click away. */
      const bulkProse = await call(alice, 'PUT', `/audits/${auditId}/enumeration/bulk`, {
        ids: [capped.body._id],
        patch: { title: 'zz overwritten' },
      });
      check('a bulk edit cannot touch the prose', bulkProse.status === 422, `got ${bulkProse.status}`);

      /* --------------------------------------------------------- saved presets -- */
      const savedPreset = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${applied.body.section}/save-preset`,
        { name: 'zz Our subdomain routine', description: 'zz saved from a test' }
      );
      check(
        'a section can be saved as a preset',
        savedPreset.status === 201 && savedPreset.body?.steps >= 8,
        `${savedPreset.status} ${JSON.stringify(savedPreset.body)}`
      );
      const offeredNow = await call(alice, 'GET', `/audits/${auditId}/enumeration/presets`);
      const mine = (offeredNow.body ?? []).find((p) => p.label === 'zz Our subdomain routine');
      check('and is then offered alongside the built-ins', Boolean(mine?.custom), JSON.stringify(mine));

      const reapplied = await call(alice, 'POST', `/audits/${auditId}/enumeration/preset`, {
        preset: mine.key,
        target: 'zz-again.example',
      });
      check(
        'applying a saved preset rebuilds the branch',
        reapplied.status === 201 && reapplied.body?.added >= 8,
        `${reapplied.status} added ${reapplied.body?.added}`
      );
      const rebuilt = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.filter(
        (r) => String(r.parent) === String(reapplied.body.section)
      );
      /*
       * Structure and commands, never the output. Carrying one client's sweep into another
       * engagement is the single mistake a preset must not make easy.
       */
      check(
        'without carrying any output into it',
        rebuilt.length >= 8 && rebuilt.every((r) => !String(r.output ?? '').trim()),
        JSON.stringify(rebuilt.map((r) => (r.output ?? '').length))
      );
      const dupe = await call(alice, 'POST', `/audits/${auditId}/enumeration/${applied.body.section}/save-preset`, {
        name: 'zz Our subdomain routine',
      });
      check('two presets cannot share a name', dupe.status === 400, `got ${dupe.status}`);
      const removed = await call(
        alice,
        'DELETE',
        `/audits/${auditId}/enumeration/presets/${mine._id}`
      );
      check('a saved preset can be removed', removed.status === 200, `got ${removed.status}`);

      /* ------------------------------------------------------------- the export -- */
      const csv = await call(alice, 'GET', `/audits/${auditId}/enumeration/export?format=csv`);
      const csvText = typeof csv.body === 'string' ? csv.body : JSON.stringify(csv.body);
      check(
        'the enumeration exports as CSV',
        csv.status === 200 && csvText.includes('Number') && csvText.includes('Outcome'),
        csvText.slice(0, 80)
      );
      check(
        'and leaves the internal rows out unless asked',
        !csvText.includes('zz child of a held section'),
        'an internal row reached a default export'
      );
      const csvAll = await call(
        alice,
        'GET',
        `/audits/${auditId}/enumeration/export?format=csv&internal=include`
      );
      const csvAllText = typeof csvAll.body === 'string' ? csvAll.body : JSON.stringify(csvAll.body);
      check(
        'while an explicit request gets everything',
        csvAllText.includes('zz child of a held section'),
        'the internal row was missing from an export that asked for it'
      );
      const json = await call(alice, 'GET', `/audits/${auditId}/enumeration/export?format=json`);
      check(
        'and as JSON, numbered',
        json.status === 200 && Array.isArray(json.body?.steps) && json.body.steps[0]?.number === '1',
        JSON.stringify(json.body?.steps?.[0]?.number)
      );

      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${reapplied.body.section}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${heldSection.body._id}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${capped.body._id}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${applied.body.section}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${parsedStep.body._id}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${prose.body._id}`);

      /* ------------------------------------------------ variables in commands -- */
      const varsApplied = await call(alice, 'POST', `/audits/${auditId}/enumeration/preset`, {
        preset: 'ports',
        target: 'zz-vars.example',
      });
      check(
        'applying a preset defines TARGET when the caller knew it',
        varsApplied.status === 201,
        `${varsApplied.status}`
      );
      const definedVars = await call(alice, 'GET', `/audits/${auditId}/enumeration/vars`);
      const targetVar = (definedVars.body ?? []).find((v) => v.name === 'TARGET');
      check('and it is there', Boolean(targetVar), JSON.stringify(definedVars.body));
      /*
       * The value is whatever the *first* apply set, not this one. Never overwriting is deliberate —
       * somebody who has set a target meant it, and a preset is not the place to argue — so this
       * reads the value rather than assuming it.
       */
      check(
        'and an existing TARGET is not overwritten by a later preset',
        targetVar?.value === 'zz-preset.example',
        JSON.stringify(targetVar)
      );

      const varRows = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body;
      const nmapStep = varRows.find((r) => (r.title ?? '') === 'Host discovery');
      /*
       * The stored command keeps the variable and the resolved one is sent alongside. That split is
       * the whole feature: one edit to the variable has to change every command, which is only
       * possible if the authored text is what is stored.
       */
      check(
        'the stored command keeps the variable',
        String(nmapStep?.command ?? '').includes('$TARGET'),
        JSON.stringify(nmapStep?.command)
      );
      check(
        'and the resolved one is sent beside it',
        String(nmapStep?.commandResolved ?? '').includes(targetVar.value) &&
          !String(nmapStep?.commandResolved ?? '').includes('$TARGET'),
        JSON.stringify(nmapStep?.commandResolved)
      );

      const reVar = await call(alice, 'PUT', `/audits/${auditId}/enumeration/vars`, {
        vars: [{ name: 'TARGET', value: 'zz-changed.example' }],
      });
      check(
        'one edit changes every command that uses it',
        (reVar.body?.steps ?? [])
          .filter((r) => String(r.command ?? '').includes('$TARGET'))
          .every((r) => String(r.commandResolved).includes('zz-changed.example')),
        JSON.stringify((reVar.body?.steps ?? []).map((r) => r.commandResolved).filter(Boolean).slice(0, 2))
      );

      /* An undefined name stays visible: a command that lost a word would look finished and not be. */
      await call(alice, 'PUT', `/audits/${auditId}/enumeration/${nmapStep._id}`, {
        command: 'nmap -sV -iL $HOSTS -oA $OUTDIR/services',
      });
      const unresolved = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body.find(
        (r) => r._id === nmapStep._id
      );
      check(
        'an undefined variable is left as written',
        String(unresolved?.commandResolved ?? '').includes('$HOSTS'),
        JSON.stringify(unresolved?.commandResolved)
      );
      check(
        'and is reported so the editor can say so',
        (unresolved?.varsMissing ?? []).includes('HOSTS') &&
          (unresolved?.varsMissing ?? []).includes('OUTDIR'),
        JSON.stringify(unresolved?.varsMissing)
      );

      const badVar = await call(alice, 'PUT', `/audits/${auditId}/enumeration/vars`, {
        vars: [{ name: 'not a name', value: 'x' }],
      });
      check('a name that could never resolve is refused', badVar.status === 422, `got ${badVar.status}`);

      /* ------------------------------------------------------- step artefacts -- */
      const artefactForm = new FormData();
      artefactForm.append(
        'file',
        new Blob([Buffer.from('<?xml version="1.0"?><nmaprun/>')], { type: 'text/xml' }),
        'zz-services.xml'
      );
      const artefactUpload = await call(
        alice,
        'POST',
        `/audits/${auditId}/enumeration/${nmapStep._id}/documents`,
        artefactForm
      );
      check(
        'the machine-readable output files against the step',
        artefactUpload.status === 201 && artefactUpload.body?.filename === 'zz-services.xml',
        `${artefactUpload.status} ${JSON.stringify(artefactUpload.body)}`
      );
      const withArtefacts = (await call(alice, 'GET', `/audits/${auditId}/enumeration`)).body;
      check(
        'and is listed on that step and no other',
        withArtefacts.filter((r) => (r.documents ?? []).length).length === 1 &&
          withArtefacts.find((r) => r._id === nmapStep._id).documents[0].filename === 'zz-services.xml',
        JSON.stringify(withArtefacts.filter((r) => (r.documents ?? []).length).map((r) => r.title))
      );
      const artefactStranger = await call(await makeUser('artefacts', 'user'), 'GET', `/audits/${auditId}/enumeration`);
      check(
        'somebody off the engagement cannot read the tree at all',
        artefactStranger.status === 403 || artefactStranger.status === 404,
        `got ${artefactStranger.status}`
      );

      /* -------------------------------------------------------- chapter preview -- */
      const previewed = await call(alice, 'GET', `/audits/${auditId}/enumeration/preview`);
      check(
        'the chapter can be read before a document is generated',
        previewed.status === 200 && String(previewed.body?.html ?? '').includes('<h1>Enumeration</h1>'),
        `${previewed.status}`
      );
      /*
       * Built from the report's data, so a held-back row must be absent here as well. A preview that
       * showed it would be worse than no preview: it would give permission to stop checking.
       */
      const heldTitle = 'zz held back from the preview';
      const heldStep = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: heldTitle,
        internal: true,
      });
      const previewAgain = await call(alice, 'GET', `/audits/${auditId}/enumeration/preview`);
      check(
        'and an internal row is absent from it',
        !String(previewAgain.body?.html ?? '').includes(heldTitle) &&
          previewAgain.body?.internal >= 1,
        JSON.stringify({ internal: previewAgain.body?.internal })
      );
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${heldStep.body._id}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${varsApplied.body.section}`);
      await call(alice, 'PUT', `/audits/${auditId}/enumeration/vars`, { vars: [] });

      /* Put the engagement back as the scenarios after this one expect to find it. */
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${loose.body._id}`);
      for (const id of [promoted.body.finding._id, again.body.finding._id]) {
        await call(alice, 'DELETE', `/audits/${auditId}/findings/${id}`);
        await call(alice, 'DELETE', `/audits/${auditId}/findings/deleted/${id}`);
      }
      const cleanScope = (withScope.body?.scope ?? []).filter((g) => g.name !== 'zz Discovered');
      await call(alice, 'PUT', `/audits/${auditId}`, { scope: cleanScope, kind: 'standard' });
    }
    ...
    log.info('Search ranking');
    {
      ...
      {
        /*
         * The question the main search has never been able to answer: *where have I seen this
         * host before*. Output lives in its own collection, is never on the engagement, and is
         * therefore invisible to a search that reads engagements.
         */
        const step = await call(alice, 'POST', `/audits/${rankId}/enumeration`, {
          title: 'zz-rank httpx sweep',
          tool: 'httpx',
          target: 'acme.example',
        });
        const stepId = step.body?._id ?? step.body?.step?._id;
        await call(alice, 'PUT', `/audits/${rankId}/enumeration/${stepId}`, {
          output: [
            'https://www.acme.example      [200] [Acme] [nginx/1.24.0]',
            'https://zz-needle.acme.example [301] [] [Apache/2.4.29]',
            'https://mail.acme.example     [200] [Mail] [nginx/1.24.0]',
          ].join('\n'),
          content: '<p>The zz-needle host is the one worth noting.</p>',
        });

        check(
          'the main search cannot see tool output — it never could',
          !((await call(alice, 'GET', '/search?q=zz-needle')).body?.results ?? []).some(
            (row) => row.type === 'output'
          )
        );

        const hit = await call(alice, 'GET', '/search/output?q=zz-needle');
        check('the output search finds it', hit.status === 200 && hit.body?.total === 1, JSON.stringify(hit.body?.total));

        const row = (hit.body?.results ?? [])[0] ?? {};
        check('and says which step, in which engagement', row.title === 'zz-rank httpx sweep' && /zz-rank/.test(row.subtitle ?? ''), JSON.stringify([row.title, row.subtitle]));
        check(
          'with the matching line and its number, which is the whole answer',
          row.lines?.[0]?.line === 2 && /zz-needle/.test(row.lines?.[0]?.text ?? ''),
          JSON.stringify(row.lines)
        );
        check(
          'the write-up is searched too, and labelled apart from the output',
          /worth noting/.test(row.notes?.[0]?.text ?? ''),
          JSON.stringify(row.notes)
        );
        check(
          'the write-up is matched as prose, not as markup',
          !/<p>/.test(row.notes?.[0]?.text ?? ''),
          JSON.stringify(row.notes)
        );

        /* A banner is exactly the thing a text index would tokenise into uselessness. */
        const banner = await call(alice, 'GET', '/search/output?q=nginx/1.24.0');
        check('a banner with punctuation in it is found whole', banner.body?.total === 1, JSON.stringify(banner.body?.total));
        check(
          'and every line carrying it comes back, not just the first',
          (banner.body?.results?.[0]?.lines ?? []).length === 2,
          JSON.stringify(banner.body?.results?.[0]?.lines)
        );

        check(
          'a needle with regex metacharacters is taken literally',
          (await call(alice, 'GET', `/search/output?q=${encodeURIComponent('acme.example [200]')}`)).body
            ?.total === 0
        );
        check(
          'two characters is refused, because it would match every sweep ever pasted',
          (await call(alice, 'GET', '/search/output?q=ip')).status === 422
        );

        /*
         * Scope, from both sides.
         *
         * Bob is a collaborator here, so he sees it — which is the point of the feature. Somebody
         * with no part in the engagement sees nothing, which matters more than usual for this
         * endpoint: raw tool output is where a hash dump or a session cookie ends up, so a search
         * that leaked a line would leak the worst line.
         */
        check(
          'a collaborator on the engagement finds it too',
          ((await call(bob, 'GET', '/search/output?q=zz-needle')).body?.results ?? []).length === 1
        );

        const stranger = await makeUser('zz-outsider-output', 'user');
        check(
          'and somebody with no part in the engagement finds nothing',
          ((await call(stranger, 'GET', '/search/output?q=zz-needle')).body?.results ?? []).length === 0
        );
        const { User: OutUsers } = await import('../models/user.model.js');
        await OutUsers.deleteOne({ _id: stranger.user._id });
      }
    ...
    log.info('Scope changes');
    {
      ...
      /* ----------------------------------- the same facts, as a narrative ----- */
      /*
       * The table is the record; this is the thing that settles a dispute, because "you never
       * tested the payment sandbox" is answered by a sentence a client reads rather than a row
       * they have to interpret.
       */
      check(
        'the counts come written as well as counted',
        data.body?.scopeChangeSentence ===
          'One addition and one removal were agreed with the client while testing was under way.',
        data.body?.scopeChangeSentence
      );
      const timeline = String(data.body?.rich?.scopeTimeline ?? '');
      check(
        'the narrative names each change in the order it was agreed',
        timeline.indexOf('Payment flows excluded') < timeline.indexOf('staging API host'),
        timeline.slice(0, 200)
      );
      check(
        'with who agreed it and how',
        /Agreed with Dana Whitfield by Email\./.test(timeline),
        timeline
      );
      check(
        'and the hosts it touched',
        timeline.includes('api-staging.acme.example, 10.0.5.0/24'),
        timeline
      );
      check(
        'every sentence ends as a sentence, whatever the summary did',
        !/[^.]<\/p>/.test(timeline.replace(/<strong>[\s\S]*?<\/strong>/g, '')),
        timeline
      );
    ...
    log.info('Phishing campaigns');
    {
      ...
      /* -------------------------------------- the campaign, as a finding ------ */
      {
        /*
         * The point of the feature is that the finding and the tab cannot disagree, so the check
         * is not "a finding appeared" — it is that the numbers in it are the numbers the summary
         * computes from the same rows.
         */
        /*
         * Through the import, not the add-a-person route, because that is where timings come from:
         * a platform's export carries the timestamps and nobody types them in. Half the value of
         * the drafted finding is those timings, so the test has to get them in the way they arrive.
         */
        const exported = [
          { email: 'zz-a@phish.test', department: 'Finance', sent: 'yes', opened: 'yes', clicked: 'yes',
            submitted: 'yes', sentAt: '2029-05-04T09:00:00Z', clickedAt: '2029-05-04T09:06:00Z',
            submittedAt: '2029-05-04T09:12:00Z' },
          { email: 'zz-b@phish.test', department: 'Finance', sent: 'yes', opened: 'yes', clicked: 'yes',
            sentAt: '2029-05-04T09:00:00Z', clickedAt: '2029-05-04T09:40:00Z' },
          { email: 'zz-c@phish.test', department: 'Engineering', sent: 'yes', opened: 'yes',
            reported: 'yes', sentAt: '2029-05-04T09:00:00Z', reportedAt: '2029-05-04T09:20:00Z' },
          { email: 'zz-d@phish.test', department: 'Engineering', sent: 'yes' },
        ];
        const imported = await call(alice, 'POST', `/audits/${auditId}/phishing/import`, {
          json: JSON.stringify(exported),
        });
        check('the export is read', imported.status === 200, JSON.stringify(imported.body?.error));

        const drafted = await call(alice, 'POST', `/audits/${auditId}/phishing/finding`);
        check('a campaign can be drafted as a finding', drafted.status === 201, JSON.stringify(drafted.body?.error));

        const wrote = drafted.body?.finding ?? {};
        const rows = await PhishingTarget.find({ audit: auditId });
        const sum = campaignSummary(rows);

        check(
          'the title says what actually happened rather than something generic',
          wrote.title === 'Staff submitted credentials to a simulated phishing site',
          wrote.title
        );
        check(
          'and the numbers in it are the numbers the tab computes',
          String(wrote.description).includes(`${sum.phished} of ${sum.reached}`) &&
            String(wrote.description).includes(`${sum.phishedPercent}%`),
          `expected ${sum.phished} of ${sum.reached} at ${sum.phishedPercent}%`
        );
        check(
          'the timings are in it too, which is the half a table cannot show',
          /first click came/i.test(String(wrote.description)),
          String(wrote.description).slice(0, 200)
        );
        check(
          'the impact is arithmetic rather than opinion',
          /working credential set/i.test(String(wrote.observation)),
          String(wrote.observation)
        );
        check(
          'and it says whether the alarm was raised before the loss',
          /after the first credentials had already been submitted/i.test(String(wrote.observation)),
          String(wrote.observation)
        );
        check(
          'which is the comparison the summary makes, not a second opinion about it',
          sum.reportedBeforeFirstPhish === false,
          JSON.stringify({
            firstPhish: sum.firstPhishMinutes,
            firstReport: sum.firstReportMinutes,
          })
        );
        check(
          'no remediation is invented — that field is left for a person',
          !String(wrote.remediation ?? '').trim(),
          wrote.remediation
        );
        check(
          'and it is left unscored, so preflight asks somebody to score it',
          !wrote.severityOverride,
          wrote.severityOverride
        );

        /* Drafting twice is a double click, not a second campaign. */
        const again = await call(alice, 'POST', `/audits/${auditId}/phishing/finding`);
        check(
          'drafting it twice hands back the one that is already there',
          again.status === 409 && again.body?.details?.findingId === String(wrote._id),
          JSON.stringify(again.body)
        );

        await call(alice, 'DELETE', `/audits/${auditId}/findings/${wrote._id}`);
        await call(alice, 'DELETE', `/audits/${auditId}/phishing`);

        check(
          'and with no campaign there is nothing to draft',
          (await call(alice, 'POST', `/audits/${auditId}/phishing/finding`)).status === 400
        );
      }
      ...
      /*
       * A red team phishes too — it is the ordinary way in — and the red team report template has
       * always had a chapter for the campaign, guarded so it stays hidden when there is none. What
       * was missing was anywhere to record one: the tab was gated to the phishing kind alone. The
       * routes never were, which is what this checks — the tab was the whole of the restriction.
       */
      await Types15.create({ name: 'zz-phish Red Team', kind: 'redteam' });
      const redTeam = await call(alice, 'POST', '/audits', {
        name: 'zz-phish red team operation',
        auditType: 'zz-phish Red Team',
      });
      check(
        'a red team engagement is a red team engagement',
        redTeam.status === 201 && redTeam.body?.kind === 'redteam',
        JSON.stringify([redTeam.status, redTeam.body?.kind])
      );

      const onRedTeam = await call(alice, 'POST', `/audits/${redTeam.body._id}/phishing`, {
        targets: [{ email: 'zz-pretext@phish.test', department: 'Reception' }],
      });
      check('and it can hold a sending list', onRedTeam.status === 200 || onRedTeam.status === 201,
        JSON.stringify(onRedTeam.body?.error));
      check(
        'which the engagement then says it has, so the tab appears and stays',
        (await call(alice, 'GET', `/audits/${redTeam.body._id}`)).body?.phishingCount === 1
      );
      await call(alice, 'DELETE', `/audits/${redTeam.body._id}`);
    ...
    log.info('Pre-engagement questionnaire');
    {
      ...
      check(
        ...
        /* The note's text, which the shaped payload flattens into `searchText` rather than sends. */
        (audit.body?.notes ?? []).some((note) =>
          /payment sandbox/.test(note.searchText ?? note.content ?? '')
        ),
    ...
    log.info('Moving and copying a finding between engagements');
    {
      ...
      /* Whole: this asserts on the write-up, which the ordinary payload no longer carries. */
      const target = await call(alice, 'GET', `/audits/${far.body._id}?full=1`);
    ...
    log.info('Replacing a screenshot');
    {
      ...
      /*
       * Whole, on purpose. The ordinary payload no longer carries write-ups, and "the old id is
       * gone from every field" asserted against a payload with no fields in it would pass without
       * looking at anything.
       */
      const after = await call(alice, 'GET', `/audits/${auditId}?full=1`);
    ...
    log.info('Time logged');
    {
      ...
      const eighth = await call(bob, 'POST', '/time', { audit: auditId, day: '2025-09-08', hours: 4 });
      check('a second day is a second entry', eighth.status === 201, JSON.stringify(eighth.body?.error));

      const odd = await call(bob, 'POST', '/time', { audit: auditId, day: '2025-09-09', hours: 1.1 });
      check('hours that are not quarters are refused', odd.status === 422, `got ${odd.status}`);
      const impossible = await call(bob, 'POST', '/time', { audit: auditId, day: '2025-09-09', hours: 25 });
      ...
      /* ---------------------------------------------------- reading it back --- */
      const windowed = await call(bob, 'GET', '/time?from=2025-09-01&to=2025-09-08');
      check(
      ...
      await Plan.insertMany([
        { audit: auditId, user: bob.user._id, start: '2025-09-07', end: '2025-09-09' },
      ]);
      ...
      check(
        ...
        perAudit.body?.totals?.firstDay === '2025-09-07' &&
          perAudit.body?.totals?.lastDay === '2025-09-10',
        JSON.stringify(perAudit.body?.totals)
      );
      ...
      /*
       * And a day booked on an engagement that is then trashed.
       *
       * This page filtered its engagement list to live ones and its bookings not at all, so
       * trashing a job left its people looking booked for the rest of the window — under an
       * expanded row with no engagement name on it, because the engagement was not in the
       * list any more.
       */
      const doomed = await call(alice, 'POST', '/audits', {
        name: 'zz-hours Booked then trashed',
        reference: 'ZZ-HOURS',
        collaborators: [bob.user._id.toString()],
      });
      await Plan.insertMany([
        { audit: doomed.body._id, user: bob.user._id, start: recent(-10), end: recent(-10) },
      ]);
      const rowFor = async (name) => {
        const page = await call(alice, 'GET', '/users/engagements?days=30');
        return (page.body?.users ?? []).find((entry) => entry.username === name);
      };

      const whileLive = await rowFor(bob.user.username);
      check(
        'a day booked on a second engagement is another day of somebody\u2019s month',
        whileLive?.booked?.days === 4 && whileLive?.booked?.engagements === 2,
        JSON.stringify(whileLive?.booked)
      );

      await call(alice, 'DELETE', `/audits/${doomed.body._id}`);
      const afterTrash = await rowFor(bob.user.username);
      check(
        'and trashing that engagement gives the day back',
        afterTrash?.booked?.days === 3 && afterTrash?.booked?.engagements === 1,
        JSON.stringify(afterTrash?.booked)
      );

      const { purgeAuditData } = await import('../services/trash.service.js');
      await purgeAuditData([doomed.body._id]);
      check(
        'and emptying the trash takes the booking with it, rather than orphaning it',
        (await Plan.countDocuments({ audit: doomed.body._id })) === 0,
        `${await Plan.countDocuments({ audit: doomed.body._id })} left behind`
      );
      await Audit.deleteMany({ name: /^zz-hours/ });
      ...
      const zeroed = await call(bob, 'POST', '/time', { audit: auditId, day: '2025-09-08', hours: 0 });
    ...
    /**
     * A real 8×8 PNG through the real uploader.
     *
     * Media is content-addressed, so two images with the same bytes are one image — which is
     * correct, and would quietly turn a two-figure test into a one-figure one. The fill byte is
     * what keeps them distinct.
     */
    const uploadImage = async (who, audit, filename, fill) => {
      const { saveMedia: store } = await import('../services/media.service.js');
      const { encodePng } = await import('../utils/png.js');
      return store({
        buffer: encodePng(Buffer.alloc(8 * 8 * 4, fill), 8, 8),
    ...
    log.info('');
    log.info('A link the client can open');
    {
      /*
       * The only surface in this app that answers somebody with no account, so most of what is
       * checked here is what it *refuses* to do.
       */
      const secret = await call(alice, 'POST', `/audits/${auditId}/findings`, {
        title: 'zz Shared finding',
        cvssv3: 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H',
        /* A script and a handler, to prove the public page never renders either. */
        description:
          '<p>The id parameter is concatenated.</p><script>alert(1)</script>' +
          '<img src="/api/media/0123456789abcdef01234567" alt="shot" onerror="alert(2)">',
        poc: '<p>curl -H "Cookie: session=SUPERSECRET" https://target/x</p>',
        remediation: '<p>Use placeholders.</p>',
      });
      const sharedId = secret.body._id;

      const made = await call(alice, 'POST', `/share/link/${auditId}`, {
        label: 'zz Dana at Northwind',
        days: 14,
      });
      check('a link can be made', made.status === 201 && Boolean(made.body?.token), JSON.stringify(made.body?.error));
      check('and its URL comes back exactly once', made.body?.path?.startsWith('/shared/'), made.body?.path);

      const token = made.body.token;

      /* Nothing about the token is stored anywhere it could be read back. */
      const { ShareLink: Links } = await import('../models/share-link.model.js');
      const stored = await Links.findById(made.body._id).lean();
      check(
        'only a hash of it is kept',
        !JSON.stringify(stored).includes(token) && /^[a-f0-9]{64}$/.test(stored.tokenHash),
      ...
      check(
        ...
        (view.body?.findings ?? []).some((f) => f.title === 'zz Shared finding'),
        JSON.stringify((view.body?.findings ?? []).map((f) => f.title))
      );

      const shown = (view.body.findings ?? []).find((f) => f.title === 'zz Shared finding');
      check(
        'with what they need to act on it',
        shown.description.includes('concatenated') && shown.remediation.includes('placeholders'),
        JSON.stringify(shown)
      );
      check(
        'and never the proof of concept',
        !JSON.stringify(view.body).includes('SUPERSECRET') && shown.poc === undefined,
        'the proof of concept reached the client'
      );
      check(
        'nor any evidence, even by reference',
        !JSON.stringify(view.body).includes('/api/media/'),
        'an image reference reached the client'
      );
      check(
        'and markup that could run is taken out before it is shown',
        !shown.description.includes('<script') && !shown.description.includes('onerror'),
        shown.description
      );
      check(
      ...
      const marked = await call(null, 'POST', `/share/${token}/findings/${sharedId}`, { fixed: true });
      check('the client can mark one fixed', marked.status === 200, `${marked.status}`);
      check(
        'and the team sees it',
        ((await call(alice, 'GET', `/audits/${auditId}?full=1`)).body?.findings ?? []).some(
          (f) => String(f._id) === String(sharedId) && f.remediationStatus === 'fixed'
        )
      );
      check(
        'with a line in the log that does not claim a person did it',
        ((await call(alice, 'GET', `/audits/${auditId}/activity`)).body?.entries ?? []).some(
          (entry) => /The client \(zz Dana at Northwind\) marked/.test(entry.summary ?? '')
        ),
        'the activity line is missing or misattributed'
      );

      /*
       * The point of the whole feature: somebody finds out. Until this, a client acting on a
       * report changed a field and the team discovered it by opening the engagement on the
       * off-chance.
       */
      const { Notification: Notes } = await import('../models/notification.model.js');
      const raised = await Notes.find({
        audit: auditId,
        type: 'client-updated-finding',
      }).lean();
      check(
        'the team is told, without inventing somebody who did it',
        raised.length > 0 && raised.every((note) => note.actor === null),
        JSON.stringify(raised.map((note) => [note.message?.slice(0, 40), note.actor]))
      );
      check(
        'and the notification opens the finding rather than a list of forty',
        raised.every((note) => String(note.href).endsWith(`/findings/${sharedId}`)),
        JSON.stringify(raised.map((note) => note.href))
      );
      check(
        'everybody on the engagement hears about it',
        raised.some((note) => String(note.user) === String(alice.user._id)),
        JSON.stringify(raised.map((note) => String(note.user)))
      );

      /* And the claim is recorded beside the status, so a report can tell the two apart. */
      const claimed = (await call(alice, 'GET', `/audits/${auditId}/findings/${sharedId}`)).body;
      check(
        'the finding records that it was the client who said so',
        claimed?.clientClaim?.status === 'fixed' && claimed.clientClaim.by === 'zz Dana at Northwind',
        JSON.stringify(claimed?.clientClaim)
      );
      check(
        'which the client never sees back',
        !JSON.stringify(marked.body).includes('clientClaim'),
        'the claim leaked to the client view'
      );

      await call(alice, 'PUT', `/audits/${auditId}/findings/${sharedId}`, {
        remediationStatus: 'open',
      });
      const cleared = (await call(alice, 'GET', `/audits/${auditId}/findings/${sharedId}`)).body;
      check(
        'and a person moving the status makes it theirs rather than a claim',
        !cleared?.clientClaim?.status,
        JSON.stringify(cleared?.clientClaim)
      );

      check(
        'a bad token says nothing useful',
        (await call(null, 'GET', '/share/notarealtokenatallnotarealtoken')).status === 404
      );
      check(
        'and neither does a well-formed one that is not ours',
        (await call(null, 'GET', `/share/${'a'.repeat(43)}`)).status === 404
      );
      check(
        'a finding on another engagement cannot be reached through it',
        (await call(null, 'POST', `/share/${token}/findings/${'a'.repeat(24)}`, { fixed: true })).status === 404
      );
      check(
        'and the link cannot be used to read the engagement itself',
        (await call(null, 'GET', `/audits/${auditId}`)).status === 401,
        'an unauthenticated read of the engagement succeeded'
      );

      /* ------------------------------------------- the other kind of link --- */
      /*
       * A progress link, which is the one exception to "a link is made after the report goes" —
       * and only because it carries no finding text at all. What is checked here is exactly that:
       * the numbers are there, the writing is not, and nothing on it can be changed from outside.
       */
      const progress = await call(alice, 'POST', `/share/link/${auditId}`, {
        label: 'zz Dana, during the test',
        kind: 'status',
      });
      check(
        'a progress link can be made',
        progress.status === 201 && progress.body?.kind === 'status',
        `${progress.status} ${JSON.stringify(progress.body?.kind)}`
      );
      check(
        'and is read-only whatever was asked for',
        progress.body?.allowUpdates === false,
        JSON.stringify(progress.body?.allowUpdates)
      );

      const status = await call(null, 'GET', `/share/${progress.body.token}`);
      check(
        'it opens with no account, like the other one',
        status.status === 200 && status.body?.kind === 'status',
        `${status.status}`
      );
      check(
        'saying where the engagement has got to',
        Boolean(status.body?.stateLabel) && Boolean(status.body?.asOf),
        JSON.stringify([status.body?.stateLabel, status.body?.asOf])
      );
      check(
        'how much of the scope has been reached',
        typeof status.body?.coverage?.total === 'number' &&
          typeof status.body?.coverage?.tested === 'number',
        JSON.stringify(status.body?.coverage)
      );
      check(
        'and how many findings there are, by severity and nothing else',
        Array.isArray(status.body?.findings?.bySeverity) &&
          typeof status.body?.findings?.total === 'number',
        JSON.stringify(status.body?.findings)
      );

      /*
       * The check the whole feature turns on. Every finding on this engagement has a title, and
       * several have write-ups; none of it may appear on a page shown during the test.
       */
      const leaked = JSON.stringify(status.body);
      const titles = (await call(alice, 'GET', `/audits/${auditId}`)).body?.findings ?? [];
      check(
        'no finding title reaches a progress link',
        titles.every((finding) => !leaked.includes(finding.title)),
        titles.filter((finding) => leaked.includes(finding.title)).map((f) => f.title).join(', ')
      );
      check(
        'and no prose of any kind',
        !leaked.includes('<p>') && !leaked.includes('description'),
        leaked.slice(0, 200)
      );
      check(
        'nothing on it can be written to, and it says why rather than "read-only"',
        (await call(null, 'POST', `/share/${progress.body.token}/findings/${sharedId}`, {
          fixed: true,
        })).status === 403
      );

      /* Read-only links, and revocation. */
      const readOnly = await call(alice, 'POST', `/share/link/${auditId}`, {
        label: 'zz their auditor',
        allowUpdates: false,
      });
      check(
        'a read-only link refuses the write',
        (await call(null, 'POST', `/share/${readOnly.body.token}/findings/${sharedId}`, { fixed: false }))
          .status === 403
      );

      const listed = await call(alice, 'GET', `/share/link/${auditId}`);
      check(
        'the team can see its links, and how often they were opened',
        /* Three now: the findings link, the progress link, and the auditor's read-only one. */
        (listed.body?.links ?? []).length === 3 &&
          (listed.body.links ?? []).some((l) => l.views > 0),
        JSON.stringify((listed.body?.links ?? []).map((l) => [l.label, l.views]))
      );
      check(
        'and the list never carries the tokens',
        !JSON.stringify(listed.body).includes(token),
        'a token came back in the listing'
      );

      await call(alice, 'POST', `/share/link/${auditId}/${made.body._id}/revoke`);
      check(
        'a revoked link stops working immediately',
        (await call(null, 'GET', `/share/${token}`)).status === 404
      );
      check(
        'and bob cannot make one on an engagement he cannot see',
        (await call(bob, 'POST', `/share/link/${'a'.repeat(24)}`, { label: 'x' })).status === 404
      );

      await call(alice, 'DELETE', `/audits/${auditId}/findings/${sharedId}`);
      await Links.deleteMany({ audit: auditId });
    }

    /* ------------------------------------------------- numbered figures ----- */
    log.info('');
    log.info('Figures, numbered and pointed at');
    {
      /*
       * The number itself is worked out over the finished document and is covered by
       * `test:figures`, which can hold a whole `word/document.xml` to a fixed input. What is worth
       * checking against a real database is the half that involves a person: a sentence that points
       * at a picture, and what happens to it when the picture goes.
       */
      const shot = await uploadImage(alice, auditId, 'zz-figure-ref.png', 120);
      /* A reference carries the media id, which is the last segment of the stored URL. */
      const shotId = String(shot.url).split('/').pop();
      const withFigure = await call(alice, 'POST', `/audits/${auditId}/findings`, {
        title: 'zz Figure numbering',
        cvssv3: 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:L/I:N/A:N',
        description: `<p>The response is shown in <span data-figref="${shotId}">the response</span>.</p>`,
        poc: `<figure><img src="${shot.url}"><figcaption>The response</figcaption></figure>`,
      });
      const figureId = withFigure.body._id;

      /* ------------------------------------- the handling marking ---------- */
      /*
       * The classification has decided the retention window and the second factor since it was
       * added, and it stopped at the door: the document said nothing. That the shipped template now
       * prints it is proved in `npm run test:tags`, which renders the header; what is checked here
       * is the two things only the API can answer — that the flag reaches the report data at all,
       * and that preflight notices a restricted engagement whose template carries no marking.
       */
      {
        const standard = await call(alice, 'GET', `/audits/${auditId}/report-data`);
        check(
          'the report data carries the marking',
          standard.body?.classificationLabel === 'Standard' &&
            standard.body?.isRestricted === false &&
            standard.body?.classification === 'standard',
          JSON.stringify({
            label: standard.body?.classificationLabel,
            restricted: standard.body?.isRestricted,
          })
        );

        /* A restricted engagement of its own: changing this one's would disturb everything after. */
        const Users2 = (await import('../models/user.model.js')).User;
        await Users2.findByIdAndUpdate(alice.user._id, { $set: { totpEnabled: true } });
        const secret = await call(alice, 'POST', '/audits', {
          name: 'zz-marking restricted engagement',
          template: (await call(alice, 'GET', '/templates?purpose=report')).body?.[0]?._id,
        });
        await call(alice, 'PUT', `/audits/${secret.body._id}/classification`, {
          classification: 'restricted',
          note: 'zz checking the marking reaches the document',
        });

        const marked = await call(alice, 'GET', `/audits/${secret.body._id}/report-data`);
        check(
          'and says so on a restricted one',
          marked.body?.classificationLabel === 'Restricted' && marked.body?.isRestricted === true,
          JSON.stringify({
            label: marked.body?.classificationLabel,
            restricted: marked.body?.isRestricted,
          })
        );

        /*
         * And the nudge. The shipped template *does* print the marking, so the honest way to test
         * the check is a template that does not — which is also the case that matters, because it
         * is somebody's own template that will be missing it.
         */
        const { Template: Templates2 } = await import('../models/template.model.js');
        const bare = await Templates2.create({
          name: 'zz-marking template with no marking',
          kind: 'docx',
          purpose: 'report',
          filename: 'zz-nonexistent.docx',
          detectedTags: ['name', 'company.name', 'findings'],
        });
        await call(alice, 'PUT', `/audits/${secret.body._id}`, { template: String(bare._id) });

        const nagged = await call(alice, 'GET', `/audits/${secret.body._id}/preflight`);
        check(
          'preflight notices a restricted report whose template prints no marking',
          (nagged.body?.issues ?? []).some((issue) => issue.code === 'unmarked-restricted'),
          JSON.stringify((nagged.body?.issues ?? []).map((issue) => issue.code))
        );
        check(
          'as a warning rather than a refusal — a firm may mark its documents another way',
          (nagged.body?.issues ?? []).find((issue) => issue.code === 'unmarked-restricted')?.level ===
            'warning',
          JSON.stringify(
            (nagged.body?.issues ?? []).find((issue) => issue.code === 'unmarked-restricted')
          )
        );
        check(
          'and says which tag would fix it',
          /classificationLabel/.test(
            (nagged.body?.issues ?? []).find((issue) => issue.code === 'unmarked-restricted')?.detail ?? ''
          ),
          'the message does not say what to add'
        );

        /* A template that does carry it is left alone. */
        await Templates2.findByIdAndUpdate(bare._id, {
          $set: { detectedTags: ['name', '#isRestricted', 'company.name'] },
        });
        const quiet = await call(alice, 'GET', `/audits/${secret.body._id}/preflight`);
        check(
          'a template that prints the marking is not nagged — however the tag is written',
          !(quiet.body?.issues ?? []).some((issue) => issue.code === 'unmarked-restricted'),
          JSON.stringify((quiet.body?.issues ?? []).map((issue) => issue.code))
        );

        /* And a standard engagement is never asked about it. */
        check(
          'and a standard engagement is never asked about a marking',
          !((await call(alice, 'GET', `/audits/${auditId}/preflight`)).body?.issues ?? []).some(
            (issue) => issue.code === 'unmarked-restricted'
          ),
          'a standard engagement was nagged about a restricted marking'
        );

        await Templates2.findByIdAndDelete(bare._id);
        await call(alice, 'DELETE', `/audits/${secret.body._id}`);
        await Users2.findByIdAndUpdate(alice.user._id, { $set: { totpEnabled: false } });
      }

      const clean = await call(alice, 'GET', `/audits/${auditId}/preflight`);
      check(
        'a reference to a captioned picture is not an issue',
        !(clean.body?.issues ?? []).some((issue) => issue.code === 'dangling-figure-reference'),
        JSON.stringify((clean.body?.issues ?? []).filter((i) => i.code === 'dangling-figure-reference'))
      );

      const saved = await call(alice, 'GET', `/audits/${auditId}/findings/${figureId}`);
      check(
        'and the chip survives a round trip through the editor and the server',
        String(saved.body?.description ?? '').includes(`data-figref="${shotId}"`),
        saved.body?.description
      );

      /* The picture goes, the sentence that promised it stays. */
      const removed = await call(alice, 'PUT', `/audits/${auditId}/findings/${figureId}`, { poc: '<p>Gone.</p>' });
      check('the picture can be taken out', removed.status === 200, JSON.stringify(removed.body?.error ?? removed.status));
      const gone = await call(alice, 'GET', `/audits/${auditId}/findings/${figureId}`);
      check(
        'and the sentence that promised it is still there',
        String(gone.body?.description ?? '').includes(`data-figref="${shotId}"`) &&
          !String(gone.body?.poc ?? '').includes('img'),
        JSON.stringify({ poc: gone.body?.poc })
      );
      const after = await call(alice, 'GET', `/audits/${auditId}/preflight`);
      const dangling = (after.body?.issues ?? []).find(
        (issue) => issue.code === 'dangling-figure-reference'
      );
      check(
        'deleting the picture makes the reference an issue before anybody generates',
        Boolean(dangling) && dangling.level === 'warning',
        JSON.stringify((after.body?.issues ?? []).map((i) => i.code))
      );
      check(
        'and it says which words will break, so it can be found',
        /the response/i.test(dangling?.detail ?? ''),
        dangling?.detail
      );

      /* Putting it back uncaptioned is enough: every standalone picture is a numbered figure. */
      await call(alice, 'PUT', `/audits/${auditId}/findings/${figureId}`, {
        poc: `<p><img src="${shot.url}"></p>`,
      });
      const naked = (
        (await call(alice, 'GET', `/audits/${auditId}/preflight`)).body?.issues ?? []
      ).find((issue) => issue.code === 'dangling-figure-reference');
      check(
        'a screenshot nobody captioned still satisfies the reference',
        !naked,
        naked?.detail
      );

      /* And inside a sentence, because every picture in the report is numbered wherever it sits. */
      await call(alice, 'PUT', `/audits/${auditId}/findings/${figureId}`, {
        poc: `<p>Seen here <img src="${shot.url}"> in passing.</p>`,
      });
      const inline = (
        (await call(alice, 'GET', `/audits/${auditId}/preflight`)).body?.issues ?? []
      ).find((issue) => issue.code === 'dangling-figure-reference');
      check('a picture inside a sentence satisfies it too', !inline, inline?.detail);

      /* The two settings, saved like everything else. */
      const numbering = await call(alice, 'PUT', '/settings', {
        report: { public: { figureNumbering: false, figureLabel: 'Screenshot' } },
      });
      check(
        'numbering can be switched off, and the word changed',
        numbering.body?.report?.public?.figureNumbering === false &&
    ...
    /* --------------------------------------------------- the assistant ------ */
    log.info('');
    log.info('The optional assistant');
    {
      /*
       * Run against the `capture` wire, which sends nothing and keeps the prompt. That is the
       * only way to assert the thing that matters most here: not what came back, but what went.
       *
       * Everything below is either a refusal or an inspection of the prompt. There is no check
       * that the model wrote anything good, because that is not this app's to promise.
       */
      const { captured, setCaptureReply } = await import('../services/assistant/wire.js');
      const wireBefore = process.env.ASSISTANT_WIRE;
      const vaultBefore = process.env.VAULT_KEY;
      const cryptoA = await import('node:crypto');
      process.env.ASSISTANT_WIRE = 'capture';
      process.env.VAULT_KEY = cryptoA.default.randomBytes(32).toString('hex');

      /*
       * The whole document, kept so it can be put back — the same discipline the settings block
       * above uses, and for a better reason than symmetry.
       *
       * Settings is a singleton. Every other block here creates `zz-` documents and deletes them,
       * but this one has to write to the instance's one and only configuration, and the suite runs
       * against a real database with a real assistant configured in it. Restoring the two fields
       * this block obviously changes is not enough: it leaves behind whatever else it wrote, an
       * administrator's next save persists it, and the first anybody knows is a job that has
       * silently stopped appearing. That is not hypothetical — it is exactly what happened.
       */
      const SettingsDoc = (await import('../models/settings.model.js')).Settings;
      const settingsSnapshot = (await SettingsDoc.getSettings()).toObject();
      delete settingsSnapshot._id;
      delete settingsSnapshot.__v;

      /*
       * Put back to the state a fresh instance is in, rather than assuming it.
       *
       * These two checks are about what an unconfigured instance does, and the settings singleton
       * outlives a run — so asserting on whatever the last one left behind is a test that passes
       * or fails depending on the order it ran in. It has already done both.
       */
      await call(alice, 'PUT', '/settings', { assistant: { enabled: false, key: '' } });
      check(
        'an instance that has not configured one says so, and offers nothing',
        (await call(alice, 'GET', '/assistant')).body?.available === false
      );
      check(
        'and a job on it is refused rather than half-attempted',
        (await call(alice, 'POST', '/assistant/summary', { auditId })).status === 400
      );

      /* Configured, the way an administrator would. */
      const saved = await call(alice, 'PUT', '/settings', {
        assistant: {
          enabled: true,
          provider: 'custom',
          wire: 'openai',
          ...
          model: 'zz-test-model',
          key: 'zz-not-a-real-key',
          jobs: { summary: true, rewrite: true, enumeration: true, library: false },
        },
      });
      check(
        'the key is stored and never handed back',
        saved.body?.assistant?.hasKey === true && !JSON.stringify(saved.body.assistant).includes('zz-not-a-real-key'),
        JSON.stringify(saved.body?.assistant)
      );
      check(
        'and neither is the ciphertext',
        !('secret' in (saved.body?.assistant ?? {})),
        JSON.stringify(Object.keys(saved.body?.assistant ?? {}))
      );

      const seen = await call(alice, 'GET', '/assistant');
      check('now it is available', seen.body?.available === true, JSON.stringify(seen.body));
      check(
        'a job switched off is reported as off, so nothing draws a dead button',
        seen.body?.jobs?.library === false && seen.body?.jobs?.summary === true,
        JSON.stringify(seen.body?.jobs)
      );
      check(
        'and the endpoint is nobody else\u2019s business',
        !JSON.stringify(seen.body).includes('127.0.0.1'),
        JSON.stringify(seen.body)
      );

      /* A finding with a secret in every place a secret ends up. */
      const risky = await call(alice, 'POST', `/audits/${auditId}/findings`, {
        title: 'zz Assistant SMB credentials',
        cvssv3: 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:N/A:N',
        description: '<p>The service account answered with --password=zzTopSecret42 from the DMZ.</p>',
        remediation: '<p>Rotate it.</p>',
        poc: '<p>smbclient //10.0.0.5/share -U svc%zzProofOnlyPassword</p>',
      });
      const riskyId = risky.body._id;

      setCaptureReply('Rotate the service account and stop reusing it across the estate.');
      const before = captured.length;
      const rewritten = await call(alice, 'POST', '/assistant/rewrite', {
        auditId,
        findingId: riskyId,
        field: 'description',
      });
      check('a rewrite comes back', rewritten.status === 200 && Boolean(rewritten.body?.html), JSON.stringify(rewritten.body));
      check(
        'as escaped HTML we built, never markup the provider sent',
        rewritten.body.html.startsWith('<p>') && !rewritten.body.html.includes('<script'),
        rewritten.body.html
      );
      check('and one prompt was sent, not two', captured.length === before + 1, `${captured.length - before}`);

      const prompt = JSON.stringify(captured.at(-1));
      check(
        'the proof of concept never left the machine',
        !prompt.includes('zzProofOnlyPassword'),
        'the proof of concept was in the prompt'
      );
      check(
        'nor did the password in the description',
        !prompt.includes('zzTopSecret42'),
        'a redacted password was in the prompt'
      );
      check('but the finding itself was described', prompt.includes('zz Assistant SMB credentials'));
      check(
        'and the count of what was removed comes back with the answer',
        rewritten.body.redacted === 1,
        String(rewritten.body.redacted)
      );

      check(
        'the proof of concept is not a field a rewrite can even ask for',
        (await call(alice, 'POST', '/assistant/rewrite', { auditId, findingId: riskyId, field: 'poc' })).status === 422
      );
      check(
        'a job switched off is refused',
        (await call(alice, 'POST', '/assistant/library', { auditId, findingId: riskyId })).status === 400
      );
      check(
        'and somebody who cannot see the engagement gets nothing',
        (await call(bob, 'POST', '/assistant/summary', { auditId: `${'a'.repeat(24)}` })).status === 404
      );

      /* A rewrite of a passage with a screenshot in it is refused rather than silently losing it. */
      await call(alice, 'PUT', `/audits/${auditId}/findings/${riskyId}`, {
        remediation: '<p>Rotate it.</p><img src="/api/media/0123456789abcdef01234567" alt="proof">',
      });
      const withPicture = await call(alice, 'POST', '/assistant/rewrite', {
        auditId,
        findingId: riskyId,
        field: 'remediation',
      });
      check(
        'a passage with a screenshot in it is refused, not flattened',
        withPicture.status === 400 && /screenshot/i.test(withPicture.body?.error ?? ''),
        JSON.stringify(withPicture.body)
      );

      /* Nothing here writes. The engagement must be exactly as it was. */
      const after = await call(alice, 'GET', `/audits/${auditId}/findings/${riskyId}`);
      check(
        'and none of it wrote anything to the finding',
        after.body?.description === '<p>The service account answered with --password=zzTopSecret42 from the DMZ.</p>',
        after.body?.description
      );

      /* --------------------------------------- restricted work stays here ---- */
      const Users = (await import('../models/user.model.js')).User;
      await Users.findByIdAndUpdate(alice.user._id, { $set: { totpEnabled: true } });
      const secretJob = await call(alice, 'POST', '/audits', { name: 'zz-assistant restricted' });
      await call(alice, 'PUT', `/audits/${secretJob.body._id}/classification`, {
        classification: 'restricted',
        note: 'zz the assistant must not see this one',
      });
      await call(alice, 'POST', `/audits/${secretJob.body._id}/findings`, {
        title: 'zz restricted finding',
        cvssv3: 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:N/A:N',
        description: '<p>Something.</p>',
      });

      const refused = await call(alice, 'POST', '/assistant/summary', { auditId: secretJob.body._id });
      check(
        'a restricted engagement is refused even with the assistant on',
        refused.status === 403 && /restricted/i.test(refused.body?.error ?? ''),
        JSON.stringify(refused.body)
      );

      await call(alice, 'PUT', '/settings', { assistant: { allowRestricted: true } });
      check(
        'and goes only when an administrator has said so separately',
        (await call(alice, 'POST', '/assistant/summary', { auditId: secretJob.body._id })).status === 200
      );

      await call(alice, 'DELETE', `/audits/${secretJob.body._id}`);
      await Users.findByIdAndUpdate(alice.user._id, { $set: { totpEnabled: false } });
      await call(alice, 'DELETE', `/audits/${auditId}/findings/${riskyId}`);

      /*
       * Byte for byte, including the encrypted key, which a `PUT` could not put back: the route
       * takes a plaintext key it does not have and this block ran under a throwaway `VAULT_KEY`.
       */
      await SettingsDoc.replaceOne({ singleton: 'settings' }, settingsSnapshot, { upsert: true });
      const restored = await SettingsDoc.getSettings();
      check(
        'and the instance is left configured exactly as it was found',
        JSON.stringify(restored.assistant?.jobs ?? {}) ===
          JSON.stringify(settingsSnapshot.assistant?.jobs ?? {}) &&
          Boolean(restored.assistant?.enabled) === Boolean(settingsSnapshot.assistant?.enabled) &&
          (restored.assistant?.model ?? '') === (settingsSnapshot.assistant?.model ?? '') &&
          (restored.assistant?.secret?.data ?? '') === (settingsSnapshot.assistant?.secret?.data ?? ''),
        JSON.stringify(restored.assistant?.jobs)
      );

      if (wireBefore === undefined) delete process.env.ASSISTANT_WIRE;
      else process.env.ASSISTANT_WIRE = wireBefore;
      if (vaultBefore === undefined) delete process.env.VAULT_KEY;
      else process.env.VAULT_KEY = vaultBefore;
    }

    /* ------------------------------------------- what an engagement weighs --- */
    log.info('');
    log.info('What the engagement payload carries');
    {
      /*
       * The point of the change, measured rather than asserted: forty findings with a page of
       * prose each is an ordinary engagement, and it used to arrive whole every time somebody
       * switched tab.
       */
      const PROSE = `<p>${'The parameter is concatenated into the query without escaping. '.repeat(40)}</p>`;
      const made = [];
      ...
      const light = await call(alice, 'GET', `/audits/${auditId}`);
      const heavy = await call(alice, 'GET', `/audits/${auditId}?full=1`);
      const size = (body) => Buffer.byteLength(JSON.stringify(body));

      check(
        'the ordinary payload is a fraction of the whole document',
        /*
         * Not a tenth of it, and deliberately: a screenful of plain text per finding stays behind
         * so the in-engagement search still works without the markup. That is the trade, and the
         * threshold is set where the trade puts it rather than where a rounder number would.
         */
        size(light.body) * 2.5 < size(heavy.body),
        `${Math.round(size(light.body) / 1024)} KB against ${Math.round(size(heavy.body) / 1024)} KB`
      );
      log.info(
        `        ${Math.round(size(heavy.body) / 1024)} KB whole, ` +
          `${Math.round(size(light.body) / 1024)} KB shaped ` +
          `(${Math.round((1 - size(light.body) / size(heavy.body)) * 100)}% less)`
      );

      const row = (light.body.findings ?? []).find((f) => f.title === 'zz Weight 0');
      check('a finding arrives without its prose', row && row.description === undefined, JSON.stringify(Object.keys(row ?? {}).slice(0, 6)));
      check('but says it has some', row?.hasDescription === true && row?.hasPoc === true);
      check(
        'and carries enough to draw a row',
        typeof row?.snippet === 'string' && row.snippet.length > 0 && row.snippet.length <= 201,
        `${row?.snippet?.length} characters`
      );
      check(
        'and enough to be searched',
        typeof row?.searchText === 'string' && row.searchText.includes('concatenated'),
        `${row?.searchText?.length} characters`
      );
      check(
        'the enumeration tree is not in it at all',
        light.body.enumeration === undefined && typeof light.body.enumerationCount === 'number',
        JSON.stringify(light.body.enumerationCount)
      );
      /*
       * And the count that replaced it is the real one. Asserting only that it is a number let a
       * wrong answer through: the tab bar draws its badge from this, and when it was still reading
       * the array the badge became a silent zero and disappeared. A count nobody checks is a count
       * that can be quietly wrong.
       */
      const steps = await call(alice, 'GET', `/audits/${auditId}/enumeration`);
      check(
        'and the count that replaced it is the real number of steps',
        light.body.enumerationCount === (steps.body ?? []).length,
        `${light.body.enumerationCount} against ${(steps.body ?? []).length}`
      );
      check('while ?full=1 still answers with everything', typeof heavy.body.findings?.[0]?.description === 'string');

      const one = await call(alice, 'GET', `/audits/${auditId}/findings/${made[0]}`);
      check(
        'and one finding can be fetched in full',
        one.status === 200 && String(one.body?.description ?? '').includes('concatenated'),
        `${one.status}`
      );

      /*
       * What the Retest tab reads, and why it is checked here of all places.
       *
       * That tab is drawn entirely from the shaped payload — no endpoint of its own — so the three
       * fields it needs have to survive the slimming that this block exists to measure. A future
       * saving that dropped `statusHistory` would cost a few kilobytes and silently empty the one
       * page that answers "who checked this, and when".
       */
      const forRetest = light.body.findings?.[0] ?? {};
      check(
        'the payload still carries what a retest is run from',
        typeof forRetest.remediationStatus === 'string' &&
          'clientClaim' in forRetest &&
          Array.isArray(forRetest.statusHistory),
        JSON.stringify(Object.keys(forRetest).filter((k) => /status|claim/i.test(k)))
      );

      {
        /*
         * And that a status move comes back attributed. "We retested it" is only a fact if it says
         * who — the whole reason `statusHistory` records `by` — and an id in the payload would
         * leave the tab saying "moved 2 hours ago" by nobody.
         */
        await call(alice, 'PUT', `/audits/${auditId}/findings/${made[0]}`, {
          remediationStatus: 'retesting',
        });
        const moved = (await call(alice, 'GET', `/audits/${auditId}`)).body.findings.find(
          (row) => String(row._id) === String(made[0])
        );
        const last = (moved?.statusHistory ?? []).at(-1);
        check(
          'and says who moved it, as a person rather than an id',
          last?.status === 'retesting' && Boolean(last?.by?.username || last?.by?.firstname),
          JSON.stringify(last)
        );
      }

      /* The literal routes under /findings must still answer, not be swallowed by :findingId. */
      check(
        'the deleted list still answers',
        (await call(alice, 'GET', `/audits/${auditId}/findings/deleted`)).status === 200
      );
      check(
        'and so does the similar-title lookup',
        (await call(alice, 'GET', `/audits/${auditId}/findings/similar?title=zz%20Weight%200`)).status === 200
      );

      for (const id of made) await call(alice, 'DELETE', `/audits/${auditId}/findings/${id}`);
    }

    /* ------------------------------------------- questions for the client --- */
    log.info('');
    log.info('Questions, and what was assumed');
    {
      const asked = await call(alice, 'POST', `/audits/${auditId}/questions`, {
        text: 'zz Is the staging host in scope?',
        context: 'staging.acme.invalid',
        askedOf: 'Dana',
      });
      check('a question can be noted', asked.status === 201 && asked.body?.status === 'open');
      check(
        'and an unanswered one does not print by default',
        asked.body?.print === false,
        JSON.stringify(asked.body?.print)
      );

      const settled = await call(alice, 'PUT', `/audits/${auditId}/questions/${asked.body._id}`, {
        status: 'assumed',
        answer: 'Treated as in scope; it serves the same certificate as production.',
      });
      check('settling it stamps who and when', Boolean(settled.body?.answeredAt) && Boolean(settled.body?.answeredBy));
      check('and it reads as an assumption', settled.body?.status === 'assumed');

      const reopened = await call(alice, 'PUT', `/audits/${auditId}/questions/${asked.body._id}`, {
        status: 'open',
      });
      check(
        'reopening takes the name back off it',
        reopened.body?.answeredAt === null && reopened.body?.answeredBy === null,
        JSON.stringify([reopened.body?.answeredAt, reopened.body?.answeredBy])
      );

      await call(alice, 'PUT', `/audits/${auditId}/questions/${asked.body._id}`, {
        status: 'assumed',
        print: true,
      });

      /* The whole point: it reaches the report as a caveat. */
      const { buildReportData: buildIt } = await import('../services/report.service.js');
      const fresh = await Audit.findById(auditId);
      const data = buildIt(fresh, null, { parts: null, numbering: null }, { target: 'html' });
      check(
        'it reaches the report as an assumption',
        (data.assumptions ?? []).some((entry) => entry.text.includes('staging host')),
        JSON.stringify((data.assumptions ?? []).map((a) => a.text))
      );
      check(
        'with the guard a template needs',
        data.hasAssumptions === true && data.openQuestions === 0,
        JSON.stringify([data.hasAssumptions, data.openQuestions])
      );

      const removed = await call(alice, 'DELETE', `/audits/${auditId}/questions/${asked.body._id}`);
      check('and deleting one offers it back', Boolean(removed.body?.undo?.id));
      await call(alice, 'POST', `/audits/${auditId}/undo/${removed.body.undo.id}`);
      const back = await call(alice, 'GET', `/audits/${auditId}/questions`);
      check(
        'which puts it back where it was',
        (back.body?.questions ?? []).some((q) => String(q._id) === String(asked.body._id))
      );
      await call(alice, 'DELETE', `/audits/${auditId}/questions/${asked.body._id}`);
    }

    /* ------------------------------------------------------------ scratchpad --- */
    log.info('');
    log.info('A scratchpad that belongs to nobody but you');
    {
      const mine = await call(alice, 'POST', '/scratch', {
        title: 'zz SSO quirk',
        content: '<p>Their IdP drops the RelayState on a POST binding.</p>',
      });
      check('a note can be written', mine.status === 201 && Boolean(mine.body?._id));

      const listed = await call(alice, 'GET', '/scratch');
      check(
        'and comes back in my own list',
        (listed.body?.notes ?? []).some((note) => note._id === mine.body._id)
      );

      /*
       * The rule the whole feature rests on. A scratchpad somebody else can read is a scratchpad
       * nobody writes honestly in, so this is asserted rather than assumed — and asserted for an
       * admin, because "except for admins" is the exception that would quietly get added.
       */
      const bobsList = await call(bob, 'GET', '/scratch');
      check(
        'nobody else can see it, not even by asking nicely',
        !(bobsList.body?.notes ?? []).some((note) => note._id === mine.body._id),
        `${(bobsList.body?.notes ?? []).length} notes`
      );
      const stolen = await call(bob, 'GET', `/scratch`);
      check(
        'and reading it directly is a not-found rather than a refusal',
        (await call(bob, 'PUT', `/scratch/${mine.body._id}`, { title: 'zz taken' })).status === 404,
        JSON.stringify(stolen.status)
      );

      const edited = await call(alice, 'PUT', `/scratch/${mine.body._id}`, { pinned: true });
      check('pinning it works', edited.body?.pinned === true);

      /* Moving one onto an engagement is what makes it public. */
      const moved = await call(alice, 'POST', `/scratch/${mine.body._id}/move`, {
        audit: auditId,
        keep: true,
      });
      check(
        'it can be copied onto an engagement',
        moved.status === 201 && Boolean(moved.body?.note?._id),
        JSON.stringify(moved.body?.error ?? moved.status)
      );

      const notes = await call(alice, 'GET', `/audits/${auditId}/notes`);
      check(
        'where it arrives as an ordinary note, with its text',
        (notes.body ?? []).some(
          (note) => note.title === 'zz SSO quirk' && note.content.includes('RelayState')
        )
      );
      check(
        'and bob, who is on the engagement, can now read it',
        ((await call(bob, 'GET', `/audits/${auditId}/notes`)).body ?? []).some(
          (note) => note.title === 'zz SSO quirk'
        )
      );

      const afterCopy = await call(alice, 'GET', '/scratch');
      check(
        'a copy leaves the original where it was',
        (afterCopy.body?.notes ?? []).some((note) => note._id === mine.body._id)
      );

      const second = await call(alice, 'POST', '/scratch', { title: 'zz one-way', content: '<p>x</p>' });
      await call(alice, 'POST', `/scratch/${second.body._id}/move`, { audit: auditId, keep: false });
      const afterMove = await call(alice, 'GET', '/scratch');
      check(
        'and a move does not',
        !(afterMove.body?.notes ?? []).some((note) => note._id === second.body._id)
      );

      await call(alice, 'DELETE', `/scratch/${mine.body._id}`);
      check(
        'deleting it removes it',
        !((await call(alice, 'GET', '/scratch')).body?.notes ?? []).some(
          (note) => note._id === mine.body._id
        )
      );
    }
    ...
    /* ----------------------------------------------------------- who is where --- */
    log.info('');
    log.info('Where everybody is');
    {
      await call(alice, 'POST', '/presence/heartbeat', {
        activity: 'writing up zz',
        location: `engagement:${auditId}:findings`,
      });
      const board = await call(alice, 'GET', '/presence/board');
      check('the board answers', board.status === 200 && Array.isArray(board.body?.here));

      const me = (board.body?.here ?? []).find((entry) => entry.isSelf);
      check('it knows where I am', me?.engagement?._id === String(auditId), JSON.stringify(me?.engagement));
      check('and which part of it', me?.where === 'findings', me?.where);
      check(
        'an engagement somebody is in is not called quiet',
        !(board.body?.quiet ?? []).some((audit) => String(audit._id) === String(auditId)),
        JSON.stringify((board.body?.quiet ?? []).map((a) => a.name))
      );
      check(
        'and the quiet list counts whole days, oldest first',
        (board.body?.quiet ?? []).every(
          (audit, index, all) =>
            Number.isInteger(audit.untouchedDays) &&
            (index === 0 || all[index - 1].untouchedDays >= audit.untouchedDays)
        )
      );
      await call(alice, 'POST', '/presence/heartbeat', { activity: '', location: '' });
    }

    /* ------------------------------------------------------------------ undo --- */
    log.info('');
    log.info('Putting back what was just deleted');
    {
      /*
       * The enumeration step is the one worth proving: it is a branch, its text lives in another
       * collection, and until now a mis-click took all of it permanently.
       */
      const section = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz Undo section',
      });
      const child = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        parent: section.body._id,
        title: 'zz undo child',
        tool: 'nmap',
      ...
      const gone = await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${section.body._id}`);
      check(
        'deleting a branch offers a way back',
        gone.status === 200 && gone.body?.removed === 2 && Boolean(gone.body?.undo?.id),
        JSON.stringify({ removed: gone.body?.removed, undo: gone.body?.undo })
      );
      check(
        'and says what it was, so the toast can name it',
        gone.body?.undo?.noun === 'Enumeration step' && gone.body?.undo?.label === 'zz Undo section',
        JSON.stringify(gone.body?.undo)
      );

      const emptied = await call(alice, 'GET', `/audits/${auditId}/enumeration`);
      check(
        'both steps are really gone first',
        !(emptied.body ?? []).some((row) => String(row._id) === String(section.body._id)),
        `${(emptied.body ?? []).length} rows`
      );

      const back = await call(alice, 'POST', `/audits/${auditId}/undo/${gone.body.undo.id}`);
      check('the undo is accepted', back.status === 200, JSON.stringify(back.body?.error));

      const restored = await call(alice, 'GET', `/audits/${auditId}/enumeration`);
      const returned = (restored.body ?? []).filter((row) =>
        [String(section.body._id), String(child.body._id)].includes(String(row._id))
      );
      check('the whole branch comes back', returned.length === 2, `${returned.length} of 2`);
      check(
        'under the ids it had, so anything pointing at it still does',
        returned.some((row) => String(row._id) === String(child.body._id))
      );
      check(
        'and the child is still nested under its section',
        String(returned.find((row) => String(row._id) === String(child.body._id))?.parent) ===
          String(section.body._id),
        JSON.stringify(returned.map((row) => [row.title, String(row.parent ?? '')]))
      );

      const text = await call(
        alice,
        'GET',
        `/audits/${auditId}/enumeration/${child.body._id}`
      );
      check(
        'the output that lived in another collection came back with it',
        String(text.body?.output ?? '').includes('22/tcp open'),
        JSON.stringify(String(text.body?.output ?? '').slice(0, 40))
      );

      const twice = await call(alice, 'POST', `/audits/${auditId}/undo/${gone.body.undo.id}`);
      check(
        'an undo cannot be taken twice',
        twice.status >= 400,
        `${twice.status} ${twice.body?.error}`
      );
      check(
        'and a token nobody issued is refused',
        (await call(alice, 'POST', `/audits/${auditId}/undo/${section.body._id}`)).status >= 400
      );

      /* A note, to prove the same mechanism carries something that is not a tree. */
      const note = await call(alice, 'POST', `/audits/${auditId}/notes`, {
        title: 'zz undo note',
        content: '<p>Worth keeping.</p>',
      });
      const removedNote = await call(alice, 'DELETE', `/audits/${auditId}/notes/${note.body._id}`);
      check('a note offers one too', Boolean(removedNote.body?.undo?.id), JSON.stringify(removedNote.body));
      await call(alice, 'POST', `/audits/${auditId}/undo/${removedNote.body.undo.id}`);
      const notes = await call(alice, 'GET', `/audits/${auditId}/notes`);
      check(
        'and comes back with its text',
        (notes.body ?? []).some(
          (row) => String(row._id) === String(note.body._id) && row.content.includes('Worth keeping')
        )
      );
      await call(alice, 'DELETE', `/audits/${auditId}/notes/${note.body._id}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${section.body._id}`);
    }

    /* ------------------------------------------------ a sheet full of findings --- */
    log.info('');
    log.info('Importing findings from a spreadsheet');
    {
      const sheet = [
        'Title,Severity,Vector,Description,Remediation',
        'zz Imported injection,High,CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H,Parameter is concatenated,Use placeholders',
        'zz Imported headers,Low,,No CSP,Add one',
        ',High,,A row with no title,',
      ].join('\n');

      const form = () => {
        const body = new FormData();
        body.append('file', new Blob([sheet], { type: 'text/csv' }), 'findings.csv');
        return body;
      };

      const preview = await call(alice, 'POST', `/audits/${auditId}/findings/import/preview`, form());
      check(
        'the preview judges every row without writing anything',
        preview.status === 200 && preview.body?.counts?.new === 2 && preview.body?.counts?.invalid === 1,
        `${preview.status} ${JSON.stringify(preview.body?.error ?? preview.body?.counts)}`
      );
      const beforeCount = (await call(alice, 'GET', `/audits/${auditId}`)).body?.findings?.length ?? 0;

      const chosen = preview.body.rows.filter((row) => row.status === 'new').map((row) => row.line);
      const body = form();
      body.append('lines', chosen.join(','));
      const imported = await call(alice, 'POST', `/audits/${auditId}/findings/import`, body);
      check(
        'the chosen rows are created',
        imported.status === 201 && imported.body?.created?.length === 2,
        JSON.stringify(imported.body?.created ?? imported.body?.error)
      );

      const after = await call(alice, 'GET', `/audits/${auditId}?full=1`);
      const made = (after.body?.findings ?? []).filter((f) => f.title.startsWith('zz Imported'));
      check('and they are on the engagement', made.length === 2, `${made.length}`);
      check(
        'with the vector from the sheet, scored',
        made.some((f) => String(f.cvssv3 ?? '').includes('C:H/I:H/A:H')),
        JSON.stringify(made.map((f) => f.cvssv3))
      );
      check(
        'and the prose as paragraphs rather than bare text',
        made.some((f) => String(f.description ?? '').startsWith('<p>')),
        JSON.stringify(made.map((f) => String(f.description ?? '').slice(0, 20)))
      );
      check('the invalid row was not created', (after.body?.findings ?? []).length === beforeCount + 2);

      /* Importing the same sheet again should now see both as duplicates. */
      const second = await call(alice, 'POST', `/audits/${auditId}/findings/import/preview`, form());
      check(
        'a second run of the same sheet is all duplicates',
        second.body?.counts?.new === 0 && second.body?.counts?.duplicate === 2,
        JSON.stringify(second.body?.counts)
      );
      check(
        'each naming the finding it matches',
        (second.body?.rows ?? []).every((row) => row.status !== 'duplicate' || row.duplicateOf?.title),
        JSON.stringify((second.body?.rows ?? []).map((row) => row.duplicateOf?.title))
      );

      for (const finding of made) {
        await call(alice, 'DELETE', `/audits/${auditId}/findings/${finding._id}`);
      }
    }

    /* --------------------------------------------------- sending the report --- */
    log.info('');
    log.info('Sending a report by email');
    /*
     * The whole path, against a transport that keeps the message instead of delivering it:
     * render the report, attach it, hand it to the mail layer, record what happened.
     *
     * `capture` is a real transport rather than a stub bolted on for the test — the same lookup
     * `smtp` goes through — so what is exercised here is the code that runs in production, minus
     * the socket. The alternative was a test that proved a mock could be called.
     */
    {
      const { Settings: MailSettings } = await import('../models/settings.model.js');
      const { captured } = await import('../services/mail/index.js');
      const { Template: MailTemplates } = await import('../models/template.model.js');
      const { Delivery: MailDeliveries } = await import('../models/delivery.model.js');

      const settingsDoc = await MailSettings.getSettings();
      const emailBefore = settingsDoc.toObject().email;
      const previousTransport = process.env.MAIL_TRANSPORT;
      process.env.MAIL_TRANSPORT = 'capture';

      const docxTemplate = await MailTemplates.findOne({ kind: { $ne: 'html' } });
      if (!docxTemplate) {
        log.warn('  no .docx template in this instance, so the send path was not exercised');
      } else {
        await call(alice, 'PUT', `/audits/${auditId}`, { template: docxTemplate._id.toString() });
        await MailDeliveries.deleteMany({ audit: auditId });

        /* Off is off: the refusal names the setting rather than failing obscurely. */
        settingsDoc.email = { ...emailBefore, enabled: false };
        settingsDoc.markModified('email');
        await settingsDoc.save();
        const refused = await call(alice, 'POST', `/audits/${auditId}/deliveries/send`, {
          recipients: [{ name: 'Dana', email: 'dana@client.invalid' }],
        });
        check(
          'sending is refused while email is switched off',
          refused.status === 400 && /Settings/i.test(refused.body?.error ?? ''),
          `${refused.status} ${refused.body?.error}`
        );
        check(
          'and nothing was recorded',
          (await MailDeliveries.countDocuments({ audit: auditId })) === 0
        );
        ...
        settingsDoc.email = {
          ...
          security: 'starttls',
          fromName: 'Engy Test',
          fromAddress: 'reports@firm.invalid',
        };
        settingsDoc.markModified('email');
        await settingsDoc.save();

        const before = captured.length;
        const sent = await call(alice, 'POST', `/audits/${auditId}/deliveries/send`, {
          recipients: [{ name: 'Dana Petrescu', email: 'dana@client.invalid' }],
          message: 'As discussed.',
          copyToMe: false,
        });
        check(
          'the report sends and comes back with a delivery',
          sent.status === 201 && Boolean(sent.body?.delivery?._id),
          `${sent.status} ${JSON.stringify(sent.body?.error ?? '').slice(0, 120)}`
        );
        check(
          'the delivery records the hash of the file that actually went',
          /^[a-f0-9]{64}$/.test(sent.body?.delivery?.fileHash ?? '') &&
            sent.body?.delivery?.fileSize > 0 &&
            sent.body?.delivery?.kind === 'docx',
          JSON.stringify({ hash: sent.body?.delivery?.fileHash, size: sent.body?.delivery?.fileSize })
        );
        check(
          'and a version, without anybody typing one',
          sent.body?.delivery?.version === '1.0',
          JSON.stringify(sent.body?.delivery?.version)
        );
        check(
          'the recipient is on the record',
          sent.body?.delivery?.recipients?.[0]?.email === 'dana@client.invalid',
          JSON.stringify(sent.body?.delivery?.recipients)
        );

        const message = captured.at(-1);
        check('one message reached the transport', captured.length === before + 1);
        check(
          'addressed to the client, from the instance',
          message?.recipients?.join() === 'dana@client.invalid' &&
            message?.raw.includes('From: "Engy Test" <reports@firm.invalid>'),
          message?.raw.split('\r\n').slice(0, 3).join(' | ')
        );
        check(
          'with the report attached as a .docx',
          message?.raw.includes('Content-Type: multipart/mixed') &&
            message?.raw.includes('wordprocessingml.document') &&
            message?.raw.includes('Content-Disposition: attachment'),
          'no attachment part'
        );
        /*
         * The body travels base64 whenever it is not pure ASCII, and a heading with an em dash in
         * it never is — so this decodes the encoded runs rather than grepping the wire format.
         * Which is also the only way to be sure the note reached the *body* and not a header.
         *
         * Run by run, not chunk by chunk: a MIME part's base64 is a block of full-width lines
         * followed immediately by the next boundary, so splitting on blank lines leaves the
         * boundary stuck to the end of the payload and nothing decodes.
         */
        const decodeRuns = (raw) => {
          const out = [];
          let run = [];
          const flush = () => {
            if (run.length) out.push(Buffer.from(run.join(''), 'base64').toString('utf8'));
            run = [];
          };
          for (const line of String(raw ?? '').split('\r\n')) {
            if (/^[A-Za-z0-9+/=]{40,}$/.test(line)) run.push(line);
            else {
              flush();
              out.push(line);
            }
        ...
        const readable = decodeRuns(message?.raw);
        check(
          'the covering note and the hash are both in the body',
          readable.includes('As discussed.') &&
            readable.includes(sent.body.delivery.fileHash.slice(0, 32)),
          'the note or the hash is missing'
        );
        check(
          'the send is on the engagement’s activity log',
          (await Activity.countDocuments({ audit: auditId, action: 'report.delivered' })) > 0
        );

        /* A second send takes the next version rather than repeating the first. */
        const again = await call(alice, 'POST', `/audits/${auditId}/deliveries/send`, {
          recipients: [{ email: 'dana@client.invalid' }],
        });
        check(
          'a second send follows on from the first',
          again.body?.delivery?.version === '1.1',
          JSON.stringify(again.body?.delivery?.version)
        );

        /* An address the mail layer will not accept is refused before anything is recorded. */
        const bad = await call(alice, 'POST', `/audits/${auditId}/deliveries/send`, {
          recipients: [{ email: 'not an address' }],
        });
        check('an unusable address is refused outright', bad.status === 400, `got ${bad.status}`);

        await MailDeliveries.deleteMany({ audit: auditId });
      }

      /* The mail server is nobody's business but an administrator's. */
      const asAdmin = await call(alice, 'GET', '/settings');
      const asUser = await call(bob, 'GET', '/settings');
      check(
        'an admin sees the mail configuration',
        asAdmin.body?.email?.host === 'smtp.invalid',
        JSON.stringify(asAdmin.body?.email)
      );
      check(
        'and everybody else is told only whether it works',
        asUser.body?.email?.host === undefined &&
          typeof asUser.body?.email?.enabled === 'boolean' &&
          typeof asUser.body?.email?.configured === 'boolean',
        JSON.stringify(asUser.body?.email)
      );
      check(
        'the stored password never leaves the server',
        JSON.stringify(asAdmin.body?.email ?? {}).includes('secret') === false,
        JSON.stringify(asAdmin.body?.email)
      );

      settingsDoc.email = emailBefore;
      settingsDoc.markModified('email');
      await settingsDoc.save();
      if (previousTransport === undefined) delete process.env.MAIL_TRANSPORT;
      else process.env.MAIL_TRANSPORT = previousTransport;
    }

    /* ------------------------------------------- renaming one thing everywhere --- */
    log.info('');
    log.info('Renaming a host everywhere it appears');
    /*
     * The one bulk write in the app with no undo behind it, so what is checked here is mostly what
     * it refuses to do. Tool output and the command beside it must come out of a rename exactly as
     * they went in — a report whose appendix has been edited to agree with its prose is worse than
     * one whose prose is out of date — and a finding somebody else is holding must be skipped and
     * named rather than quietly overwritten.
     */
    {
      const HOST = 'zz-old.acme.invalid';
      const RENAMED = 'zz-new.northwind.invalid';

      const target = await call(alice, 'POST', `/audits/${auditId}/findings`, {
        title: `zz Rename weak TLS on ${HOST}`,
        description: `<p>The host ${HOST} negotiates TLS 1.0.</p>`,
        scope: `<p>${HOST}:443</p>`,
      });
      const targetId = target.body?._id;
      const bystander = await call(alice, 'POST', `/audits/${auditId}/findings`, {
        title: 'zz Rename bystander',
        remediation: `<p>Also mentions ${HOST}.</p>`,
      });
      const bystanderId = bystander.body?._id;

      const step = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: `zz Rename scan of ${HOST}`,
        tool: 'nmap',
        command: `nmap -sV ${HOST}`,
        output: `Nmap scan report for ${HOST}\n443/tcp open  https`,
        content: `<p>The TLS on ${HOST} is old.</p>`,
        phase: 'recon',
      });
      const stepId = step.body?._id;

      const preview = await call(alice, 'POST', `/audits/${auditId}/replace/preview`, {
        find: HOST,
        replace: RENAMED,
      });
      const where = (preview.body?.hits ?? []).map((hit) => hit.where);
      check(
        'the preview finds every place the writing names it',
        preview.status === 200 && preview.body.total === 6,
        `${preview.status} ${JSON.stringify([preview.body?.total, where])}`
      );
      check(
        'across the finding, its prose, the step and the write-up',
        ['finding.title', 'finding.description', 'finding.scope', 'step.title', 'step.content'].every(
          (kind) => where.includes(kind)
        ),
        JSON.stringify(where)
      );
      check(
        'and nothing anywhere near the output or the command',
        !JSON.stringify(preview.body?.hits ?? []).includes('Nmap scan report') &&
          !JSON.stringify(preview.body?.hits ?? []).includes('nmap -sV'),
        JSON.stringify(preview.body?.hits)
      );
      check(
        'each hit names where it is and shows the text around it',
        (preview.body?.hits ?? []).every((hit) => hit.label && hit.excerpts?.length && hit.count > 0),
        JSON.stringify(preview.body?.hits?.map((hit) => [hit.label, hit.count]))
      );
      check(
        'nothing has been written yet',
        (await call(alice, 'GET', `/audits/${auditId}`)).body?.findings?.some(
          (finding) => String(finding._id) === targetId && finding.title.includes(HOST)
        ),
        'the preview wrote something'
      );

      /* A stale list must not be allowed to commit. */
      const stale = await call(alice, 'POST', `/audits/${auditId}/replace`, {
        find: HOST,
        replace: RENAMED,
        expectedTotal: preview.body.total + 1,
      });
      check(
        'a count that no longer agrees is refused',
        stale.status === 409 && /changed while/.test(stale.body?.error ?? ''),
        `${stale.status} ${JSON.stringify(stale.body?.error)}`
      );

      await call(bob, 'POST', `/audits/${auditId}/findings/${bystanderId}/lock`, {
        note: 'reading it',
      });
      const done = await call(alice, 'POST', `/audits/${auditId}/replace`, {
        find: HOST,
        replace: RENAMED,
        expectedTotal: preview.body.total,
      });
      check(
        'the rest is replaced even though one finding is held',
        done.status === 200 && done.body?.changed === 5,
        `${done.status} ${JSON.stringify(done.body)}`
      );
      check(
        'and the held one is named rather than overwritten',
        (done.body?.skipped ?? []).join() === 'zz Rename bystander',
        JSON.stringify(done.body?.skipped)
      );

      const after = await call(alice, 'GET', `/audits/${auditId}?full=1`);
      const renamed = (after.body?.findings ?? []).find((f) => String(f._id) === targetId);
      const held = (after.body?.findings ?? []).find((f) => String(f._id) === bystanderId);
      check(
        'the title and the prose now say the new name',
        renamed?.title === `zz Rename weak TLS on ${RENAMED}` &&
          renamed?.description?.includes(RENAMED) &&
          renamed?.scope?.includes(RENAMED),
        JSON.stringify([renamed?.title, renamed?.scope])
      );
      check('the held finding still says the old one', held?.remediation?.includes(HOST));

      const body = await call(alice, 'GET', `/audits/${auditId}/enumeration/${stepId}`);
      check(
        'the write-up was rewritten',
        body.body?.content?.includes(RENAMED),
        JSON.stringify(body.body?.content)
      );
      check(
        'the command and the output are exactly as they were run',
        body.body?.command === `nmap -sV ${HOST}` &&
          body.body?.output?.includes(`Nmap scan report for ${HOST}`),
        JSON.stringify([body.body?.command, body.body?.output?.slice(0, 40)])
      );
      check(
        'the rename is on the activity log, with what it was and what it became',
        (await Activity.countDocuments({ audit: auditId, action: 'audit.text-replaced' })) > 0
      );

      await call(bob, 'DELETE', `/audits/${auditId}/findings/${bystanderId}/lock`);
      await call(alice, 'DELETE', `/audits/${auditId}/findings/${targetId}`);
      await call(alice, 'DELETE', `/audits/${auditId}/findings/${bystanderId}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${stepId}`);
    }

    /* --------------------------------------------- whose write-up this is --- */
    log.info('');
    log.info('Handing findings to somebody');
    /*
     * Ownership of a finding. `createdBy` and `updatedBy` say who wrote it and who touched it
     * last; neither answers "whose is it", which is the question a two-person engagement asks
     * every morning — and the answer has to be somebody who can actually open the engagement,
     * has to reach them once rather than forty times, and has to leave a queue that empties.
     */
    {
      const CRITICAL = 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H';
      const first = await call(alice, 'POST', `/audits/${auditId}/findings`, {
        title: 'zz-assign the critical one',
        cvssv3: CRITICAL,
      });
      const second = await call(alice, 'POST', `/audits/${auditId}/findings`, {
        title: 'zz-assign a smaller one',
      });
      const third = await call(alice, 'POST', `/audits/${auditId}/findings`, {
        title: 'zz-assign another small one',
      });
      const ids = [first.body?._id, second.body?._id, third.body?._id];

      /* This run's bells only: earlier blocks have left plenty of others. */
      await Notification.deleteMany({ user: bob.user._id, type: 'finding-assigned' });
      await Notification.deleteMany({ user: alice.user._id, type: 'finding-assigned' });

      const given = await call(alice, 'PUT', `/audits/${auditId}/findings/${ids[0]}`, {
        assignedTo: bob.user._id.toString(),
      });
      check(
        'a finding can be given to somebody on the engagement',
        given.status === 200 && String(given.body?.assignedTo?._id ?? '') === String(bob.user._id),
        `${given.status} ${JSON.stringify(given.body?.assignedTo ?? given.body?.error)}`
      );

      const listed = await call(alice, 'GET', `/audits/${auditId}`);
      check(
        'and the list says so without anybody asking for the whole engagement',
        (listed.body?.findings ?? []).some(
          (finding) =>
            String(finding._id) === ids[0] &&
            String(finding.assignedTo?._id ?? '') === String(bob.user._id)
        ),
        JSON.stringify(
          (listed.body?.findings ?? [])
            .filter((f) => f.title.startsWith('zz-assign'))
            .map((f) => [f.title, f.assignedTo?.username ?? null])
        )
      );

      const handover = await Activity.findOne({
        audit: auditId,
        action: 'finding.assigned',
      }).sort({ createdAt: -1 });
      check(
        'the log says who got it, which is the only interesting part of the sentence',
        /gave .* to .* to write/.test(handover?.summary ?? ''),
        JSON.stringify(handover?.summary)
      );
      check(
        'and the bell tells them',
        (await Notification.countDocuments({ user: bob.user._id, type: 'finding-assigned' })) === 1,
        `${await Notification.countDocuments({ user: bob.user._id, type: 'finding-assigned' })}`
      );

      const stranger = await makeUser('assign', 'user');
      const refused = await call(alice, 'PUT', `/audits/${auditId}/findings/${ids[1]}`, {
        assignedTo: stranger.user._id.toString(),
      });
      check(
        'somebody who is not on the engagement cannot be given one',
        refused.status === 400 && /not on this engagement/.test(refused.body?.error ?? ''),
        `${refused.status} ${JSON.stringify(refused.body?.error)}`
      );

      await call(alice, 'PUT', `/audits/${auditId}/findings/${ids[1]}`, {
        assignedTo: alice.user._id.toString(),
      });
      check(
        'taking one yourself tells nobody, least of all you',
        (await Notification.countDocuments({ user: alice.user._id, type: 'finding-assigned' })) === 0
      );

      const batch = await call(alice, 'POST', `/audits/${auditId}/findings/bulk`, {
        ids: [ids[1], ids[2]],
        action: 'assign',
        value: bob.user._id.toString(),
      });
      check(
        'a selection can be split in one go',
        batch.status === 200 && batch.body?.changed?.length === 2,
        `${batch.status} ${JSON.stringify(batch.body?.changed ?? batch.body?.error)}`
      );
      check(
        'and a batch is one bell, not one per finding',
        (await Notification.countDocuments({ user: bob.user._id, type: 'finding-assigned' })) === 2,
        `${await Notification.countDocuments({ user: bob.user._id, type: 'finding-assigned' })}`
      );

      /* ------------------------------------------------------- and the queue --- */
      const queue = await call(bob, 'GET', '/inbox');
      const mine = (queue.body?.findings ?? []).filter((row) => row.title.startsWith('zz-assign'));
      check(
        'his inbox lists what is his to write',
        mine.length === 3,
        JSON.stringify((queue.body?.findings ?? []).map((row) => row.title))
      );
      check(
        'worst first, which is the order somebody works in',
        mine[0]?.title === 'zz-assign the critical one' && mine[0]?.severity === 'Critical',
        JSON.stringify(mine.map((row) => [row.title, row.severity]))
      );
      check(
        'naming the engagement, because a queue crosses them',
        Boolean(mine[0]?.auditName),
        JSON.stringify(mine[0])
      );
      check(
        'and it counts as something waiting on him',
        (queue.body?.counts?.findings ?? 0) >= 3 && (queue.body?.counts?.total ?? 0) >= 3,
        JSON.stringify(queue.body?.counts)
      );
      const hers = await call(alice, 'GET', '/inbox');
      check(
        'while it is not waiting on the person who handed it over',
        !(hers.body?.findings ?? []).some((row) => row.title.startsWith('zz-assign')),
        JSON.stringify((hers.body?.findings ?? []).map((row) => row.title))
      );

      const cleared = await call(alice, 'PUT', `/audits/${auditId}/findings/${ids[0]}`, {
        assignedTo: null,
      });
      check(
        'it can be given back to nobody',
        cleared.status === 200 && !cleared.body?.assignedTo,
        `${cleared.status} ${JSON.stringify(cleared.body?.assignedTo)}`
      );
      const unassigned = await Activity.findOne({ audit: auditId, action: 'finding.assigned' }).sort({
        createdAt: -1,
      });
      check(
        'and the log says that rather than naming somebody',
        /left .* unassigned/.test(unassigned?.summary ?? ''),
        JSON.stringify(unassigned?.summary)
      );

      /* --------------------------------- an engagement that is over, and a move --- */
      const elsewhere = await call(alice, 'POST', '/audits', {
        name: 'zz-assign elsewhere',
        collaborators: [bob.user._id.toString()],
      });
      const there = await call(alice, 'POST', `/audits/${elsewhere.body._id}/findings`, {
        title: 'zz-assign on the other one',
      });
      await call(alice, 'PUT', `/audits/${elsewhere.body._id}/findings/${there.body._id}`, {
        assignedTo: bob.user._id.toString(),
      });
      check(
        'a finding on another engagement lands in the same queue',
        ((await call(bob, 'GET', '/inbox')).body?.findings ?? []).some(
          (row) => row.title === 'zz-assign on the other one'
        )
      );

      await call(alice, 'POST', `/audits/${elsewhere.body._id}/archive`, {});
      check(
        'and leaves it when the engagement is put away, because it is nobody’s work now',
        !((await call(bob, 'GET', '/inbox')).body?.findings ?? []).some(
          (row) => row.title === 'zz-assign on the other one'
        )
      );
      await call(alice, 'DELETE', `/audits/${elsewhere.body._id}/archive`);

      /*
       * Moving a finding to an engagement its owner is not on. Being assigned means being a
       * member — the route refuses anybody else — so the assignment cannot survive the move, or
       * it would sit in a queue its owner can never see.
       */
      const stripped = await call(alice, 'POST', '/audits', { name: 'zz-assign destination' });
      const carried = await call(alice, 'POST', `/audits/${auditId}/findings/bulk`, {
        ids: [ids[2]],
        action: 'transfer',
        target: stripped.body._id,
        mode: 'move',
      });
      const landedThere = await call(alice, 'GET', `/audits/${stripped.body._id}`);
      check(
        'a moved finding arrives belonging to nobody',
        carried.status === 200 &&
          (landedThere.body?.findings ?? []).every((finding) => !finding.assignedTo),
        JSON.stringify((landedThere.body?.findings ?? []).map((f) => [f.title, f.assignedTo]))
      );
      ...
      await Notification.deleteMany({ user: bob.user._id, type: 'finding-assigned' });
      await Audit.deleteMany({ name: /^zz-assign/ });
    }

    /* ------------------------------------------- the operation, in one order --- */
    log.info('');
    log.info('One timeline out of four logs');
    /*
     * The merge itself is checked without a database in `npm run test:timeline`. What matters here
     * is that the route composes the same four sources the report does — a page and a document
     * that disagree about when something happened is worse than neither having a timeline.
     */
    {
      const step = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz-timeline external scan',
        tool: 'nmap',
        command: 'nmap -sV zz.example',
        output: 'Nmap scan report for zz.example',
        phase: 'recon',
      });
      /* Ran, or at least written down, but with no output and therefore no recorded time. */
      const untimed = await call(alice, 'POST', `/audits/${auditId}/enumeration`, {
        title: 'zz-timeline something we did',
        tool: 'crackmapexec',
        phase: 'recon',
      });
      const event = await call(alice, 'POST', `/audits/${auditId}/detections`, {
        action: 'zz-timeline spray against the VPN',
        target: 'vpn.zz.example',
        occurredAt: new Date('2026-07-22T14:00:00.000Z').toISOString(),
        detectedAt: new Date('2026-07-22T14:26:00.000Z').toISOString(),
        outcome: 'alerted',
        noise: 'loud',
      });

      const timeline = await call(alice, 'GET', `/audits/${auditId}/timeline`);
      const rows = timeline.body?.rows ?? [];
      check(
        'the route answers with a merged timeline',
        timeline.status === 200 && Array.isArray(rows),
        `${timeline.status} ${JSON.stringify(timeline.body?.error)}`
      );
      check(
        'with the detection event in it, saying how long it took to be noticed',
        rows.some(
          (row) => row.kind === 'detection' && /Noticed after 26 min/.test(row.note ?? '')
        ),
        JSON.stringify(rows.filter((row) => row.kind === 'detection').map((row) => row.note))
      );
      check(
        'in order, oldest first',
        rows.every((row, index) => index === 0 || new Date(rows[index - 1].at) <= new Date(row.at)),
        JSON.stringify(rows.map((row) => row.at))
      );
      check(
        'and a step that has never run is counted rather than placed',
        (timeline.body?.skipped?.runs ?? 0) >= 1,
        JSON.stringify(timeline.body?.skipped)
      );
      check(
        'the summary carries the shape of the time',
        timeline.body?.summary && typeof timeline.body.summary.events === 'number',
        JSON.stringify(timeline.body?.summary)
      );

      await call(alice, 'DELETE', `/audits/${auditId}/detections/${event.body?._id}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${step.body?._id}`);
      await call(alice, 'DELETE', `/audits/${auditId}/enumeration/${untimed.body?._id}`);
    }

    /* --------------------------------------------- the report, as a PDF --- */
    log.info('');
    log.info('Rendering a PDF');
    /*
     * Only the half that is the same on every machine. Whether a converter exists is a property of
     * the machine rather than of the app, so the conversion itself is proved in `npm run test:pdf`
     * — which converts a real template where one is installed and says so loudly where it is not.
     * What belongs here is the contract: the route is there, it refuses politely when nothing is
     * set up, and it says who can fix that.
     */
    {
      const { Settings: PdfSettings } = await import('../models/settings.model.js');
      const settingsRow = await PdfSettings.getSettings();
      const pdfBefore = settingsRow.toObject().pdf;

      settingsRow.pdf = { converter: 'none', command: '', timeoutSeconds: 120 };
      settingsRow.markModified('pdf');
      await settingsRow.save();

      const off = await call(alice, 'GET', `/audits/${auditId}/report.pdf`);
      check(
        'with no converter set up, a PDF is refused rather than half-made',
        off.status === 400 && /No PDF converter/.test(off.body?.error ?? ''),
        `${off.status} ${JSON.stringify(off.body?.error)}`
      );
      check(
        'and the message says who can do something about it',
        /administrator/i.test(off.body?.error ?? ''),
        off.body?.error
      );

      /*
       * And that the render is rate limited at all.
       *
       * Asserted by the allowance headers rather than by asking twenty-one times: exhausting it
       * would add a minute to this suite to prove a number that is a judgement call anyway. What
       * matters is that the limiter is *on this route* — a full render is seconds of one thread, and
       * before this there was nothing between somebody holding the button and everybody waiting.
       */
      check(
        'a render says how much of its allowance is left',
        Number(off.headers['ratelimit-limit']) > 0 &&
          off.headers['ratelimit-remaining'] !== undefined,
        JSON.stringify({
          limit: off.headers['ratelimit-limit'],
          remaining: off.headers['ratelimit-remaining'],
        })
      );
      const docx = await call(alice, 'GET', `/audits/${auditId}/report`);
      check(
        'and so does the .docx render, which costs the same',
        Number(docx.headers['ratelimit-limit']) > 0,
        JSON.stringify(docx.headers['ratelimit-limit'])
      );
      check(
        'the allowance is per account, so one person cannot spend the floor’s',
        Number((await call(bob, 'GET', `/audits/${auditId}/report.pdf`)).headers[
          'ratelimit-remaining'
        ]) > Number(docx.headers['ratelimit-remaining']),
        'a second account started from the first one’s remaining allowance'
      );

      /* --------------------------------------- request ids ---------------- */
      /*
       * One id per request, in three places: a header on everything, the body of anything that
       * failed, and every log line written while it was being served. The third is what it is for
       * and is the one this cannot see — so what is checked here is that the first two agree, which
       * is what makes an id somebody quotes worth anything.
       */
      {
        const { acceptRequestId, newRequestId, runWithRequestId, currentRequestId } = await import(
          '../utils/request-context.js'
        );

        const first = await call(alice, 'GET', '/settings');
        const second = await call(alice, 'GET', '/settings');
        check(
          'every answer says which request it was',
          /^[0-9a-f]{8}$/.test(first.headers['x-request-id'] ?? ''),
          JSON.stringify(first.headers['x-request-id'])
        );
        check(
          'and no two requests share one',
          first.headers['x-request-id'] !== second.headers['x-request-id'],
          `${first.headers['x-request-id']} twice`
        );

        const missing = await call(alice, 'GET', '/audits/000000000000000000000000');
        check(
          'a failure carries it in the body as well as the header',
          Boolean(missing.body?.requestId) &&
            missing.body.requestId === missing.headers['x-request-id'],
          JSON.stringify({ body: missing.body?.requestId, header: missing.headers['x-request-id'] })
        );

        const nowhere = await call(alice, 'GET', '/no-such-route-at-all');
        check(
          'so does a route that does not exist',
          nowhere.status === 404 && Boolean(nowhere.body?.requestId),
          JSON.stringify(nowhere.body)
        );

        /* A proxy that already stamps requests should win, so its id is kept. */
        const mine = 'proxy-abc123';
        const passed = await fetch(`${base}/settings`, {
          headers: { Authorization: `Bearer ${alice.token}`, 'X-Request-Id': mine },
        });
        check(
          'an id from a reverse proxy is honoured rather than replaced',
          passed.headers.get('x-request-id') === mine,
          String(passed.headers.get('x-request-id'))
        );

        /*
         * But not any header. This value is written into log lines and back into a response header,
         * so a newline in it would forge log entries and a carriage return would let somebody append
         * a header of their own. Replaced rather than stripped: a mangled id looks like it came from
         * somewhere, which is worse than an honest new one.
         */
        const hostile = await fetch(`${base}/settings`, {
          headers: {
            Authorization: `Bearer ${alice.token}`,
            /* Percent-encoded, because `fetch` refuses to send a raw newline in a header at all. */
            'X-Request-Id': 'evil%0d%0aX-Injected:%20yes',
          },
        });
        const answered = hostile.headers.get('x-request-id') ?? '';
        check(
          'and one that is not plainly an id is replaced',
          /^[0-9a-f]{8}$/.test(answered) && hostile.headers.get('x-injected') === null,
          JSON.stringify({ answered, injected: hostile.headers.get('x-injected') })
        );

        check(
          'anything with a newline, a colon or a space in it is refused outright',
          ['a b', 'a:b', 'a\nb', 'a\rb', '', 'x'.repeat(65)].every(
            (value) => acceptRequestId(value) !== value
          ),
          'something unsafe was accepted as an id'
        );
        check(
          'while a plain one is kept as it is',
          acceptRequestId('req-1.2_3') === 'req-1.2_3' &&
            /^[0-9a-f]{8}$/.test(acceptRequestId(undefined))
        );

        /* And that it survives an await, which is the whole reason it is async-local storage. */
        const seen = await runWithRequestId('deadbeef', async () => {
          await new Promise((resolve) => setTimeout(resolve, 5));
          return currentRequestId();
        });
        check('the id follows the work across an await', seen === 'deadbeef', seen);
        check(
          'and there is none outside a request, rather than a stale one',
          currentRequestId() === '',
          currentRequestId()
        );
        check('every fresh id is eight hex characters', /^[0-9a-f]{8}$/.test(newRequestId()));
      }

      /* --------------------------------------- a protected report --------- */
      /*
       * The archive itself is proved against a real 7-Zip in `npm run test:zipcrypt`. What is left
       * for here is the route: that a password cannot travel in a URL, that a weak one is refused,
       * that what comes back is genuinely the encrypted thing, and — the one that matters — that the
       * password is nowhere afterwards.
       */
      {
        const PASSWORD = 'harbour-lichen-sextant-quarry';

        const nothing = await call(alice, 'POST', `/audits/${auditId}/report/protected`, {});
        check(
          'a protected report needs a password',
          nothing.status === 422,
          `got ${nothing.status}`
        );
        const weak = await call(alice, 'POST', `/audits/${auditId}/report/protected`, {
          password: 'short',
        });
        check('and not a trivial one', weak.status === 422, `got ${weak.status}`);

        /* Raw bytes, because a zip put through `response.text()` is no longer a zip. */
        const response = await fetch(`${base}/audits/${auditId}/report/protected`, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${alice.token}`,
        ...
        check(
          'a good one gets an archive',
          response.status === 200 && response.headers.get('content-type') === 'application/zip',
          `${response.status} ${response.headers.get('content-type')}`
        );
        check(
          'named so what is inside is obvious from the outside',
          /\.docx\.zip/.test(response.headers.get('content-disposition') ?? ''),
          String(response.headers.get('content-disposition'))
        );
        check(
          'which really is a zip, with an AES-encrypted entry in it',
          archive.readUInt32LE(0) === 0x04034b50 && archive.readUInt16LE(8) === 99,
          JSON.stringify({ signature: archive.readUInt32LE(0), method: archive.readUInt16LE(8) })
        );
        check(
          'and is digested, so the delivery record needs no retyping',
          /^[0-9a-f]{64}$/.test(response.headers.get('x-report-sha256') ?? ''),
          String(response.headers.get('x-report-sha256'))
        );

        /*
         * The document is in there and unreadable. Checked by looking for a marker every .docx
         * contains — a plain zip of a docx would have `word/` in it in the clear.
         */
        check(
          'the document inside is not readable without the password',
          !archive.includes(Buffer.from('word/document.xml', 'ascii')) &&
            !archive.includes(Buffer.from('PK\u0003\u0004', 'ascii'), 4),
          'part of the .docx is sitting in the archive unencrypted'
        );

        /*
         * And the password is not kept anywhere. The whole feature is worthless if the key to the
         * document ends up beside the record of the document — so this looks through the activity
         * log and the render records rather than at one field.
         */
        const { Activity } = await import('../models/activity.model.js');
        const { RenderRecord } = await import('../models/render-record.model.js');
        const trail = await Activity.find({ audit: auditId }).lean();
        const renders = await RenderRecord.find({ audit: auditId }).lean();
        check(
          'the password is nowhere in the activity log',
          !JSON.stringify(trail).includes(PASSWORD),
          'the password was recorded in the trail'
        );
        check(
          'nor in what was recorded about the render',
          !JSON.stringify(renders).includes(PASSWORD),
          'the password was recorded against the render'
        );
        check(
          'but the log does say this one left protected',
          trail.some((row) => row.meta?.encrypted === true),
          'nothing recorded that an archive was made'
        );

        /* The suggestion behind the button. */
        const offered = await call(alice, 'GET', `/audits/${auditId}/report/passphrase`);
        check(
          'the button offers four words rather than eight characters',
          /^[a-z]+(-[a-z]+){3}$/.test(offered.body?.passphrase ?? ''),
          JSON.stringify(offered.body?.passphrase)
        );
        check(
          'and says how the client is meant to open it',
          /7-Zip/.test(offered.body?.openedWith ?? ''),
          JSON.stringify(offered.body?.openedWith)
        );
        check(
          'a second call offers a different one',
          (await call(alice, 'GET', `/audits/${auditId}/report/passphrase`)).body?.passphrase !==
            offered.body?.passphrase,
          'the same passphrase twice'
        );

        /* Somebody of this block's own: the strangers made elsewhere are out of scope here. */
        const outsider = await makeUser('protected-outsider', 'user');
        const stranger = await call(outsider, 'POST', `/audits/${auditId}/report/protected`, {
          password: PASSWORD,
        });
        check(
          'and somebody not on the engagement cannot have one',
      ...
      /* --------------------------------------- backup and restore -------- */
      /*
       * The buttons on the Settings page. Whether a backup can be *put back* is proved in
       * `npm run test:backup`, which does the whole round trip in a scratch database of its own —
       * this suite runs against the development database, so nothing here applies a restore. What
       * is checked is everything up to that point: who may ask, what comes back, and that the
       * confirmation cannot be got past.
       */
      {
        const Users3 = (await import('../models/user.model.js')).User;
        const settingsRow2 = await PdfSettings.getSettings();
        const backupBefore = settingsRow2.toObject().backup;

        /* Without a second factor, both are refused — the same bar a restricted engagement sets. */
        await Users3.findByIdAndUpdate(alice.user._id, { $set: { totpEnabled: false } });
        const bare = await call(alice, 'POST', '/backup', {});
        check(
          'a backup needs two-factor authentication, even for an admin',
          bare.status === 403 && /two-factor/i.test(bare.body?.error ?? ''),
          `${bare.status} ${JSON.stringify(bare.body?.error)}`
        );
        check(
          'and the message says what to do about it',
          /profile/i.test(bare.body?.error ?? ''),
          bare.body?.error
        );

        /* The page can ask what a backup *would* contain without a second factor. */
        const before = await call(alice, 'GET', '/backup');
        check(
          'the card is told what is here now',
          before.status === 200 && before.body?.now?.documents > 0 && before.body?.now?.collections > 0,
          JSON.stringify(before.body?.now)
        );
        check(
          'and whether the button will work',
          before.body?.secondFactor === false,
          JSON.stringify(before.body?.secondFactor)
        );

        check(
          'a tester cannot see any of it',
          (await call(bob, 'GET', '/backup')).status === 403,
          `got ${(await call(bob, 'GET', '/backup')).status}`
        );

        await Users3.findByIdAndUpdate(alice.user._id, { $set: { totpEnabled: true } });

        /* Raw, because a gzip through `response.text()` is no longer a gzip. */
        const response = await fetch(`${base}/backup`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${alice.token}` },
        });
        const archive = Buffer.from(await response.arrayBuffer());
        check(
          'with one, a backup comes back',
          response.status === 200 && response.headers.get('content-type') === 'application/gzip',
          `${response.status} ${response.headers.get('content-type')}`
        );
        check(
          'named so a folder of them sorts by date',
          /engy-backup-\d{4}-\d{2}-\d{2}T[\d-]+\.tar\.gz/.test(
            response.headers.get('content-disposition') ?? ''
          ),
          String(response.headers.get('content-disposition'))
        );
        check(
          'and it really is a gzip',
          archive[0] === 0x1f && archive[1] === 0x8b,
          JSON.stringify([archive[0], archive[1]])
        );

        const after = await call(alice, 'GET', '/backup');
        check(
          'the card now says when the last one was taken, and by whom',
          Boolean(after.body?.last?.at) &&
            after.body?.last?.bytes > 0 &&
            String(after.body?.last?.by).length > 0,
          JSON.stringify(after.body?.last)
        );

        /* ------------------------------------------ reading one back in ---- */
        const form = new FormData();
        form.append('archive', new Blob([archive], { type: 'application/gzip' }), 'backup.tar.gz');
        const inspected = await call(alice, 'POST', '/backup/inspect', form);
        check(
          'the archive it just made can be read back',
          inspected.status === 200 && Boolean(inspected.body?.id),
          `${inspected.status} ${JSON.stringify(inspected.body?.error)}`
        );
        check(
          'every entry checked against the manifest',
          inspected.body?.entriesChecked > 0,
          JSON.stringify(inspected.body?.entriesChecked)
        );
        check(
          'and the summary says what a person needs before replacing anything',
          Boolean(inspected.body?.manifest?.createdAt) &&
            Boolean(inspected.body?.manifest?.database) &&
            inspected.body?.manifest?.counts?.documents > 0,
          JSON.stringify(inspected.body?.manifest)
        );
        check(
          'including what is deliberately not in it',
          (inspected.body?.manifest?.skipped ?? []).includes('sessions'),
          JSON.stringify(inspected.body?.manifest?.skipped)
        );

        /* Something that is not a backup at all. */
        const rubbish = new FormData();
        rubbish.append('archive', new Blob([Buffer.from('not a backup')]), 'nope.tar.gz');
        const refusedFile = await call(alice, 'POST', '/backup/inspect', rubbish);
        check(
          'something that is not an archive is refused rather than staged',
          refusedFile.status === 400,
          `got ${refusedFile.status}`
        );

        /*
         * The confirmation. This is the one that matters: everything above is reversible and this is
         * the last thing standing between a click and a firm's whole history.
         */
        const wrong = await call(alice, 'POST', '/backup/restore', {
          id: inspected.body.id,
          confirm: 'yes',
        });
        check(
          'a restore is refused unless the database name is typed out',
          wrong.status === 400 && wrong.body?.error?.includes(inspected.body.manifest.database),
          `${wrong.status} ${JSON.stringify(wrong.body?.error)}`
        );

        const madeUp = await call(alice, 'POST', '/backup/restore', {
          id: 'restore-../../../etc/passwd',
          confirm: 'x',
        });
        check(
          'and an upload handle that is not one of ours is refused',
          madeUp.status === 400,
          `got ${madeUp.status}`
        );

        /*
         * Deliberately not applied. The restore path is proved in `test:backup` against a scratch
         * database; running it here would replace the development instance this suite is standing on.
         */
        const discarded = await call(alice, 'DELETE', `/backup/staged/${inspected.body.id}`);
        check('a staged upload can be thrown away', discarded.status === 200);
        check(
          'and is gone, so an abandoned upload is not left on disk',
          (
            await call(alice, 'POST', '/backup/restore', {
              id: inspected.body.id,
              confirm: inspected.body.manifest.database,
            })
          ).status === 400,
          'the staged file survived being discarded'
        );

        settingsRow2.backup = backupBefore;
        settingsRow2.markModified('backup');
        await settingsRow2.save();
        await Users3.findByIdAndUpdate(alice.user._id, { $set: { totpEnabled: false } });
      }

      /* --------------------------------------- the sharing switch --------- */
      /*
       * One switch, and everything about shared editing hangs off it. Checked here as a setting
       * like any other; that the socket obeys it is proved in `npm run test:collab-socket`.
       */
      const collabBefore = settingsRow.toObject().collab;
      const sharingOn = await call(alice, 'PUT', '/settings', { collab: { enabled: true } });
      check(
        'an admin can switch shared editing on',
        sharingOn.status === 200 && sharingOn.body?.collab?.enabled === true,
        `${sharingOn.status} ${JSON.stringify(sharingOn.body?.collab)}`
      );
      check(
        'and everybody is told, because everybody sees the fields it governs',
        (await call(bob, 'GET', '/settings')).body?.collab?.enabled === true
      );
      check(
        'switching it off is told to everybody too',
        (await call(alice, 'PUT', '/settings', { collab: { enabled: false } })).body?.collab
          ?.enabled === false
      );
      check(
        'and nobody but an admin can touch it',
        (await call(bob, 'PUT', '/settings', { collab: { enabled: true } })).status === 403
      );
      ...
      /*
       * The settings side of the webhooks. What the payloads look like and how the request behaves
       * when the far end misbehaves is `npm run test:webhook`; what matters here is the one thing
       * only the route can get wrong — that the URL goes in and never comes back out.
       */
      const hooksBefore = settingsRow.toObject().webhooks;
      const CHANNEL = 'https://discord.com/api/webhooks/123456/zz-collab-test-secret-part';
      ...
      const saved = await call(alice, 'PUT', '/settings', {
        webhooks: {
          ...
          provider: 'discord',
          url: CHANNEL,
          events: { findings: true, review: false, delivery: false, client: false, links: false },
        },
      });
      check(
        'an admin can wire up a channel',
        saved.status === 200 && saved.body?.webhooks?.enabled === true,
        `${saved.status} ${JSON.stringify(saved.body?.webhooks)}`
      );
      check(
        'and is told one is stored without being told what it is',
        saved.body?.webhooks?.hasUrl === true && saved.body?.webhooks?.url === undefined,
        JSON.stringify(saved.body?.webhooks)
      );
      check(
        'the host comes back, so the page can say which service it points at',
        saved.body?.webhooks?.urlHost === 'discord.com',
        JSON.stringify(saved.body?.webhooks?.urlHost)
      );
      check(
        'the chosen groups are kept',
        saved.body?.webhooks?.events?.findings === true &&
          saved.body?.webhooks?.events?.review === false,
        JSON.stringify(saved.body?.webhooks?.events)
      );

      /*
       * The credential is not in the response anywhere — not under another key, not inside the
       * change log the settings page shows, not as ciphertext. Checked by looking for the secret
       * part of the URL in the whole body rather than at the field it should not be in, because the
       * failure worth catching is the one that puts it somewhere nobody thought to look.
       */
      const readBack = await call(alice, 'GET', '/settings');
      check(
        'and the URL itself is nowhere in what an admin is sent',
        !JSON.stringify(readBack.body).includes('zz-collab-test-secret-part'),
        'the webhook URL came back to the browser'
      );
      check(
        'not even as the ciphertext it is stored as',
        readBack.body?.webhooks?.url === undefined &&
          readBack.body?.webhooks?.signingSecret === undefined,
        JSON.stringify(Object.keys(readBack.body?.webhooks ?? {}))
      );
      check(
        'the form is given the services and the groups to offer',
        (readBack.body?.webhooks?.providers ?? []).some((entry) => entry.id === 'teams') &&
          (readBack.body?.webhooks?.groups ?? []).some((entry) => entry.id === 'findings'),
        JSON.stringify(readBack.body?.webhooks?.providers?.map((entry) => entry.id))
      );

      const theirs = await call(bob, 'GET', '/settings');
      check(
        'a tester is told nothing about it at all — it governs no control they see',
        theirs.body?.webhooks === undefined,
        JSON.stringify(theirs.body?.webhooks)
      );
      check(
        'and cannot wire one up',
        (await call(bob, 'PUT', '/settings', { webhooks: { enabled: true } })).status === 403
      );
      check(
        'nor press the test button',
        (await call(bob, 'POST', '/settings/webhooks/test', {})).status === 403
      );

      /* A URL that is not one is refused on the way in rather than at the first event. */
      const nonsense = await call(alice, 'PUT', '/settings', {
        webhooks: { url: 'not-a-url-at-all' },
      });
      check(
        'something that is not a URL is refused when it is saved',
        nonsense.status === 400 && /URL/i.test(nonsense.body?.error ?? ''),
        `${nonsense.status} ${nonsense.body?.error}`
      );

      /*
       * Clearing it is a deliberate empty string, which is different from the field being absent
       * and meaning "leave it alone" — the same contract as the mail password.
       */
      const cleared = await call(alice, 'PUT', '/settings', { webhooks: { url: '' } });
      check(
        'and an empty one forgets what was stored',
        cleared.body?.webhooks?.hasUrl === false && cleared.body?.webhooks?.urlHost === '',
        JSON.stringify(cleared.body?.webhooks)
      );
      check(
        'so the test button has nothing to post to and says so',
        (await call(alice, 'POST', '/settings/webhooks/test', {})).status === 400
      );

      /* ------------------------------ and a real event, end to end ------- */
      /*
       * The one thing neither the settings checks above nor `npm run test:webhook` can prove: that
       * writing a finding in the app actually reaches a channel. Both halves passing while the wire
       * between them is missing is the failure worth spending a loopback server on — the payloads
       * are right, the settings are right, and nothing is ever posted.
       */
      {
        const http = await import('node:http');
        const { installWebhooks } = await import('../services/webhooks/index.js');
        ...
        /* What `index.js` does at boot. Installed here because the test builds its own app. */
        installWebhooks();
        ...
        /* Fire and forget, so the answer to the request is not the answer about the webhook. */
        const waitFor = async (count) => {
          for (let attempt = 0; attempt < 60 && arrived.length < count; attempt += 1) {
            await new Promise((resolve) => setTimeout(resolve, 50));
          }
          return arrived.length;
        };

        const written = await call(alice, 'POST', `/audits/${auditId}/findings`, {
          title: 'zz-webhook A finding that should be announced',
          cvssv3: 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H',
        });
        check(
          'a finding is written as usual',
          written.status === 201,
          `${written.status} ${JSON.stringify(written.body?.error)}`
        );

        await waitFor(1);
        check('and the channel is told about it', arrived.length === 1, `${arrived.length} posts`);

        const posted = arrived.length ? JSON.parse(arrived[0]) : {};
        check(
          'with the action, the group and the sentence the activity log shows',
          posted.action === 'finding.created' &&
            posted.event === 'findings' &&
            posted.summary?.includes('zz-webhook A finding that should be announced'),
          JSON.stringify(posted).slice(0, 200)
        );
        check(
          'the severity of the finding it is about',
          posted.severity === 'Critical',
          JSON.stringify(posted.severity)
        );
        check(
          'the engagement it belongs to',
          posted.engagement?.id === String(auditId) && Boolean(posted.engagement?.name),
          JSON.stringify(posted.engagement)
        );
        check(
          'and a link back to it',
          String(posted.url ?? '').endsWith(`/engagements/${auditId}`),
          JSON.stringify(posted.url)
        );

        /*
         * Editing the write-up must not post. This is the difference between a channel people read
         * and a channel people mute, and it is decided by a predicate that is easy to get wrong.
         */
        arrived.length = 0;
        await call(alice, 'PUT', `/audits/${auditId}/findings/${written.body._id}`, {
          description: '<p>More detail, saved as somebody types.</p>',
        });
        await new Promise((resolve) => setTimeout(resolve, 400));
        check(
          'editing the write-up says nothing, however many times it is saved',
          arrived.length === 0,
          `${arrived.length} posts`
        );
        ...
        await waitFor(1);
        check(
          'but the severity moving does',
          arrived.length === 1 &&
            JSON.parse(arrived[0]).action === 'finding.updated',
          `${arrived.length} posts`
        );

        /* Switched off, and nothing at all — the state an instance that never wanted this is in. */
        await call(alice, 'PUT', '/settings', { webhooks: { enabled: false } });
        arrived.length = 0;
        await call(alice, 'POST', `/audits/${auditId}/findings`, {
          title: 'zz-webhook A finding nobody should hear about',
          cvssv3: 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H',
        });
        await new Promise((resolve) => setTimeout(resolve, 400));
        check(
          'and switched off, the channel hears nothing',
          arrived.length === 0,
          `${arrived.length} posts`
        );
        ...
        await Audit.updateOne(
          { _id: auditId },
          { $pull: { findings: { title: /^zz-webhook/ } } }
        );
        receiver.close();
      }

      settingsRow.webhooks = hooksBefore;
      settingsRow.markModified('webhooks');
      await settingsRow.save();

      const asUser = await call(bob, 'GET', '/settings');
      check(
        'everybody is told whether a PDF can be made, because everybody sees the button',
        typeof asUser.body?.pdf?.enabled === 'boolean',
        JSON.stringify(asUser.body?.pdf)
      );
      check(
        'and nobody but an admin is told which program does it or where it lives',
        asUser.body?.pdf?.converter === undefined && asUser.body?.pdf?.command === undefined,
        JSON.stringify(asUser.body?.pdf)
      );

      const asAdmin = await call(alice, 'GET', '/settings');
      check(
        'an admin sees the converter and the presets to choose from',
        asAdmin.body?.pdf?.converter === 'none' && Boolean(asAdmin.body?.pdf?.converters?.libreoffice),
        JSON.stringify(asAdmin.body?.pdf?.converter)
      );

      const chosen = await call(alice, 'PUT', '/settings', {
        pdf: { converter: 'libreoffice', command: 'zz-no-such-converter', timeoutSeconds: 15 },
      });
      check(
        'choosing one saves, like every other setting',
        chosen.status === 200 && chosen.body?.pdf?.converter === 'libreoffice',
        `${chosen.status} ${JSON.stringify(chosen.body?.pdf)}`
      );
      check(
        'and everybody is now told a PDF can be made',
        (await call(bob, 'GET', '/settings')).body?.pdf?.enabled === true
      );

      const proved = await call(alice, 'POST', '/settings/pdf/test', {
        converter: 'libreoffice',
        command: 'zz-no-such-converter',
        timeoutSeconds: 15,
      });
      check(
        'the test button reports the converter’s own failure rather than a stack trace',
        proved.status === 200 &&
          proved.body?.ok === false &&
          /zz-no-such-converter/.test(proved.body?.error ?? ''),
        `${proved.status} ${JSON.stringify(proved.body)}`
      );
      check(
        'and it is an administrator’s button',
        (await call(bob, 'POST', '/settings/pdf/test', { converter: 'libreoffice' })).status === 403
      );

      /* Put the instance back exactly as it was found. */
      settingsRow.pdf = pdfBefore;
      settingsRow.markModified('pdf');
      await settingsRow.save();
    }
    ...
    const { Scratch: Scratches } = await import('../models/scratch.model.js');
    await Scratches.deleteMany({ user: { $in: throwaway.map((u) => u._id) } });
