...
import { saveEnumerationBody } from '../services/enumeration-body.service.js';
...
async function clean() {
  ...
  for (const audit of audits) {
    ...
    /*
     * The bin as well as the findings. `mediaIdsInAudit` finds what the engagement *references*,
     * and an image in the bin is by definition referenced by nothing, so it would survive a clean
     * and sit in storage forever with no way back to what it belonged to.
     */
    for (const item of await auditEvidenceBin(audit._id)) {
      if (await deleteMedia(item.id)) images += 1;
    }
    await Activity.deleteMany({ audit: audit._id });
    await Notification.deleteMany({ audit: audit._id });
    await EnumerationBody.deleteMany({ audit: audit._id });
    await Delivery.deleteMany({ audit: audit._id });
  }
...
async function build() {
  ...
  /* ------------------------------- enumeration ------------------------------ */
  /*
   * The tree, flattened into the parent/order pair the model stores it as.
   *
   * The output goes through `saveEnumerationBody` rather than being assigned: a step keeps only the
   * counters the tree draws with (how many lines there are, the first 240 characters, whether it
   * parsed as columns) and that service is the one thing that maintains them. Written here by hand
   * they would be a set of numbers that were true once.
   */
  const findingIdByTitle = new Map(audit.findings.map((finding) => [finding.title, finding._id]));
  const outputs = [];
  let sectionOrder = 0;
  for (const section of DEMO_ENUMERATION) {
    sectionOrder += 1;
    const parent = audit.enumeration.create({
      title: section.title,
      phase: section.phase ?? '',
    ...
    audit.enumeration.push(parent);

    let childOrder = 0;
    for (const step of section.children ?? []) {
      childOrder += 1;
      const child = audit.enumeration.create({
        title: step.title,
        tool: step.tool ?? '',
        ...
        parent: parent._id,
        order: childOrder,
        author: team[step.author ?? 0]._id,
        /* By title, so the link survives somebody reordering the findings above. */
        ledTo: (step.ledTo ?? []).map((title) => findingIdByTitle.get(title)).filter(Boolean),
      });
      audit.enumeration.push(child);
      if (step.output) outputs.push([child, step.output]);
    }
  }

  await audit.save();
  for (const [step, output] of outputs) {
    await saveEnumerationBody(audit, step, { output });
  }
  /* The counters that service wrote onto the subdocuments still have to be persisted. */
  await audit.save();
  log.info(
    `Recorded ${audit.enumeration.length} enumeration steps in ${DEMO_ENUMERATION.length} sections, ` +
      `${outputs.length} with tool output`
  );

  /* ------------------------------ the evidence bin -------------------------- */
  /* Tagged with the engagement and referenced by nothing, which is what the bin is. */
  for (const spec of DEMO_BIN) {
    await saveMedia({
      buffer: terminalPng(spec),
      contentType: 'image/png',
  ...
  log.info(`Left ${DEMO_BIN.length} screenshots waiting in the evidence bin`);
  ...
  if (template) {
    try {
      ...
      /*
       * And a row in the delivery register for it.
       *
       * Hashed here rather than written into the demo data, because the whole point of the register
       * is that the hash belongs to a file that exists: this one is the SHA-256 of the bytes just
       * written to disk, so somebody can verify the row against the document with sha256sum.
       */
      await Delivery.create({
        ...DEMO_DELIVERY,
        audit: audit._id,
        sentAt: new Date(Date.UTC(2026, 7, 3, 15, 20)),
        recipients: [
          { client: contact._id, name: `${contact.firstname} ${contact.lastname}`, email: contact.email },
        ],
        filename,
        fileHash: crypto.createHash('sha256').update(buffer).digest('hex'),
        fileSize: buffer.length,
        contentFingerprint: populated.contentFingerprint ?? '',
        sentBy: lead._id,
        createdBy: lead._id,
      });
      log.info('Recorded it as delivered, with the hash of that file');
    } catch (error) {
  ...
  log.info('Checks (a part-worked methodology), Enumeration (a tree of tool runs, three of which');
  log.info('name the finding they produced), the evidence bin, the delivery register, Activity,');
  log.info('Insights, and the client page.');
