...
/*
 * A picture the app composed is not evidence.
 *
 * Charts carry their own legend and signatures go on a page somebody signs; numbering either would
 * push every real figure along and put "Figure 8" under somebody's handwriting. Marked at the point
 * they are built, because by the time they reach the converter they are an `<img>` like any other.
 */
const composed = htmlToOoxml(
  `<p><img src="/api/media/${A}" data-figure="no"></p><p><img src="/api/media/${B}"></p>`,
  {
    parts: makeParts(),
    media: new Map([[A, { buffer: PNG, ext: 'png' }], [B, { buffer: PNG, ext: 'png' }]]),
  }
);
check(
  'a picture marked as not a figure is not numbered',
  (composed.match(/SEQ Figure/g) ?? []).length === 1,
  String((composed.match(/SEQ Figure/g) ?? []).length)
);
check('and the evidence beside it still is', composed.includes('SEQ Figure'));

const composedHtml = numberFiguresHtml(
  `<p><img src="/api/media/${A}" data-figure="no"></p><p><img src="/api/media/${B}"></p>`,
  {}
);
check(
  'the HTML report agrees about which pictures are figures',
  composedHtml.count === 1 && !composedHtml.html.includes(`fig-${A}`),
  String(composedHtml.count)
);
...
/* -------------------------------------------------------------------------- */
/* A still frame out of a recording                                           */
/* -------------------------------------------------------------------------- */

/*
 * A document cannot hold a video, so a recording prints as the frame taken out of it. Everything
 * about numbering has to go on working — it is an ordinary picture — and the caption has to say
 * what it is, or the report implies a static screenshot was the whole of the evidence.
 *
 * Both deliverables are checked, because the failure worth guarding against is them disagreeing:
 * the .docx saying "still from a screen recording" while the HTML report quietly does not.
 */
console.log('\nA recording, which prints as its still:');

const REC = 'dddddddddddddddddddddddd';
const recordingHtml =
  `<p>Before the click.</p>` +
  `<figure><img src="/api/media/${REC}" data-video="/api/media/eeeeeeeeeeeeeeeeeeeeeeee">` +
  `<figcaption>The token being reused</figcaption></figure>`;

const recordingDocx = htmlToOoxml(recordingHtml, {
  parts: makeParts(),
  media: new Map([[REC, { buffer: PNG, ext: 'png' }]]),
});
check(
  'the caption says the picture is a still, after what the author wrote',
  /The token being reused.*still from a screen recording/s.test(recordingDocx),
  recordingDocx.slice(-320)
);
check(
  'and it is still numbered like any other figure',
  recordingDocx.includes('SEQ Figure'),
  recordingDocx.slice(-320)
);

const numberedHtml = numberFiguresHtml(recordingHtml).html;
check(
  'the HTML report says the same thing',
  /still from a screen recording/.test(numberedHtml),
  numberedHtml
);
check(
  'once, not twice',
  (numberedHtml.match(/still from a screen recording/g) ?? []).length === 1,
  numberedHtml
);

const plainStill = numberFiguresHtml(`<p><img src="/api/media/${A}"></p>`).html;
check(
  'an ordinary screenshot is not accused of being a recording',
  !/still from a screen recording/.test(plainStill),
  plainStill
);

const uncaptioned = numberFiguresHtml(
  `<p><img src="/api/media/${REC}" data-video="/api/media/eeeeeeeeeeeeeeeeeeeeeeee"></p>`
).html;
check(
  'a recording with no caption of its own still says what it is',
  /still from a screen recording/i.test(uncaptioned),
  uncaptioned
);
