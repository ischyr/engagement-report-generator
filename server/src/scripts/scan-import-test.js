...
import { hostLine, hostsFromScope, scopeHostsHtml } from '../../../client/src/lib/scope-hosts.js';
...
/* -------------------------------------------------------------------------- */
/* And back out again: the imported scope filling a finding's affected assets  */
/* -------------------------------------------------------------------------- */

/*
 * The other end of the same pipe. A scan comes in as XML and eventually has to come back out as a
 * sentence in a report — "nine of the forty" — and the two halves are checked together here
 * because the shape in the middle is this file's fixture and nobody else's.
 */
const groups = hostsFromScope(merged.scope);
check(
  'the imported scope is pickable',
  groups.length === 1 && groups[0].hosts.length === 3,
  groups
);

/* The middle one is the host the fixture's scope already knew about, which the scan never saw. */
const [portal, known, snmp] = groups[0].hosts;
check('ports come through in order', portal.ports.join() === '22,80,443', portal.ports);
check(
  'an unusual protocol is spelled out and tcp is not',
  snmp.ports.join() === '161/udp',
  snmp.ports
);

check(
  'a host with both addresses prints both, so the per-host view can match it',
  hostLine(portal) === 'acme-portal (203.0.113.10) — 22, 80, 443',
  hostLine(portal)
);
check(
  'a host with only an address prints that',
  hostLine(snmp) === '203.0.113.12 — 161/udp',
  hostLine(snmp)
);
check(
  'ports can be left off',
  hostLine(portal, { withPorts: false }) === 'acme-portal (203.0.113.10)'
);

check(
  'a subset leads with the count',
  scopeHostsHtml([portal], { total: 40 }).startsWith('<p>1 of the 40 hosts in scope:</p><ul><li>'),
  scopeHostsHtml([portal], { total: 40 })
);
check(
  'and the whole scope says so instead',
  scopeHostsHtml(groups[0].hosts, { total: 3 }) ===
    '<p>All 3 hosts in scope:</p><ul>' +
      `<li>${hostLine(portal)}</li><li>${hostLine(known)}</li><li>${hostLine(snmp)}</li></ul>`,
  scopeHostsHtml(groups[0].hosts, { total: 3 })
);
check(
  'a host the scan never touched still lists its addresses, with no ports to show',
  hostLine(known) === 'legacy.acme.example (198.51.100.5)',
  hostLine(known)
);
const ampersand = scopeHostsHtml([{ ...portal, hostname: 'a&b', ip: '', ports: [] }], { total: 9 });
check(
  'a banner with an ampersand cannot break the markup it lands in',
  !/&(?!amp;)/.test(ampersand),
  ampersand
);
check('nothing picked writes nothing', scopeHostsHtml([], { total: 9 }) === '');
