...
/**
 * The enumeration tree: how the ground was mapped, in the order it was mapped.
 *
 * Nested rather than flat, because that is the shape the tab draws and the shape the report prints:
 * a section is a question, and the steps under it are the tools that answered it. Three of them
 * carry `ledTo`, naming the finding they produced, which is the link a client asks about in the
 * readout and the one thing a flat log of commands can never show.
 *
 * The output is written the way the tools actually print it, trimmed to the lines somebody would
 * keep. Invented hosts under .example, as everywhere else in here.
 */
export const DEMO_ENUMERATION = [
  {
    title: 'Perimeter and subdomains',
    phase: 'recon',
    summary: 'What is exposed under the client domain, before touching the portal itself.',
    children: [
      {
        title: 'Passive subdomain sweep',
        tool: 'amass',
        target: 'northwind-logistics.example',
        command: 'amass enum -passive -d northwind-logistics.example -o subs.txt',
        ranAt: '21 Jul 2026, 09:12',
        status: 'completed',
        output: [
          'portal.northwind-logistics.example',
          'www.northwind-logistics.example',
          'mail.northwind-logistics.example',
          'vpn.northwind-logistics.example',
          'api.northwind-logistics.example',
          'staging-portal.northwind-logistics.example',
          'legacy-edi.northwind-logistics.example',
          '',
          '7 names, 4 of them in the scope document. staging-portal is not, and was',
          'raised with the client the same morning (see Questions).',
        ].join('\n'),
      },
      {
        title: 'Which of them answer, and as what',
        tool: 'httpx',
        target: '7 names',
        command: 'httpx -l subs.txt -sc -title -tech-detect -follow-redirects',
        ranAt: '21 Jul 2026, 09:31',
        status: 'completed',
        author: 1,
        output: [
          'https://portal.northwind-logistics.example         [200] [Northwind Shipment Portal] [nginx:1.24.0,Express,React]',
          'https://www.northwind-logistics.example            [301] [] [nginx:1.24.0]',
          'https://api.northwind-logistics.example            [401] [] [nginx:1.24.0,Express]',
          'https://staging-portal.northwind-logistics.example [200] [Northwind Shipment Portal (staging)] [nginx:1.24.0]',
          'https://vpn.northwind-logistics.example            [200] [Login] [Fortinet]',
          '',
          'staging-portal serves the application with nothing in front of it.',
        ].join('\n'),
      },
    ],
  },
  {
    title: 'The portal',
    phase: 'recon',
    summary: 'As the customer account the client provided, and then without it.',
    children: [
      {
        title: 'Content discovery under /api',
        tool: 'ffuf',
        target: 'https://portal.northwind-logistics.example/api/',
        command:
          "ffuf -u https://portal.northwind-logistics.example/api/FUZZ -w raft-medium-words.txt -mc 200,201,204,301,401,403 -H 'Cookie: nw_session=<customer>'",
        ranAt: '22 Jul 2026, 11:04',
        status: 'completed',
        output: [
          'documents               [Status: 200, Size: 4188, Words: 91]',
          'invoices                [Status: 200, Size: 2611, Words: 64]',
          'shipments               [Status: 200, Size: 9042, Words: 220]',
          'users                   [Status: 403, Size: 84,   Words: 3]',
          'admin                   [Status: 403, Size: 84,   Words: 3]',
          'debug                   [Status: 200, Size: 1204, Words: 38]',
          '',
          ':: Progress: [29999/29999] :: 412 req/sec :: Errors: 0 ::',
        ].join('\n'),
      },
      {
        title: 'The document endpoint, with the session removed',
        tool: 'curl',
        target: '/api/documents/8814',
        command: 'curl -is https://portal.northwind-logistics.example/api/documents/8814 | head -n 14',
        ranAt: '22 Jul 2026, 11:26',
        status: 'completed',
        ledTo: ['Shipment documents readable without authentication'],
        output: [
          'HTTP/2 200',
          'content-type: application/pdf',
          'content-disposition: inline; filename="waybill-8814.pdf"',
          'content-length: 214417',
          'x-request-id: 7d2f0c1a',
          '',
          'No cookie, no bearer token, no referer check. The id is sequential:',
          '8813 and 8815 belong to two other customers.',
        ].join('\n'),
      },
      {
        title: 'A customer id swapped on the invoice endpoint',
        tool: 'Burp Repeater',
        target: '/api/invoices?customer=',
        command: 'GET /api/invoices?customer=4471   (the session belongs to customer 4468)',
        ranAt: '22 Jul 2026, 14:02',
        status: 'completed',
        author: 1,
        ledTo: ['A customer can read another customer’s invoices'],
        output: [
          'HTTP/2 200',
          'content-type: application/json',
          '',
          '{"customer":4471,"invoices":[{"number":"NW-2026-0338","total":18240.55,',
          '"po":"PO-88213"},{"number":"NW-2026-0341","total":2190.00,"po":"PO-88240"}]}',
          '',
          'The session is checked. Ownership of the customer parameter is not.',
        ].join('\n'),
      },
    ],
  },
  {
    title: 'Mail perimeter',
    phase: 'recon',
    children: [
      {
        title: 'What the mail host negotiates',
        tool: 'testssl.sh',
        target: 'mail.northwind-logistics.example:25',
        command: 'testssl.sh --starttls smtp --protocols mail.northwind-logistics.example:25',
        ranAt: '23 Jul 2026, 10:18',
        status: 'completed',
        author: 1,
        ledTo: ['Mail perimeter accepts TLS 1.0'],
        output: [
          ...
          'TLS 1.3   offered (OK)',
          '',
          'Inbound only, and no cipher weaker than AES-128 was accepted.',
        ].join('\n'),
      },
      {
        ...
        tool: 'dig',
        target: 'northwind-logistics.example',
        command: 'dig +short TXT northwind-logistics.example _dmarc.northwind-logistics.example',
        ranAt: '23 Jul 2026, 10:35',
        status: 'nothing',
        output: [
          '"v=spf1 include:_spf.nw-mail.example -all"',
          '"v=DMARC1; p=reject; rua=mailto:dmarc@northwind-logistics.example"',
          '',
          'All three in place and strict. Nothing to report.',
        ].join('\n'),
      },
    ],
  },
...
/**
 * Two screenshots left in the evidence bin.
 *
 * The bin is where a tester parks a capture during the test, before there is a finding to put it in.
 * A demo where it is empty says the feature does not exist. These two are deliberately referenced by
 * no finding: that is what being in the bin means.
 */
export const DEMO_BIN = [
  { name: 'ffuf-api-endpoints', accent: [79, 161, 255], lines: 12 },
  { name: 'smtp-starttls-versions', accent: [248, 224, 90], lines: 7 },
];

/**
 * One delivery, recorded.
 *
 * The register answers "which version does the client actually have" months later, and it can only
 * demonstrate that if there is a row in it. The hash is not written here: the seed hashes the
 * document it has just rendered, so the record is true about a file on the disk.
 */
export const DEMO_DELIVERY = {
  version: '1.0',
  channel: 'email',
  kind: 'docx',
  note: 'Sent inside an AES-256 archive. The passphrase went to Marijke by phone and is stored nowhere.',
};
