...
/**
 * How many of the document's pictures are evidence.
 *
 * Not every drawing: the severity charts and the signature images are composed by the app, carry
 * `data-figure="no"` on the way in, and are deliberately left unnumbered. They are identified here
 * by the alt text the app itself writes, which is the only thing that survives into the package.
 */
function evidenceImages(xml) {
  /*
   * `wp:docPr` rather than any `descr=`: the same description is written twice per drawing, once
   * on the inline frame and once on the picture inside it, so matching the attribute alone counted
   * every image twice.
   */
  return [...xml.matchAll(/<wp:docPr\b[^>]*descr="([^"]*)"/g)].filter(
    ([, alt]) => alt !== 'Findings by severity' && !alt.startsWith('Signature of')
  ).length;
}
...
const checks = [
   ...
   */
  [
    'every piece of evidence is numbered, in one sequence across findings and enumeration',
    (doc.match(/<w:t xml:space="preserve">Figure <\/w:t>/g) ?? []).length === evidenceImages(doc),
    `${(doc.match(/<w:t xml:space="preserve">Figure <\/w:t>/g) ?? []).length} captions for ${evidenceImages(doc)} pieces of evidence`,
  ],
  [
    // The other half of the same rule, and the one that used to be wrong: a signature captioned
    // "Figure 8" in the document somebody signs, and a chart pushing every real figure along by two.
    'and the pictures the app draws itself are not',
    (doc.match(/<w:drawing>/g) ?? []).length > evidenceImages(doc),
    'the fixture rendered no charts or signatures, so this proves nothing',
  ],
  [
    'and they are numbered 1..n with none repeated',
    (() => {
      const numbers = [...doc.matchAll(/ SEQ Figure \\\* ARABIC "><w:r><w:t>(\d+)<\/w:t>/g)].map((m) =>
        Number(m[1])
      );
      /* Derived rather than written down, so adding a picture to the fixture cannot break it. */
      return numbers.join() === numbers.map((_, index) => index + 1).join();
    })(),
