...
/**
 * The same render, handing back the header and footer parts instead of the body.
 *
 * Its own helper because the question is different: a tag in `word/document.xml` is processed by
 * anything that can render at all, while a tag in `word/header1.xml` is only processed if
 * docxtemplater is walking those parts too. If it is not, a marking in a header ships to the client
 * as a literal `{{#isRestricted}}` — which is worse than having no marking.
 */
function renderChrome(buffer, data) {
  ...
  return Object.keys(out.files)
    .filter((name) => /^word\/(header|footer)\d*\.xml$/.test(name))
    .map((name) => out.file(name).asText())
    .join('\n');
}
...
async function main() {
  ...
  /* ------------------------------------------- an inverted section is a section */
  {
    /*
     * `{{^x}}…{{/x}}` opens a block, exactly as `{{#x}}…{{/x}}` does.
     *
     * It used not to be pushed onto the scope stack, and the damage was not to the tags inside it
     * — there are rarely any — but to every tag after it. Its closing tag found nothing by that
     * name on the stack and the tolerant fallback popped whatever *was* there, which on the red
     * team template was the findings loop: from that point on, every tag was judged at the top
     * level, so a finding's own `rich.description` was reported as a tag the app does not have in
     * a template that rendered perfectly. Twenty more were called empty for the same reason.
     */
    const { eachTag, stepScope } = await import('../services/tag-scan.js');
    ...
    const walk = (text) => {
      ...
      for (const found of eachTag(text)) {
        ...
        if (step) out.push({ kind: step.kind, tag: step.tag, scope: step.scope.join('.') });
      }
      return { steps: out, left: [...stack] };
    };

    const both = walk(
      '{{#findings}}{{#cvss.impact}}{{ .cvss.impact }}{{/cvss.impact}}' +
        '{{^cvss.impact}}Not published{{/cvss.impact}}{{@rich.description}}{{/findings}}'
    );
    const rich = both.steps.find((step) => step.tag === 'rich.description');
    check(
      'a tag after an inverted section is still inside the loop it is written in',
      rich?.scope === 'findings',
      JSON.stringify(both.steps.map((step) => [step.tag, step.scope]))
    );
    check('and the stack unwinds to nothing by the end', both.left.length === 0, JSON.stringify(both.left));

    const inside = walk('{{#findings}}{{^hasEvidence}}{{ .title }}{{/hasEvidence}}{{/findings}}');
    check(
      'a tag inside an inverted section reports it as its scope',
      inside.steps.find((step) => step.tag === 'title')?.scope === 'findings.hasEvidence',
      JSON.stringify(inside.steps.map((step) => [step.tag, step.scope]))
    );

    const nested = walk('{{^a}}{{^b}}{{ .x }}{{/b}}{{ .y }}{{/a}}{{ .z }}');
    check(
      'inverted sections nest like any other',
      nested.steps.find((step) => step.tag === 'x')?.scope === 'a.b' &&
        nested.steps.find((step) => step.tag === 'y')?.scope === 'a' &&
        nested.steps.find((step) => step.tag === 'z')?.scope === '',
      JSON.stringify(nested.steps.map((step) => [step.tag, step.scope]))
    );

    /* Still tolerant of a half-written template, which is the normal state of one. */
    const mismatched = walk('{{#findings}}{{^x}}{{/nonsense}}{{ .title }}{{/findings}}');
    check(
      'a mismatched close still does not throw',
      Array.isArray(mismatched.steps) && mismatched.steps.length > 0
    );

    /* The two shipped templates, judged the way the playground judges them. */
    const fsp = await import('node:fs/promises');
    const nodePath = await import('node:path');
    const { fileURLToPath } = await import('node:url');
    const here = nodePath.dirname(fileURLToPath(import.meta.url));
    const { lintTemplateTags } = await import('../services/template-test.service.js');
    for (const name of ['engy-default-template.docx', 'engy-redteam-template.docx']) {
      /* From this file, not from the working directory: the suite is run from both. */
      const file = nodePath.join(here, '../../storage/templates', name);
      let buffer = null;
      try {
        buffer = await fsp.readFile(file);
      } catch {
        check(`${name} is on disk`, false, 'run its make script');
        continue;
      }
      const result = lintTemplateTags({ buffer, settings: null, user: { role: 'admin' } });
      check(
        `${name}: every tag in it is one the app produces`,
        result.unknown.length === 0,
        result.unknown.map((row) => `${row.tag} in "${row.where}"`).join(', ')
      );
    }
  }
...
/* -------------------------------------------------------------------------- */
/* The handling marking, on the page                                          */
/* -------------------------------------------------------------------------- */
async function classificationMarking() {
  log.info('');
  log.info('The handling marking');

  const fs2 = await import('node:fs/promises');
  const path2 = await import('node:path');
  const { env: env2 } = await import('../config/env.js');

  const file = path2.join(env2.storage.templates, 'engy-default-template.docx');
  const buffer = await fs2.readFile(file).catch(() => null);
  if (!buffer) {
    log.warn('  SKIPPED — no shipped template in storage. Run `npm run make:template`.');
    return;
  }
  ...
  const standard = renderChrome(buffer, { ...base, isRestricted: false });
  const restricted = renderChrome(buffer, { ...base, isRestricted: true });

  /*
   * The tag has to have been *processed*, whatever it evaluated to. A header still carrying its own
   * braces is the failure this block exists for, and it would otherwise look exactly like a
   * standard engagement correctly printing nothing.
   */
  check(
    'the shipped template’s header and footer are rendered at all',
    !standard.includes('isRestricted') && !standard.includes('{{'),
    'a tag survived into the header or footer unprocessed'
  );
  check(
    'a restricted engagement is marked RESTRICTED on the page',
    restricted.includes('RESTRICTED'),
    'the marking is not in the header or footer'
  );
  check(
    'and a standard one is not',
    !standard.includes('RESTRICTED'),
    'every report is marked restricted'
  );
  check(
    'while the firm’s own footer marking is left alone either way',
    standard.includes('CONFIDENTIAL') && restricted.includes('CONFIDENTIAL'),
    'the existing footer marking was replaced rather than added to'
  );
  check(
    'and the client and the engagement type are still in the header',
    standard.includes('Northwind Logistics') && standard.includes('Web application'),
    'the header lost what it had'
  );
}
