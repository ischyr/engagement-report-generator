...
async function issueSession(res, user, req, existingSid = null) {
  ...
  /*
   * And the one a WebSocket handshake can carry, scoped to `/api/collab` alone. Set here rather
   * than fetched later so a page open since before this existed picks it up on its next token
   * refresh, without anybody being made to sign in again.
   */
  res.cookie(COLLAB_COOKIE, signCollabToken(user, sid), collabCookieOptions());
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
     ...
     */
    let branding = { appName: 'Engy • Report Generation', tagline: 'Engagement Reporting', logo: '' };
    try {
...
/*
 * The API tokens somebody holds, beside the sessions they hold. Its own file because the two
 * lists answer different questions and only one of them has scopes, engagements and a secret that
 * exists for a single response.
 */
router.use('/tokens', apiTokensRoutes);
...
router.put(
  ...
  validate(
    z.object({
      ...
      /** Whether notifications also arrive as mail. Does nothing until a server is configured. */
