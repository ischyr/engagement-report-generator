...
import { docxToPdf, pdfConfig } from '../services/pdf.service.js';
import { OPENED_WITH, encryptedZip, suggestPassphrase } from '../services/zip-crypt.service.js';
import PizZip from 'pizzip';
import { INTRUSION_ACTIONS } from '../models/audit.model.js';
import { credentialUsage } from '../services/credential-usage.service.js';
import { safeDocName } from '../services/docx-render.service.js';
import {
  PUBLIC_KEY_FILENAME,
  signBytes,
  signingConfigured,
...
import { buildTrackerCsv, TRACKERS } from '../services/tracker-export.service.js';
import { findEverywhere, replaceEverywhere } from '../services/replace.service.js';
...
const findingSchema = z.object({
  ...
  /** Normalised exactly as the engagement's are, by the same rule in the same words. */
  tags: z
...
const POPULATE = [
  ...
  /* Whose write-up each one is, so the list can say so without a second request. */
  { path: 'findings.assignedTo', select: 'username firstname lastname' },
  /*
   * Who moved a finding's status, and when — the Retest tab's "verified by Alice on the 3rd".
   *
   * Added to the shared populate rather than fetched per row, having weighed it against the work
   * that made this payload small: a status history is one to three entries on a finding that has
   * moved at all, and a populated name is about a hundred bytes, so forty findings cost single-digit
   * kilobytes against a payload of a hundred and forty. That is worth paying for the one attribution
   * a retest argument actually turns on — "we checked it" is only a fact if it says who.
   */
...
/**
 * What every projected read must carry, whatever it asked for.
 *
 * Three jobs, and leaving any of them out fails in a different way:
 *
 *   - **the guards.** `creator`, `collaborators`, `reviewers` and `memberUntil` are what decides
 *     whether the caller may see this at all; `classification` is what `assertMayOpen` reads;
 *     `deletedAt` is what hides the trash. A projection missing one of these would not error — it
 *     would quietly answer "you are not on this engagement" to somebody who is, or worse.
 *   - **validation.** `name` is `required` on the schema, and Mongoose validates the whole
 *     document on `save()`. A projected document saved without it fails validation on a field the
 *     route never touched.
 *   - **the conflict check.** `updatedAt` and `state` are read by the write paths to refuse a
 *     stale edit and to refuse an approved one.
 *
 * So `only` adds to this rather than replacing it, and a caller cannot accidentally project it
 * away.
 */
const AUDIT_BASE_FIELDS =
  'name creator collaborators reviewers memberUntil classification classifiedBy deletedAt state updatedAt createdAt';

/**
 * Loads the engagement behind `:id`, after checking the caller may have it.
 *
 * `only` names the arrays and fields this route actually needs, e.g. `only: 'intrusions'`. Without
 * it the whole document is read, which is what every one of these calls used to do: opening the
 * Notes tab of an engagement read its findings, its 53 checklist items, its enumeration tree and
 * its sections in order to answer with four notes. On the largest engagement on the instance this
 * was written against that is 53 kB to send 1 kB, and the ratio gets worse as an engagement fills
 * up rather than better.
 *
 * A projected document is still a real Mongoose document and still saves correctly — only the
 * paths that were modified are written — which is why `AUDIT_BASE_FIELDS` above has to include
 * what `save()` validates.
 *
 * @param {object} req
 * @param {{populate?: boolean, includeDeleted?: boolean, only?: string|string[]}} [options]
 */
async function loadAudit(req, { populate = true, includeDeleted = false, only = null } = {}) {
  const query = Audit.findById(req.params.id);
  /*
   * `!= null` rather than a truthiness check, and that is not pedantry: `only: ''` is how a route
   * says "the base fields and nothing else", which is the *most* aggressive projection and the one
   * with the biggest win — those handlers read the whole engagement to authorise a query against
   * another collection. Written as `if (only)` it silently did nothing for exactly those routes.
   */
  if (only != null) {
    const wanted = Array.isArray(only) ? only.join(' ') : String(only).trim();
    query.select(wanted ? `${AUDIT_BASE_FIELDS} ${wanted}` : AUDIT_BASE_FIELDS);
  }
...
/* `nextIdentifier` and `assertEditable` now live in engagement-write.service.js, imported above:
 * `/api/v1` needs the same two rules, and a second copy of either is a report with two findings
 * numbered VULN-04 or a script editing an engagement somebody has already signed off. */
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const audits = await Audit.find(filter)
      ...
      .sort({ updatedAt: -1 })
      /*
       * Plain objects from the driver, not documents.
       *
       * The projection above already stops the finding bodies crossing the wire. What was left was
       * the cost of *hydrating* what does come back: Mongoose was casting every field of every
       * engagement and building a subdocument array for every finding, and the very next line threw
       * all of that away with `toObject()` to get back to the plain object it started from. On a
       * register of a couple of hundred engagements that is thousands of subdocuments constructed
       * to be discarded. Nothing here writes, and neither `findingSeverity` nor `engagementHealth`
       * needs a document — they read fields.
       */
      .lean();

    // Summarise findings so the list can show severity chips without the payload.
    const rows = audits.map((object) => {
...
/**
 * Every tag in use on a *finding*, with how many carry it.
 *
 * Instance-wide rather than per engagement, and scoped the same way the engagement tags are: the
 * point of a suggestion list is that the same idea is not spelled three ways across three tests, so
 * it has to reach past the one being written. A tag only used on work you cannot see is not offered,
 * because the suggestion itself would say that work exists.
 *
 * Declared with the other literal path, before anything parameterised, or Express reads
 * "finding-tags" as an engagement id.
 */
router.get(
  '/finding-tags',
  asyncHandler(async (req, res) => {
    const rows = await Audit.aggregate([
      ...
      { $unwind: '$findings' },
      { $unwind: '$findings.tags' },
      { $group: { _id: '$findings.tags', count: { $sum: 1 } } },
      { $sort: { count: -1, _id: 1 } },
      { $limit: 200 },
    ]);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const audits = await Audit.find({ deletedAt: { $ne: null }, ...visible })
      /*
       * `findings._id` rather than `findings`: this list shows a *count*, and asking for the whole
       * array meant every trashed engagement's write-ups — description, observation and proof of
       * concept, the longest HTML in the database — were read, hydrated and then reduced to a
       * number. Ids are all a count needs.
       */
      .select('name reference auditType deletedAt deletedBy company findings._id')
      .populate([
    ...
    res.json({
      retentionDays,
      audits: audits.map((object) => {
        const elapsed = (Date.now() - new Date(object.deletedAt).getTime()) / 86_400_000;
        return {
          ...object,
...
router.get(
  '/:id/activity',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false, includeDeleted: true, only: '' });
...
router.get(
  '/:id/questions',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false, only: 'questions' });
...
router.get(
  '/:id/locks',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false, only: 'findings' });
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    /*
     * Whose it is has to be somebody on the engagement.
     *
     * Checked before the diff below, so `changed` compares against a value that has already been
     * normalised — an empty string and a null both mean "nobody", and one of them would otherwise
     * look like a change every time the editor saved.
     */
    if (patch.assignedTo !== undefined) {
      patch.assignedTo = patch.assignedTo
        ? assertAssignable(audit, patch.assignedTo, 'a finding')
        : null;
    }
    ...
    if (changed.includes('remediationStatus')) {
       ...
       */
      /*
       * All of it, including what they wrote and what they sent.
       *
       * The reset predated both: leaving a note and a list of attachment ids behind an empty status
       * would hide them from every screen while keeping them in the document, which is the kind of
       * data nobody remembers is there until it appears somewhere it should not.
       *
       * The files themselves stay in the evidence bin, marked as the client's. They are evidence
       * somebody sent us and deleting them because a status moved would be losing the only record
       * of what was claimed.
       */
      finding.clientClaim = { status: '', at: null, by: '', note: '', media: [] };
    }
    ...
    const handedOver = changed.includes('assignedTo') && Boolean(finding.assignedTo);
    ...
    await audit.save();
    /*
     * A handover is logged as itself.
     *
     * The same choice the check route makes: one line, and the interesting one. `fields` still
     * lists everything that moved, so a save that changed the prose *and* handed it on loses
     * nothing — but the log reads "gave it to Ana" rather than "updated: assignedTo".
     */
    await recordActivity({
      audit,
      actor: req.user,
      action: changed.includes('assignedTo') ? ACTIONS.FINDING_ASSIGNED : ACTIONS.FINDING_UPDATED,
      target: finding.title,
      fields: changed,
      ...(changed.includes('assignedTo')
        ? {
            meta: {
              assigned: Boolean(finding.assignedTo),
              to: await assigneeName(finding.assignedTo),
            },
          }
        : {}),
    });
...
const BULK_ACTIONS = [
  ...
  'transfer',
  /*
   * Adding and removing rather than setting, which is why these are not in the table of scalar
   * fields below. "Set the tags of these nine to `needs retest`" would throw away whatever else
   * each of them was already labelled with — and the whole reason to tag nine at once is that they
   * have something *in addition* in common.
   */
  'tag',
  'untag',
];
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (action === 'tag' || action === 'untag') {
      const tag = String(value ?? '').trim().toLowerCase().slice(0, 40);
      if (!tag) throw badRequest('Say which tag.');

      for (const finding of workable) {
        const current = finding.tags ?? [];
        const has = current.includes(tag);
        if (action === 'tag' ? has : !has) {
          skipped.push({
            id: String(finding._id),
            title: finding.title,
            reason: 'unchanged',
            message: has ? 'It already had that tag.' : 'It did not have that tag.',
          });
          continue;
        }

        /* Twenty is the same ceiling one at a time has; a batch is not a way round it. */
        if (action === 'tag' && current.length >= 20) {
          skipped.push({
            id: String(finding._id),
            title: finding.title,
            reason: 'full',
            message: 'It already has twenty tags.',
          });
          continue;
        }

        finding.set({
          tags: action === 'tag' ? [...current, tag] : current.filter((entry) => entry !== tag),
        });
      ...
      if (changed.length) {
        ...
        await recordActivity({
          ...
          target: `${changed.length} finding${changed.length === 1 ? '' : 's'}`,
          fields: ['tags'],
          meta: { tag, added: action === 'tag', count: changed.length },
        });
      }
      ...
      return res.json({
        ...
        changed,
        skipped,
        findings: changed.map((row) => row.id),
      });
    }
    ...
    const FIELDS = {
      ...
      /*
       * Splitting the writing up, which is the whole reason this action exists: "you take these
       * nine, I will do the rest" is one decision, and it was forty dialogs or a conversation
       * nobody could look up afterwards.
       */
      assign: {
        field: 'assignedTo',
        read: () => (value ? assertAssignable(audit, String(value), 'a finding') : null),
      },
      complexity: {
    ...
    if (changed.length) {
      ...
      await recordActivity({
        audit,
        actor: req.user,
        action: action === 'assign' ? ACTIONS.FINDING_ASSIGNED : ACTIONS.FINDING_UPDATED,
        target: `${changed.length} finding${changed.length === 1 ? '' : 's'}`,
        fields: [field],
        meta: {
          bulk: true,
          count: changed.length,
          value: next,
          ...(action === 'assign' ? { assigned: Boolean(next), to: await assigneeName(next) } : {}),
    ...
    /*
     * One bell for the whole batch. Twenty notifications for one decision is how a bell stops
     * being read — see `notifyFindingAssigned`, which takes the list rather than a finding.
     */
    if (action === 'assign' && next && changed.length) {
      await notifyFindingAssigned({
        user: next,
        actor: req.user,
        audit,
        titles: changed.map((row) => row.title),
      });
    }
...
router.get(
  '/:id/handovers',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false, only: 'handovers' });
...
/* -------------------------------------------------------------------------- */
/* Intrusive: what the test changed                                           */
/* -------------------------------------------------------------------------- */

/**
 * What was created, modified or deleted on somebody else's system.
 *
 * Testing is not read-only. You disable a control to see what happens, you PATCH an object you do
 * not own to prove that you can, you upload a file to find out whether it is executed. All of that
 * is the work, and none of it was written down anywhere the client could be told about it or the
 * team could clean it up: it lived in somebody's shell history and a message that began "sorry".
 *
 * The three fields that make this more than a notes tab are `before`, `after` and `revertedAt`.
 * The first is what somebody types back in; the last is what makes an outstanding change findable
 * at the end of a job rather than at the start of an awkward phone call. Preflight refuses to
 * approve a report while any reversible change is still standing.
 */
const intrusionBody = z.object({
  title: z.string().trim().max(200).optional(),
  action: z.enum(INTRUSION_ACTIONS).optional(),
  target: z.string().trim().max(500).optional(),
  host: z.string().trim().max(200).optional(),
  before: z.string().max(8000).optional(),
  after: z.string().max(8000).optional(),
  revertPlan: z.string().max(2000).optional(),
  notes: z.string().optional(),
  at: z.string().trim().max(120).optional(),
  authorisedBy: z.string().trim().max(160).optional(),
  reversible: z.boolean().optional(),
  print: z.boolean().optional(),
  ledTo: z.array(z.string()).max(50).optional(),
  credentialsUsed: z.array(z.string()).max(20).optional(),
...
router.get(
  '/:id/intrusions',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false, only: 'intrusions' });
    await audit.populate([
      { path: 'intrusions.author', select: 'username firstname lastname' },
      { path: 'intrusions.revertedBy', select: 'username firstname lastname' },
    ]);
    res.json(audit.intrusions ?? []);
  })
);

router.post(
  '/:id/intrusions',
  validate(intrusionBody),
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false });
    assertEditable(audit, req.user);
    ...
    audit.intrusions.push({
      ...
      author: req.user._id,
      updatedBy: req.user._id,
      /* Recorded in the order it happened, which is the order it is read and undone in. */
      order: Math.max(0, ...(audit.intrusions ?? []).map((entry) => entry.order ?? 0)) + 1,
    });
    await audit.save();

    const entry = audit.intrusions.at(-1);
    await recordActivity({
      audit,
      actor: req.user,
      action: ACTIONS.INTRUSION_RECORDED,
      target: entry.title || entry.target || 'a change',
      meta: { action: entry.action, host: entry.host },
    });
...
router.put(
  '/:id/intrusions/:intrusionId',
  validate(intrusionBody.partial()),
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false });
    assertEditable(audit, req.user);

    const entry = audit.intrusions.id(req.params.intrusionId);
    if (!entry) throw notFound('That change is not on this engagement.');
...
/**
 * Put it back, or say it is back.
 *
 * Its own endpoint rather than a field on the update, because it is the one thing on this tab that
 * is an event: somebody went and undid it, at a moment, and that is what the report prints and what
 * preflight is watching. A PUT that happened to include `revertedAt` would let it be set by a form
 * that was only fixing a typo in the title.
 */
router.post(
  '/:id/intrusions/:intrusionId/revert',
  validate(z.object({ note: z.string().max(1000).optional(), undo: z.boolean().optional() })),
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false });
    assertEditable(audit, req.user);
    ...
    /* `undo: true` is for the mistake: marked as put back when it was not. */
    if (req.body.undo) {
      ...
      entry.revertedBy = req.user._id;
      entry.revertNote = req.body.note ?? '';
    }
    ...
    await recordActivity({
      audit,
      actor: req.user,
      action: req.body.undo ? ACTIONS.INTRUSION_RECORDED : ACTIONS.INTRUSION_REVERTED,
      target: entry.title || entry.target || 'a change',
      meta: { undone: Boolean(req.body.undo) },
    });
    res.json(entry);
  })
);
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    /*
     * Deleting the record of a change is not the same as undoing the change, and the log says
     * which happened. A row removed because it was recorded twice is housekeeping; a row removed
     * because the change is inconvenient is the thing this tab exists to prevent.
     */
    const title = entry.title || entry.target || 'a change';
    entry.deleteOne();
    await audit.save();
    await recordActivity({
      audit,
...
/**
 * Which of the borrowed accounts was used for what.
 *
 * Derived on read from the work itself — see `credential-usage.service.js` for why it is not a
 * second array on the credential. No secret is read and no reveal is recorded: asking what an
 * account was used for is not asking for the account.
 */
router.get(
  '/:id/credentials/usage',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false });
    const credentials = await Credential.find({ audit: audit._id })
      .select('label username url expiresAt')
      .lean();
    res.json(credentialUsage(audit, credentials));
  })
);
...
router.get(
  '/:id/notes',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false, only: 'notes' });
    await audit.populate([
...
router.get(
  '/:id/enumeration/vars',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false, only: 'enumerationVars' });
    res.json(
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    await loadAudit(req, { populate: false, only: '' });
    /*
 ...
 */
/**
 * The name to put in the log line, for whoever a thing was just given to.
 *
 * One query, on an action somebody took deliberately. The alternative was a log that reads
 * "Alice assigned a finding" and leaves the only interesting part of the sentence out — and the
 * assignee cannot come from the loaded engagement, which holds the team as ids.
 */
async function assigneeName(userId) {
  if (!userId) return '';
  const person = await User.findById(userId).select('firstname lastname username');
  return person?.fullname || person?.username || '';
}

function assertAssignable(audit, userId, what = 'a check') {
  ...
  if (!team.includes(String(userId))) {
    throw badRequest(`That person is not on this engagement, so ${what} cannot be theirs.`);
  }
...
router.get(
  '/:id/test-checks',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false, only: 'testChecks' });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    /*
     * And what the chosen template actually prints, so preflight can notice a restricted report
     * whose template carries no marking. Read from the stored analysis rather than by parsing the
     * file: the tags are worked out once when a template is uploaded, precisely so that questions
     * like this one are a lookup.
     */
    const template = audit.template?.detectedTags
      ? audit.template
      : audit.template
...
/* -------------------------------------------------------------------------- */
/* Renaming one thing everywhere it appears                                    */
/* -------------------------------------------------------------------------- */

const replaceSchema = z.object({
  find: z.string().min(2, 'Type at least two characters').max(200),
  replace: z.string().max(200).optional().default(''),
  /* On by default: somebody renaming `api.acme.example` means that exact string. */
  matchCase: z.boolean().optional().default(true),
});

/**
 * Every place the needle appears, before anything is written.
 *
 * The preview is the whole safety of this feature — see `replace.service.js` on why a text
 * substitution across forty fields is not the shape of thing `remember()` can undo. So it names
 * each location, counts the hits and shows them in context, and it counts *what would actually
 * change*: text between tags only, so a hit inside an attribute is neither shown nor touched.
 */
router.post(
  '/:id/replace/preview',
  requireWrite,
  ...
  asyncHandler(async (req, res) => {
    ...
    const bodies = [...(await loadEnumerationBodies(audit._id)).values()].map((row) => ({
      step: (audit.enumeration ?? []).find((entry) => String(entry._id) === String(row.step)),
      content: row.content ?? '',
    }));

    res.json(findEverywhere(audit, bodies, req.body));
  })
);
 ...
 * Does it.
 *
 * `expectedTotal` comes back from the preview and is checked against a fresh count, so a rename
 * cannot run against an engagement that changed while somebody was reading the list. The same
 * argument as `expectedUpdatedAt` on a section: an optimistic check is cheap, and the alternative
 * is a bulk edit applied to data nobody looked at.
 */
router.post(
  '/:id/replace',
  requireWrite,
  validate(
    replaceSchema.extend({
  ...
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req);
    assertEditable(audit, req.user);
    if (req.body.find === req.body.replace) throw badRequest('Those are the same text.');

    const bodyRows = [...(await loadEnumerationBodies(audit._id)).values()];
    const bodies = bodyRows.map((row) => ({
      step: (audit.enumeration ?? []).find((entry) => String(entry._id) === String(row.step)),
      content: row.content ?? '',
    }));

    const now = findEverywhere(audit, bodies, req.body);
    if (now.total !== req.body.expectedTotal) {
      throw conflict(
        `This engagement changed while you were looking: ${now.total} match(es) now, not ${req.body.expectedTotal}. Have another look before replacing.`,
        { total: now.total }
      );
    }

    const done = replaceEverywhere(audit, bodies, { ...req.body, user: req.user });

    /*
     * Marked as a details change so an open editor is warned rather than quietly overwritten —
     * the same flag the scope importer sets, and for the same reason.
     */
    if (done.changed) audit.detailsUpdatedAt = new Date();
    await audit.save();

    for (const body of done.bodies) {
      const step = (audit.enumeration ?? []).find((entry) => String(entry._id) === body.step);
      if (step) await saveEnumerationBody(audit, step, { content: body.content });
    }

    await recordActivity({
      audit,
      actor: req.user,
      action: ACTIONS.AUDIT_TEXT_REPLACED,
      target: req.body.find,
      meta: { replaced: req.body.replace, count: done.changed, skipped: done.skipped.length },
    });

    res.json({ changed: done.changed, skipped: done.skipped });
  })
);
...
router.get(
  '/:id/findings/similar',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { only: 'findings' });
 ...
 */
/**
 * Did the rendered document actually print the copy marking?
 *
 * The label is offered to the template and the template decides. So the only honest way to know
 * is to look in the bytes that came out: unzip the parts a marking could be in and see whether the
 * words are there. Word splits text across runs freely, so the XML tags are stripped before the
 * search, the same trick the tag extractor uses.
 *
 * A false answer is not an error. It means the copy is traceable through its properties and the
 * register but says nothing on the page, and the caller passes that on so nobody assumes a
 * watermark that is not there.
 */
function reportSaysCopy(buffer, label) {
  ...
  try {
    ...
    /* The recipient's name is the part worth finding; the rest of the label is house wording. */
    const needle = label.split(' \u00b7 ')[0].replace(/\s+/g, ' ').trim();
    return parts.some((name) =>
      zip.file(name).asText().replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').includes(needle)
    );
  } catch {
    return false;
  }
...
async function renderReportFile(req, audit, { templateId = '', copy = null } = {}) {
  ...
  const { buffer, filename, provenance } = await generateReport({
    ...
    // And "the API host was added on the 14th, agreed by Dana" — with the engagement's own start
    // date, so the narrative can say "Day 3", which is how the work is actually argued about.
    scopeChanges: await scopeChangesFor(
      audit._id,
      (value) => formatDate(value, resolveReportSettings(settings, audit.company).dateFormat),
      { startedOn: audit.date_start }
    ),
    ...
    /*
     * Who this copy is for, on the copies that are for somebody.
     *
     * Threaded through rather than applied afterwards, because a marking added to finished bytes
     * would be a second layout path: the template decides where a per-recipient line belongs, and
     * it can only do that while it is still a template. See report.service.js.
     */
    copy,
    /*
     * The borrowed accounts, for the appendix that says what was done with them.
     *
     * Only the fields a report may print. The secret is not selected, so it cannot reach a
     * template even by accident: the appendix is about accountability, not about the password.
     */
    credentials: await Credential.find({ audit: audit._id })
      .select('label username url expiresAt')
      .lean(),
   ...
   */
  /*
   * The detached signature over exactly these bytes, when this instance has a key.
   *
   * Computed here so that every path out of this function is signed, and never allowed to fail the
   * download: a client waiting for a report should not be told "no" because a signature could not
   * be produced. When it fails the report goes out unsigned and the log says why, which is a
   * missing convenience rather than a missing document.
   */
  let signature = null;
  if (signingConfigured(settings)) {
    try {
      signature = signBytes(buffer, settings);
    } catch (error) {
      log.warn(`Could not sign ${filename}: ${error.message}`);
    }
  }
  ...
  await RenderRecord.create({
    ...
    copy: provenance.copy
      ? {
          ...
          number: provenance.copy.number,
          of: provenance.copy.of,
          /*
           * Whether the marking is on the page or only in the properties.
           *
           * Asked of the rendered bytes rather than assumed: the label is exposed to the template
           * and a template that does not use it produces a copy that is traceable and silent. The
           * caller is told which happened, because "marked" and "traceable" are different promises
           * and only one of them is visible to the person holding the file.
           */
          marked: reportSaysCopy(buffer, provenance.copy.label),
        }
      : undefined,
  }).catch((error) => log.warn(`Could not record the render of ${filename}: ${error.message}`));

  return { buffer, filename, provenance, template, settings, signature };
}

/**
 * The same report, as a PDF, for looking at rather than for sending.
 *
 * Declared above `/:id/report` because `report.pdf` is a different path and a reader deserves to
 * find the two together — and because what it does is one line different: it renders exactly what
 * Generate renders, and then converts those bytes. The preview is evidence about the file that
 * would be sent only if it *is* that file; a second layout path would make it a picture of
 * something else.
 *
 * Deliberately not recorded as a delivery-shaped event. Looking at a report is not generating one,
 * and a register that filled up with previews would stop being a record of what went to a client —
 * the render itself is still logged by `renderReportFile`, which is the fact worth keeping.
 */
/**
 * How often one account may ask for a whole report.
 *
 * Generating is seconds of one Node thread: the template is filled, every figure renumbered, every
 * screenshot inlined, and for a PDF an external converter is started on top of that. Nothing else
 * on this server is in that league, and there was no limit on either — so somebody leaning on the
 * Render button while a template misbehaves is an accidental denial of service against their own
 * instance, and every colleague waits.
 *
 * Keyed by account rather than by address, which matters more here than anywhere else: a firm behind
 * one office IP would otherwise share a single allowance, and the first person to tune a template
 * would lock out the rest of the floor.
 *
 * Generous, because the honest use of this is a loop — generate, look at page four, fix the
 * template, generate again. Twenty in five minutes is faster than anybody reads a report and far
 * slower than a stuck button.
 */
const renderLimiter = rateLimit({
  ...
  message: {
    error:
      'That is a lot of reports in a short time. Give it a few minutes — each one is a full render, and they queue behind each other for everybody.',
  },
});
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const settings = await Settings.getSettings();
    const config = pdfConfig(settings);
    /* Asked before the document is built: a two-minute render that ends in "no converter" is rude. */
    if (!config.enabled) {
      throw badRequest(
    ...
    const audit = await loadAudit(req);
    const { buffer, filename } = await renderReportFile(req, audit, {
      templateId: req.query.template,
    });

    const pdf = await docxToPdf(buffer, { config });
    log.info(`Converted ${filename} to PDF with ${config.label} in ${pdf.ms}ms`);

    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Length', pdf.buffer.length);
    /* Inline: this exists to be shown in a viewer, and a preview that downloads is not a preview. */
    res.setHeader(
      'Content-Disposition',
      contentDisposition(filename.replace(/\.docx$/i, '.pdf'), { type: 'inline' })
    );
    res.end(pdf.buffer);
  })
);
...
/**
 * One copy per recipient, each marked with the name of the person it is for.
 *
 * ## What this is for
 *
 * A report that turns up somewhere it should not. The delivery register already answers "is this
 * the file we sent" from the hash, and stops there: four people were sent the same bytes, so the
 * answer is four names. Making each recipient their own copy turns that into one name, which is
 * the difference between suspecting a leak and knowing where it came from.
 *
 * It is a deterrent rather than a control. Anybody can open a .docx and delete a line, and this
 * makes no attempt to stop them: the marking is in the page and in the document's properties, and
 * both can be removed by somebody who thinks to. What it defends against is the ordinary case,
 * which is a file forwarded without a thought.
 *
 * ## Why the whole report is rendered again per recipient
 *
 * Because the alternative is editing finished bytes. The marking belongs wherever the house style
 * puts it, which the template decides while it is still a template; stamping a line into a
 * rendered document afterwards would mean a second layout path, in this file, with an opinion
 * about the top of somebody's page. Four renders of a report is a few seconds.
 *
 * Each copy is its own render with its own id, its own signature and its own row, because each is
 * a different document. That is what makes the hash lookup unambiguous.
 *
 * ## Marked, and traceable
 *
 * Two different promises, and the response keeps them apart. Every copy is **traceable**: the
 * recipient is in its custom properties and its hash is in the register, whatever the template
 * does. A copy is **marked** only if the template used `{{#isCopy}}{{ copyLabel }}{{/isCopy}}`
 * somewhere a reader will see it. A template that does not is not an error, but the operator is
 * told, because otherwise they would believe in a watermark that is not there.
 */
router.post(
  '/:id/report/copies',
  renderLimiter,
  validate(
    z.object({
      /*
       * Named explicitly rather than taken from the engagement's contact list, because who a
       * report is going to this time is a decision made at the moment of sending. A contact may
       * be referenced, and then the row links back to them.
       */
      recipients: z
        .array(
          z.object({
      ...
      /** Recorded as sent, unless you are only making them to look at. */
  ...
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { write: true });
    const { recipients, template: templateId = '', record = true } = req.body;

    const zip = new PizZip();
    const made = [];

    for (const [index, recipient] of recipients.entries()) {
      const { buffer, filename, provenance, signature } = await renderReportFile(req, audit, {
        templateId,
        copy: { recipient: recipient.name, number: index + 1, of: recipients.length },
      });

      /*
       * The recipient's name in the filename as well as in the document.
       *
       * Not decoration: these arrive as four attachments in four emails, and a folder of files all
       * called "Report.docx" is how the wrong one gets sent to the wrong person.
       */
      const name = `${filename.replace(/\.docx$/i, '')} - ${safeDocName(recipient.name)}.docx`;
      zip.file(name, buffer);
      if (signature?.value) {
        zip.file(`${name}.sig`, Buffer.from(signature.value, 'base64'));
      }
      ...
      made.push({
        ...
        copyNumber: index + 1,
        fileHash: provenance.outputHash,
        filename: name,
        renderId: provenance.renderId,
        marked: reportSaysCopy(buffer, provenance.copy?.label ?? ''),
      });
    ...
    /*
     * A manifest inside the archive, in plain text.
     *
     * The register holds this and is the record; this is for the twenty minutes in which somebody
     * is attaching four files to four emails and needs to know which is which without opening
     * them. It is also what makes the archive make sense to whoever finds it in a year.
     */
    zip.file(
      'copies.txt',
      [
        `${audit.name}${audit.reference ? ` (${audit.reference})` : ''}`,
        `${made.length} ${made.length === 1 ? 'copy' : 'copies'}, made ${new Date().toISOString()}`,
        '',
        ...made.map(
          (copy) =>
            `copy ${copy.copyNumber} of ${made.length}  ${copy.name}\n` +
            `  file    ${copy.filename}\n` +
            `  sha256  ${copy.fileHash}\n` +
            `  marked  ${copy.marked ? 'yes, on the page' : 'no, in the document properties only'}`
        ),
        '',
        'Each copy is a different document with a different hash. The delivery register maps each',
        'hash to the person it was made for, so a file found later can be traced to one recipient.',
        '',
      ].join('\n')
    );

    /*
     * Recorded as one delivery with many recipients rather than one delivery each.
     *
     * It is one act of sending: the same version of the same report, on the same day, to a list of
     * people. Splitting it would make the register answer "how many times did this go out" wrongly,
     * and the per-recipient hashes are what needed the row anyway.
     */
    let delivery = null;
    if (record) {
      delivery = await Delivery.create({
        ...
        recipients: made.map(({ client, name, email, copyNumber, fileHash }) => ({
          client,
          name,
          email,
          copyNumber,
        ...
        copies: made.length,
        filename: made[0]?.filename ?? '',
        /* The first copy's, so a register that shows one hash per row shows a real one. */
        fileHash: made[0]?.fileHash ?? '',
        fileSize: null,
        kind: 'docx',
      ...
      await recordActivity({
        ...
        meta: {
          copies: made.length,
          recipients: made.map((copy) => copy.name).join(', '),
        },
      });
    }
    ...
    /*
     * The summary in headers, because the body is an archive.
     *
     * Small and bounded — twenty recipients at most — and it saves the client a second request to
     * find out whether the copies it just downloaded actually say whose they are.
     */
    res.setHeader('X-Engy-Copies', String(made.length));
    res.setHeader('X-Engy-Copies-Marked', String(made.every((copy) => copy.marked)));
    if (delivery) res.setHeader('X-Engy-Delivery', String(delivery._id));
    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Length', archive.length);
    res.setHeader(
      'Content-Disposition',
      contentDisposition(`${safeDocName(audit.reference || audit.name)} - copies.zip`)
    );
    res.end(archive);
  })
);
...
/**
 * The signature for a render, and the two files that make it checkable.
 *
 * A zip rather than three downloads, because these three things are attached to one email
 * together and separately they are three chances to forget one. Nothing secret is in it: a
 * signature, a public key, and instructions.
 *
 * Keyed by render id rather than by engagement, because the question is about a file. The id is in
 * the document's own properties, so somebody holding a mystery .docx can find the right bundle.
 */
router.get(
  '/:id/renders/:renderId/verification.zip',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req);
    const record = await RenderRecord.findOne({
      renderId: req.params.renderId,
      audit: audit._id,
    }).lean();

    if (!record) throw notFound('There is no record of that render.');
    if (!record.signature?.value) {
      throw badRequest(
        'That render was not signed. Either no signing key was configured at the time, or it was ' +
          'made before one was. Generate the report again to get a signed copy.'
      );
    }

    const settings = await Settings.getSettings();
    if (!settings.signing?.publicKey) {
      throw badRequest('The signing key has been removed, so the public half cannot be published.');
    }
    /*
     * Said plainly rather than shipping a bundle that cannot verify. A key rotated since the render
     * leaves old signatures valid only against the old public half, and a client told to check with
     * the new one would conclude the document had been altered.
     */
    if (
      settings.signing.fingerprint &&
      record.signature.fingerprint &&
      settings.signing.fingerprint !== record.signature.fingerprint
    ) {
      throw badRequest(
        'That render was signed with a key that has since been replaced, so the current public key ' +
          'cannot verify it. Generate the report again to sign it with the key in use now.'
      );
    }

    const filename = record.filename || 'report.docx';
    const zip = new PizZip();
    zip.file(`${filename}.sig`, Buffer.from(record.signature.value, 'base64'));
    zip.file(PUBLIC_KEY_FILENAME, settings.signing.publicKey);
    zip.file(
      'HOW-TO-VERIFY.txt',
      verificationNote({
        filename,
        digest: record.outputHash,
        signature: record.signature,
        publicKey: settings.signing.publicKey,
    ...
    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Length', archive.length);
    res.setHeader('Content-Disposition', contentDisposition(`${filename}.verify.zip`));
    res.end(archive);
  })
);

/**
 * The same report, in an archive nobody can open without the password.
 *
 * A pentest report is the most sensitive document a client receives all year and it left here as a
 * plain attachment. This is the other option: the identical bytes Generate produces, wrapped in an
 * AES-256 archive — see `zip-crypt.service.js` for why that format and not another.
 *
 * ## A POST, for a download
 *
 * Unusual, and deliberate. A password cannot travel in a query string: it would be in the browser's
 * history, in any proxy's access log, and — since this server started stamping requests — on the
 * request line of its own log. So it goes in a body, which means a POST, and the file comes back in
 * the response like any other download.
 *
 * The plain route is untouched. Encrypting is a choice made per download rather than a mode the
 * instance is in, because the person who knows whether this one is going by mail to a client or
 * into a folder on the same machine is the person pressing the button.
 *
 * ## What is recorded, and what is not
 *
 * That an archive was made, and its digest. **Never the password.** It is not stored, not hashed,
 * not logged, and cannot be recovered here — if it is lost the report is generated again. Anything
 * else would put the key to the document beside the record of the document, which is the mistake
...
router.post(
  ...
  validate(
    z.object({
      /*
       * Eight is not a serious password and the app should not pretend otherwise — but the ceiling
       * is high because a passphrase from the button is four words and somebody's own may be longer.
       */
      password: z.string().min(8, 'Use at least eight characters — the button suggests a good one.').max(200),
  ...
  asyncHandler(async (req, res) => {
    ...
    /* `report.docx` becomes `report.docx.zip`: what is inside is obvious from the outside. */
    ...
    await recordActivity({
      ...
      meta: {
        ...
        renderId: provenance.renderId,
        templateVersion: provenance.templateVersion,
        /* So the register can say this one left protected, without saying anything more. */
        encrypted: true,
      },
    });
    ...
    /*
     * The digest of the *archive*, because that is the artefact that leaves — it is what a delivery
     * row should record and what a client can check against what they received.
     */
    attachDigest(res, archive);
    res.end(archive);
  })
);

/**
 * A passphrase worth using, for the button beside the box.
 *
 * On the server rather than in the browser because this is the one thing in the flow that must be
 * unguessable, and `crypto.randomInt` over a fixed list is a stronger promise than whatever the
 * page can assemble. It is never stored — it is generated, shown once, and forgotten here.
 */
router.get(
  '/:id/report/passphrase',
  asyncHandler(async (req, res) => {
    await loadAudit(req);
    res.json({ passphrase: suggestPassphrase(), openedWith: OPENED_WITH });
  })
);
...
async function transferOneFinding({ source, target, finding, user, move }) {
  ...
  target.findings.push({
    ...
    comments: [],
    /*
     * And whose it is does not travel either. Being assigned means being on the engagement — the
     * route refuses anybody who is not — so carrying it over would leave a finding owned by
     * somebody who cannot open the engagement it is now on, waiting in a queue they will never see.
     */
    assignedTo: null,
  ...
  asyncHandler(async (req, res) => {
    ...
    if (options.findings) {
      duplicate.findings = (plain.findings ?? []).map(({ _id, ...finding }, index) => ({
        ...
        comments: [],
        /* And nobody has been given it yet: last year's split is not this engagement's. */
        assignedTo: null,
        createdBy: req.user._id,
        updatedBy: req.user._id,
        sortIndex: index,
 ...
 *
 * Every number in a phishing write-up already exists — the tab computes them, the report prints
 * them — and until now the way they reached a finding was somebody reading them off the screen and
 * typing them in. That is where a report that says 38% next to a table saying 31% comes from.
 *
 * So the finding is written from `campaignSummary()`, the same function the other two use, and the
 * three agree by construction. It is a draft in the ordinary sense: an unscored vector, no
 * remediation, and a person to read it — `campaignFinding()` says why there is no remediation.
 *
 * Registered before `/:targetId` because Express matches in order and `finding` is a literal
 * segment. Three routes in this file have already learned that the hard way.
 */
router.post(
  '/:id/phishing/finding',
  requireWrite,
  asyncHandler(async (req, res) => {
    ...
    const rows = await PhishingTarget.find({ audit: audit._id });
    if (!rows.length) {
      throw badRequest('There is no campaign here yet — import the results first.');
    }

    const draft = campaignFinding(campaignSummary(rows));

    /*
     * Drafted twice is almost always a double click or a second look, not a second campaign — and
     * the useful answer to both is the finding that already exists rather than a duplicate of it.
     * The same 409-with-the-clash the library promotion uses.
     */
    const existing = (audit.findings ?? []).find((finding) => finding.title === draft.title);
    if (existing) {
      throw conflict('A finding for this campaign is already here.', {
        findingId: existing._id.toString(),
    ...
    audit.findings.push({
      ...
      /*
       * Unscored, deliberately. A phishing result is not a CVSS vector and guessing one here would
       * put a number in the report that nobody decided — preflight will ask for it, which is the
       * right moment and the right person.
       */
      identifier: await nextIdentifier(audit),
      sortIndex: (audit.findings ?? []).length,
      createdBy: req.user._id,
    ...
    await recordActivity({
      audit,
      actor: req.user,
      action: ACTIONS.PHISHING_FINDING_DRAFTED,
      target: draft.title,
      meta: { findingId: finding._id.toString() },
    });
...
/**
 * The whole operation in one order, for the tab that shows it.
 *
 * Composed here rather than on the client for the reason the detection summary is: the report
 * prints the same chronology out of `buildReportData`, and two implementations of "what happened
 * when" would eventually tell an operator one story and their client another.
 *
 * Times are sent as instants and formatted in the browser, unlike the report's — a page can use
 * the reader's own locale, and a document cannot.
 */
router.get(
  '/:id/timeline',
  asyncHandler(async (req, res) => {
    const audit = await loadAudit(req, { populate: false });
    const [detection, phishing, scopeChanges, deliveries] = await Promise.all([
      detectionFor(audit._id),
      phishingFor(audit._id),
      scopeChangesFor(audit._id, (value) => String(value ?? ''), { startedOn: audit.date_start }),
      deliveriesFor(audit._id, (value) => String(value ?? '')),
    ]);
    ...
    const timeline = operationTimeline({
      ...
      phishing,
      scopeChanges: scopeChanges.scopeChanges,
...
/**
 * Two fields that are not about the delivery: when to retest, and who.
 *
 * On this body rather than a second request because it is one decision made at one moment — "this
 * went out, come back in a month" — and a second call would be a second thing to fail after the
 * register had already been written.
 */
const deliverySchemaBody = z.object({
  /** True books it at the default below; a number overrides that. */
  retest: z.boolean().optional(),
  retestInDays: z.number().int().min(1).max(365).optional(),
  retestUser: z.string().optional(),
...
/**
 * How long after a delivery a retest lands, unless somebody says otherwise.
 *
 * Thirty days is the window most remediation SLAs use for anything below critical, so it is the
 * date somebody would have picked. A constant rather than a setting for now: the number is visible
 * where it is used and every caller may override it, which is most of what a setting would buy.
 */
const RETEST_DEFAULT_DAYS = 30;

/**
 * Puts a retest in the schedule, N days after a report went out.
 *
 * Returns the booking, or null when none was asked for. Never throws into the delivery: the
 * delivery is the fact being recorded and a diary entry that could not be made is not a reason to
 * lose it. A failure is logged and reported back so the interface can say so.
 */
async function bookRetest({ audit, actor, days, user }) {
  const inDays = Number(days);
  if (!Number.isFinite(inDays) || inDays < 1 || inDays > 365) return null;

  /*
   * Whoever is asked for, else whoever ran the job.
   *
   * The creator rather than the whole team: a booking is one person's day, and a retest assigned
   * to four people is a retest nobody has.
   */
  const assignee = user || actor?._id || audit.creator;
  ...
  try {
    const booking = await Booking.create({
      ...
      note: `Retest of ${audit.reference || audit.name}`.slice(0, 300),
    ...
    await recordActivity({
      ...
      target: on,
      meta: { retest: true, days: inDays },
    });
    return booking;
  } catch (error) {
    log.warn(`Could not book the retest for ${audit._id}: ${error.message}`);
    return null;
  }
}
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    /*
     * And the retest, if it was asked for.
     *
     * After the delivery is written, deliberately: the report having gone out is the fact worth
     * keeping, and a schedule that refused would otherwise take the record with it.
     */
    const retest = await bookRetest({
      audit,
      actor: req.user,
      days: req.body.retestInDays ?? (req.body.retest ? RETEST_DEFAULT_DAYS : null),
      user: req.body.retestUser,
    });

    await row.populate(DELIVERY_POPULATE);
    res.status(201).json({
      ...deliverySummary(row),
      /** So the interface can say "retest booked for the 12th" rather than guessing. */
      retest: retest ? { _id: retest._id, on: retest.start, user: retest.user } : null,
    });
  })
);
...
/**
 * The findings as a CSV the client's own tracker will import.
 *
 * The spreadsheet beside this one is for a person; this is for a machine. A remediation team that
 * lives in Jira does not want a tracker of ours, they want tickets in theirs, and somebody was
 * making those by hand out of the .xlsx — losing the report reference on the way, which is what
 * makes the argument six months later impossible.
 *
 * Declared above `/:id/findings/:findingId`: `findings.csv` is one path segment and that route is
 * two, so they cannot collide — but the file is long enough that the next literal path added near
 * the findings routes very much could, and this is where somebody will look for the precedent.
 */
router.get(
  '/:id/findings.csv',
  asyncHandler(async (req, res) => {
    const tracker = String(req.query.tracker ?? 'jira');
    if (!TRACKERS[tracker]) {
      throw badRequest(
        `Choose one of: ${Object.keys(TRACKERS).join(', ')}. Each tracker's importer wants a different shape.`
      );
    }

    const audit = await loadAudit(req);
    const settings = await Settings.getSettings();
    const { text, filename, findings } = buildTrackerCsv({
      audit,
      settings,
      user: req.user,
      history: await findingHistoryFor(audit, req.user),
    ...
    await recordActivity({
      ...
      actor: req.user,
      action: ACTIONS.REPORT_GENERATED,
      meta: { template: `${TRACKERS[tracker].label} export`, findings },
    });

    const buffer = Buffer.from(text, 'utf8');
    /*
     * `text/csv` with the charset stated. No byte-order mark: the consumer is a tracker importer
     * rather than Excel — which is what the .xlsx beside this is for — and a BOM is a stray
     * character at the front of the first header name to everything except Excel.
     */
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', contentDisposition(filename));
    /* A file that goes to a client can be recorded as a delivery, like any other. */
    attachDigest(res, buffer);
    res.end(buffer);
  })
);
