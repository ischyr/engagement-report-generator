...
/*
 * The client's view of their own findings. Mounted before the gate for the same reason intake is —
 * the person it exists for has no account — and the file guards its own management routes.
 */
router.use('/share', shareRoutes);

/*
 * The versioned API for scripts.
 *
 * Mounted above the session gate because it has a gate of its own and the two are deliberately not
 * interchangeable: a session cannot reach `/api/v1`, and an API token cannot reach anything else.
 * `middleware/api-auth.js` argues for that separation; `v1.routes.js` states what v1 promises.
 *
 * It is not the app's own routes with a second credential bolted on. Those change shape whenever
 * the interface does, which is a freedom worth keeping and one you cannot promise to somebody's
...
/*
 * The optional assistant. Answers `available: false` and nothing else on an instance that has not
 * configured one, which is every instance until somebody does.
 */
...
/*
 * Its own path rather than under `/settings`, so nothing depends on the order two routers are
 * mounted in. The card that drives it lives on the Settings page all the same.
 */
