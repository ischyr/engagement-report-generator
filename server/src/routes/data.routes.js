...
import { programmeFor } from '../services/programme.service.js';
...
/**
 * The same client's engagements read as a programme: the direction of travel.
 *
 * The overview above answers "what have we done for them". This answers the question a renewal
 * turns on — is it getting better, what did they fix since last time, and how long did it take.
 * Same query shape, same scoping; everything it returns is derived rather than stored.
 */
router.get(
  '/companies/:id/programme',
  asyncHandler(async (req, res) => {
    const company = await Company.findOne({
      _id: req.params.id,
      ...(await visibleCompanyFilter(req.user)),
    ...
    const audits = await Audit.find(visibleAuditFilter(req.user, { company: company._id }))
      /* The same narrow projection the overview uses, for the same reason: descriptions carry
         screenshots, and none of this reads one. */
      .select(
        'name reference auditType state date date_start date_end createdAt ' +
          'findings._id findings.title findings.cvssv3 findings.remediationStatus findings.vulnerability'
      )
      .lean();

    const programme = programmeFor(audits);
    res.json({
