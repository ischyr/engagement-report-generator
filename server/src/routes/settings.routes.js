...
import { PDF_CONVERTERS, pdfConfig, testPdfConverter } from '../services/pdf.service.js';
import {
  EVENT_GROUPS,
  providerChoices,
  testWebhook,
...
const settingsSchema = z
  .object({
    ...
    /** One switch, and everything about shared editing hangs off it. */
    collab: z.object({ enabled: z.boolean().optional() }).optional(),
    /*
     * `url` and `signingSecret` are write-only, like the mail password: sent to be stored, never
     * sent back. Undefined leaves what is stored alone; an empty string clears it.
     */
    webhooks: z
      .object({
        enabled: z.boolean().optional(),
      ...
      .optional(),
    /**
     * The PDF converter. No secret in it, so no vault and no write-only field — it is a choice of
     * program and where to find it, which is exactly as sensitive as it looks.
     */
    pdf: z
      .object({
        converter: z.enum(['none', 'libreoffice', 'word']).optional(),
...
function forClient(settings, user) {
  ...
  /*
   * Everybody is told whether a PDF can be made, because everybody sees the button that makes one.
   * Which program does it, and where it lives on disk, is an administrator's business.
   */
  const pdf = plain.pdf ?? {};
  const pdfEnabled = Boolean(pdf.converter && pdf.converter !== 'none');
  /* Everybody is told, because every account sees the fields it governs. */
  const collabEnabled = Boolean(plain.collab?.enabled);
  /*
   * And whether reports are signed, because that is a promise a tester makes to a client and there
   * is a button about it on the Delivery tab. The key itself is not in this answer.
   */
  const signingEnabled = Boolean(plain.signing?.enabled && plain.signing?.publicKey);

  if (user?.role !== 'admin') {
    /*
     * Nothing at all about webhooks. Unlike the PDF button or shared editing, this governs no
     * control anybody but an administrator sees — and where a firm's work is being posted is not
     * something to tell every account.
     */
    delete plain.webhooks;
    plain.collab = { enabled: collabEnabled };
    plain.pdf = { enabled: pdfEnabled };
    ...
    plain.signing = { enabled: signingEnabled };
    return plain;
  }

  /*
   * The public half, the fingerprint and when it was made. The private half is deleted rather than
   * masked: an administrator has no use for the ciphertext, and a settings payload that carried it
   * would be a copy of the key in every browser tab that has this page open.
   */
  const signing = plain.signing ?? {};
  delete signing.privateKey;
  plain.signing = {
    ...signing,
  ...
  plain.pdf = { ...pdf, enabled: pdfEnabled, converters: PDF_CONVERTERS };
  plain.collab = { enabled: collabEnabled };
  /* The card reads the detail from `/backup`; this is only so the page knows to draw it. */
  plain.backup = { lastAt: plain.backup?.lastAt ?? null };

  /*
   * The webhook URL never comes back — for Teams and Discord it is the whole authorisation, and an
   * administrator's browser is one more place it could be read from. What comes back is whether one
   * is set and which host it points at, which is everything the form needs to be honest about the
   * state without being a second copy of the credential.
   */
  const webhooks = plain.webhooks ?? {};
  const urlFromEnvironment = Boolean(process.env.WEBHOOK_URL);
  /*
   * Read *before* the deletes below, which is not a stylistic choice: `webhooks` is the same object
   * as `plain.webhooks`, so asking whether a URL is stored after removing it always answered no —
   * and the page then offered to set up a webhook that was already there. The mail block above puts
   * its two flags at the top of the function for the same reason.
   */
  const hasUrl = urlFromEnvironment || Boolean(webhooks.url?.data);
  const hasSigningSecret =
    Boolean(process.env.WEBHOOK_SIGNING_SECRET) || Boolean(webhooks.signingSecret?.data);
  delete webhooks.url;
  delete webhooks.signingSecret;
  plain.webhooks = {
    ...webhooks,
    hasUrl,
    urlFromEnvironment,
    ...
    groups: Object.entries(EVENT_GROUPS).map(([id, group]) => ({
      id,
      name: group.name,
      hint: group.hint,
    })),
...
/**
 * Whether the configured converter is really there.
 *
 * Converts one of the shipped templates rather than asking the program for its version: a version
 * string proves a binary exists and says nothing about whether it can produce a PDF as the account
 * the server runs as, which is where this actually fails. The answer is the converter's own words
 * when it fails, for the same reason the mail test reports the server's own refusal.
 *
 * Takes the settings from the request rather than from the database, so a converter can be proved
 * before it is saved — the same as the mail and assistant tests.
 */
router.post(
  '/pdf/test',
  requireRole('admin'),
  ...
  asyncHandler(async (req, res) => {
    ...
    const config = pdfConfig({ pdf: { ...(stored.toObject().pdf ?? {}), ...req.body } });
    if (!config.enabled) throw badRequest('Choose a converter first.');

    /*
     * A real report template, because that is the shape that actually gets converted: fields,
     * headers, a table of contents and an image. A one-paragraph document would prove far less.
     */
    const sample = await readFile(
      path.join(env.storage.templates, 'engy-default-template.docx')
    ).catch(() => null);
    if (!sample) {
      throw badRequest('The default template is missing — run `npm run make:template` and try again.');
    }

    res.json(await testPdfConverter(config, sample));
  })
);

/* -------------------------------------------------------------------------- */
/* Signing the deliverable                                                    */
/* -------------------------------------------------------------------------- */

/**
 * Generate a signing key, or replace the one there is.
 *
 * A POST rather than part of the settings PATCH, because generating a key is an event and not a
 * field: it happens once, it cannot be undone, and replacing one silently while somebody edited
 * an unrelated checkbox would be the worst possible way to lose the ability to verify every
 * document already sent.
 *
 * ## Replacing is destructive, and says so
 *
 * The private half is the only thing that can produce signatures this key's public half accepts.
 * Replacing it means every signature already handed to a client stops verifying against the key
 * published from here — the documents are unchanged and their signatures are still valid, but the
 * key that checks them is gone. So a replacement has to be asked for twice, once as a body flag,
 * and the answer says how many renders are affected.
 *
 * See `signing.service.js` for why Ed25519 and a detached file.
 */
router.post(
  '/signing/key',
  requireRole('admin'),
  validate(z.object({ replace: z.boolean().optional() })),
  asyncHandler(async (req, res) => {
    if (!vaultEnabled()) throw badRequest(VAULT_DISABLED_MESSAGE);

    const settings = await Settings.getSettings();
    const existing = settings.signing?.privateKey?.data;

    if (existing && !req.body.replace) {
      const signed = await RenderRecord.countDocuments({
        'signature.fingerprint': settings.signing.fingerprint,
      });
      throw badRequest(
        `There is already a signing key (${settings.signing.fingerprint}). Replacing it means the ` +
          `public key published from here will no longer verify the ${signed} ` +
          `${signed === 1 ? 'document' : 'documents'} already signed with it. Send replace: true if ` +
          'that is what you want.'
      );
    }

    /*
     * The before, for the audit entry. Only the fields worth diffing: the ciphertext of a private
     * key changing is not a fact anybody reads, and putting it in an audit log would be a copy of
     * a secret in a second place.
     */
    const before = {
      signing: {
        enabled: Boolean(settings.signing?.enabled),
        fingerprint: settings.signing?.fingerprint ?? '',
      },
    };
    ...
    /* Replaced wholesale, so Mongoose has to be told: a nested object assigned in one go is not
     * seen as dirty by the change tracker, and the save would write nothing. */
    settings.markModified('signing');
    await settings.save();

    /*
     * Logged as a settings change like the others, and the fingerprint goes in the entry: "who
     * turned signing on and when" is answerable, and so is "which key was in use in March".
     */
    await recordSettingsChange({
      actor: req.user,
      before,
      after: { signing: { enabled: true, fingerprint: key.fingerprint } },
      req,
    });
...
/**
 * Stop signing, and forget the key.
 *
 * Both halves go, including the public one, because a published key nothing can sign with is a
 * misleading thing to leave on an endpoint. Documents already signed keep their signatures and the
 * records keep their fingerprints; what is lost is this instance's ability to hand out the key that
 * checks them, which is why the answer says how many that is.
 */
router.delete(
  '/signing/key',
  requireRole('admin'),
  asyncHandler(async (req, res) => {
    const settings = await Settings.getSettings();
    const fingerprint = settings.signing?.fingerprint ?? '';
    const signed = fingerprint
      ? await RenderRecord.countDocuments({ 'signature.fingerprint': fingerprint })
      : 0;

    const before = { signing: { enabled: Boolean(settings.signing?.enabled), fingerprint } };

    settings.signing = {
      enabled: false,
      algorithm: '',
      publicKey: '',
      privateKey: { iv: '', tag: '', data: '' },
      fingerprint: '',
      createdAt: null,
      createdBy: null,
    };
    ...
    await recordSettingsChange({
      actor: req.user,
      before,
      after: { signing: { enabled: false, fingerprint: '' } },
      req,
    });

    res.json({ enabled: false, removed: fingerprint, orphaned: signed });
  })
);

/**
 * The public key, as a file.
 *
 * Signed in, but not administrator-only: a tester emailing a report is the person who needs to
 * attach this, and it is published information by design — the private half is what matters and
 * that never leaves the vault. Served as a download with the same filename the verification note
 * tells the client to expect, because instructions that name a file the download does not produce
 * are instructions that waste somebody's afternoon.
 */
router.get(
  '/signing/public-key',
  asyncHandler(async (req, res) => {
    const settings = await Settings.getSettings();
    if (!settings.signing?.publicKey) throw notFound('No signing key has been generated.');

    res.setHeader('Content-Type', 'application/x-pem-file');
    res.setHeader('Content-Disposition', `attachment; filename="${PUBLIC_KEY_FILENAME}"`);
    res.end(settings.signing.publicKey);
  })
);

/**
 * Posts a real message to the real URL, and reports what the service said if it refused.
 *
 * The URL comes from the request rather than the database, like the mail and PDF tests, so it can
 * be proved before it is saved — and unlike those two it *has* to be, because the only way to know
 * a chat webhook works is that something appears in the channel. A test that checked the URL's
 * shape would prove nothing: a revoked Discord webhook and a live one are the same string.
 *
 * When nothing is sent the answer includes whether the URL looks like the chosen service's at all,
 * which is the mistake people actually make — a Discord URL pasted under Teams.
 */
router.post(
  '/webhooks/test',
  requireRole('admin'),
  ...
  asyncHandler(async (req, res) => {
    const stored = (await Settings.getSettings()).toObject().webhooks ?? {};
    const provider = req.body.provider ?? stored.provider ?? 'teams';

    /*
     * A URL in the request wins; otherwise the stored one is used, decrypted here and nowhere else.
     * That is what lets somebody press Test on an instance they did not configure — the common case
     * for "has this stopped working?" — without retyping a credential they cannot read.
     */
    let url = String(req.body.url ?? '').trim();
    if (!url) {
      const { webhookConfig } = await import('../services/webhooks/index.js');
      url = webhookConfig({ webhooks: stored }).url;
    }
    if (!url) throw badRequest('Paste a webhook URL first.');

    const result = await testWebhook({
      provider,
      url,
      signingSecret: req.body.signingSecret,
      timeoutSeconds: req.body.timeoutSeconds ?? stored.timeoutSeconds,
    });

    res.json({ ...result, warning: urlLooksWrong(provider, url) });
  })
);
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    /*
     * The webhook URL takes the same road as the SMTP password, and for a stronger reason: for
     * Teams and Discord it is not a password *for* a service, it is the whole authorisation to post
     * as this app for ever. `urlHost` is set alongside it so the settings page can say which
     * service it points at without anything having to decrypt the URL to render a line of text.
     */
    if (body.webhooks && 'url' in body.webhooks) {
      if (!settings.webhooks) settings.webhooks = {};
      const { url, ...rest } = body.webhooks;
      body.webhooks = rest;
      if (url === '') {
        settings.webhooks.url = { iv: '', tag: '', data: '' };
        settings.webhooks.urlHost = '';
      } else if (url) {
        if (!vaultEnabled()) throw badRequest(VAULT_DISABLED_MESSAGE);
        settings.webhooks.url = encryptSecret(url);
        try {
          settings.webhooks.urlHost = new URL(url).host;
        } catch {
          throw badRequest('That webhook URL cannot be read as a URL.');
        }
      }
    }
    if (body.webhooks && 'signingSecret' in body.webhooks) {
      if (!settings.webhooks) settings.webhooks = {};
      const { signingSecret, ...rest } = body.webhooks;
      body.webhooks = rest;
      if (signingSecret === '') {
        settings.webhooks.signingSecret = { iv: '', tag: '', data: '' };
      } else if (signingSecret) {
        if (!vaultEnabled()) throw badRequest(VAULT_DISABLED_MESSAGE);
        settings.webhooks.signingSecret = encryptSecret(signingSecret);
      }
    }
    ...
    settings.markModified('pdf');
    settings.markModified('collab');
