...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    /*
     * Five collections, all `.lean()`.
     *
     * This is the heaviest read in the app — every account, every live engagement with a field from
     * each of its findings and checks, and every booking, hour and day off inside the window — and
     * every document of it was being hydrated into a Mongoose model to be read once and discarded.
     * Nothing here writes, and nothing here needs a document: the rows this page answers with are
     * built from scratch below, field by field, which is why the swap is invisible from outside.
     *
     * The same change on the engagement list measured 5.2× faster; the trash list, which was reading
     * whole findings to count them, 41×.
     */
    const [users, audits, bookings, timeEntries, leave] = await Promise.all([
      ...
      Booking.find({ end: { $gte: window.from }, start: { $lte: window.to } })
        .select('user audit start end')
        .lean(),
      // A time entry is a single day, so a plain range on `day` is the whole story —
      // unlike a booking, which can straddle the window.
      TimeEntry.find({ day: { $gte: window.from, $lte: window.to } })
        .select('user audit day hours')
        .lean(),
    ...
    /*
     * Only bookings on engagements that still exist.
     *
     * The engagement list above has always been filtered to live ones; the bookings were
     * not. So trashing a job left everybody who had been booked on it looking busy for the
     * rest of the window, on an expanded row labelled with no engagement at all — and once
     * the trash was emptied the rows stayed, because bookings were not among the things an
     * engagement takes with it. A day promised to work that no longer exists is a free day.
     */
    const liveAudits = new Set(audits.map((audit) => String(audit._id)));
    const bookingsByUser = new Map();
    for (const booking of bookings) {
      if (!liveAudits.has(idOf(booking.audit))) continue;
