...
/**
 * Already exists — and, where the caller can say so, *which* one.
 *
 * The details carry the thing that clashed, so the browser can offer to open it rather than only
 * refusing. `badRequest` and `unprocessable` already took a second argument; this one did not,
 * which meant a 409 could name the problem and never the record.
 */
export const conflict = (msg = 'Already exists', details) => new HttpError(409, msg, details);
