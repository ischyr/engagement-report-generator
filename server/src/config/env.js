...
export const env = {
  ...
  corsOrigin: (process.env.CORS_ORIGIN ?? 'http://localhost:5173')
    ...
    .filter(Boolean),
  /**
   * Where this instance lives, for a link in an email.
   *
   * A notification that says "somebody asked you to review PT-2025-004" is worth little without a
   * way to get there, and a relative path in a mail client goes nowhere. Falls back to the first
   * allowed CORS origin, which for a normal deployment is the same host the browser uses.
   */
